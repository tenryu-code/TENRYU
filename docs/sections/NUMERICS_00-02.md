...
**T = 0 等温線（cold curve）の保持（2026-09-15）**：SESAME 301/304 の温度格子は T = 0 K の行（cold curve）から始まる。log 双線形補間は T = 0 のノードを持てないため、従来はこの行を捨て、最初の正の等温線（5263 重水素は 290 K、7592 ポリスチレンは 301 が 145 K・304 が 2901 K）より低温の状態をその等温線にクランプしていた — 液体 D2（0.17 g/cc、1 meV）が 290 K の \(P=2.9\) kbar で評価され（cold curve では 0）、CH 側（0 kbar）との圧力差で界面が動いていた（NIF DS デッキ、0.5〜2 ns で D2 側 −12 %・CH 側 +11 % の密度変化）。現在は最後の非正の行を T = 0 のアンカーとして保持し、`sesame_cold_curve_rows`（既定 12）本の合成行 \(T_k=T_1/2^k\)（\(k=1..12\)）に、cold curve と最初の等温線の間で T について線形な補間 \(f(\rho,T)=f_c(\rho)+(T/T_1)\,(f_1(\rho)-f_c(\rho))\)（P, e とも）を置く。既存の補間器はこのモデルを隣接行間の log 補間誤差（熱的部分の 7 % 未満）で再現し、\(c_v\) は行間で \((e_1-e_c)/T_1\) に一致する。最下行 \(T_1/2^{12}\) 未満はその行にクランプ。0 を指定すると従来どおり行を捨てる（bit 恒等）。EOS 署名は行数と \(T_{\min}\) を含むので、既定では古いチェックポイントと一致しない。

**低温平衡構成則（`Numerics.hydro.T_start_inactive_cells="cold_equilibrium"`、2026-09-16）**：表の極低温側（cold curve の平坦化・張力・スピノーダル）に依らず、指定した参照状態 \((\rho_0, T_{e0}, T_{i0})\) を全圧 \(P_0\)・体積弾性率 \(K_0\) の力学平衡に置き、閾値 \(T_*=\)`T_start_eV` を超えた電子温度では厳密に元の表へ戻す構成則。単一ポテンシャルから導く電子側の補正 \(G(v,T_e)=w(T_e)\,C(v)\)（\(v=1/\rho\)）を用い、
\[
P_{cold}(v)=P_0+K_0\left(\frac{v_0}{v}-1\right),\quad
C_0(v)=\int_{v_0}^{v}\bigl[P_N(s)-P_{cold}(s)\bigr]\,ds,\quad
P_N(v)=P_i^N(v,T_{i0})+P_e^N(v,T_{e0}),
\]
\(C=\chi(\rho)C_0\)（\(\chi\) は \(|\ln(\rho/\rho_0)|\le\ln 1.10\) で 1、\(\ln 1.50\) で 0 になる 5 次スムーズステップ \(S_5(s)=10s^3-15s^4+6s^5\)）、\(w(T_e)\) は \(T_e\le T_*/2\) で 1、\(T_*\) で 2 階微分まで 0（同じ \(S_5\)）。修正量は
\[
...
- **非活性セルの扱い（`hydro.T_start_inactive_cells`、2026-09-14 追加）**：
  ...
  - `"cold_equilibrium"`（2026-09-16）：hydro のマスクを行わない（`hydro_active` は全セル 1、`T_start_eV` は §1 (b) の低温平衡構成則の電子温度遷移の終点として使う）。冷たいセルも通常どおり力・慣性・仕事を持ち、衝撃波は透過する。
