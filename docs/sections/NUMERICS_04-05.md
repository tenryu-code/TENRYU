...
両端は零流束、`cell_is_void` のセルは恒等行。線のセル数が 640 以下なら 1 ブロックの並列巡回縮約（`core::pcr_solve_strided`、FLD の灰色系と同じ）で、それより長い線は cuSPARSE の gtsv2（電子の陰解法と同じ）で解く: 結合した行は対角に \(M/\Delta t>0\) を持つ狭義の優対角行列なので、並列巡回縮約にピボット選択は要らない（2 セル以下も同じ GPU の経路）。1 ブロックの縮約は倍精度の計算が 1 つの SM に集まるので、長い線では複数ブロックの gtsv2 の方が速い（RTX 4090 で 720 セル付近が境目）。解法による違いは丸め誤差の程度。
