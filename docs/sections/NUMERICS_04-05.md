...
- **臨界近傍ガード**：いずれかのLaserMeshセルで \(\hat{n}_i > \hat{n}_{margin} - \varepsilon_{crit\_guard}\)
  （既定 \(\varepsilon_{crit\_guard} = 0.01\)）が成立する場合は強制的に再計算する。
  臨界面近傍では \(\hat{n}\) の微小変化がレイ terminate の有無を左右し、
  吸収パターンが不連続に変化し得るため、Skip は安全でない
