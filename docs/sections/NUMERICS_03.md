...
走査し、そのセルを中心とする半幅 `relief_halfwidth_cells`（既定 3）の窓の
span 保存 respace（per-cell 目標 \(t_i=\min(\text{target\_factor}\cdot
\text{floor\_cm},\,w_i\cdot\text{max\_growth\_factor})\)、固定 8 回反復）が
当該セル幅を厳密改善（≥0.1%）する最初のセルを採用する。pinned node は窓の
分割壁、密度床セル（\(\rho < 100\rho_{floor}\)）を含む窓は対象外、remap の
one-cell sweep 述語に対する安全クランプを候補側でも課す。改善可能なセルが
無い場合は `no_relief_available` を自己申告し remap を起動せず復帰する
（span 保存の局所窓は広域 under-floor 帯（例: HELIOS 移植メッシュの外側
