...
### 3.2 性能
- 実測 KPI・プロファイル・ホスト側オーバーヘッドの現況は `PERFORMANCE.md` を正とする（決定論輸送化後の主律速はホスト側 CUDA driver 経路 — ホスト側オーバーヘッド削減の系列で追跡）
...
- `dimension: Literal["1D_SPH","1D_CYL","2D_RZ"]`
  - `"1D_CYL"`：1D cylindrical per-unit-length radial geometry = `"1D_SPH"` with `Mesh.geometry_1d="cylindrical"`（2026-09-24: the builder maps it so; every physics that supports the cylindrical 1D geometry — FLD/S_N radiation, conduction, `Laser.mode="radial_absorption_1d"` and (2026-09-24) `"raytrace_2d"` with the characteristic integrator, ALE — is available, and the frozen config records `"1D_SPH"` + `geometry_1d="cylindrical"`; `Mesh.geometry_1d="planar"` with it is a ConfigError; checkpoints of earlier `"1D_CYL"` runs restart. Until 2026-09-23 the initial scope was the pure hydro core and radiation/laser/conduction/ale1d were rejected; NUMERICS 1D geometry note; verification gate H3-RADIAL-CYL）
...
| レベル | ソルバーが追加で出す行 |
|--------|---------|
| quiet  | 進捗行を出さない（退役した imc_ddmc 経路の一部の警告も出さない） |
| normal | 100 step ごとの進捗行 `[progress]` |
| verbose | 毎 step の進捗行と、時間刻みの内訳 `[dt_breakdown]`・段ごとの時間計測などの診断行 |
| debug  | 進捗行を出さない。再試行経路の一部の診断（`[driver-retry]`）を出す |

`verbosity` はソルバーが自分で出す上表の行だけを切り替え、ほかのログ行は `verbosity` によらず出る。表示するログの最低レベルは CLI フラグで決まる：既定は INFO 以上、`--verbose` で DEBUG 以上、`--quiet` で WARNING 以上。CLI フラグは `verbosity` の値を変えない。
...
- 2D_RZ：
  ...
  - `multiblock_cart_core_r_match: float` [cm]（cart-core 既定 `2*multiblock_cart_core_r_c`、polar-tier 既定 `spherical_polar_s_max/6`）。Hermite bridge/fan または polar-tier center と polar shell の接続半径。cart-core family では `sqrt(2)*r_c < r_match < spherical_polar_s_max`、`multiblock_polar_tier` と `multiblock_polar_tier_cart_center` では `0 < r_match < spherical_polar_s_max`（hybrid でも r_match は親 polar ladder のスケール入力のまま — cart 側 seam 半径は実現値 `r_cut` であり本キーからは読まない）。
  - `polar_tier_chi_lo: float`（既定 `0.75`；有限、`>0`、単位なし）および `polar_tier_chi_hi: float`（既定 `1.50`；有限、`>polar_tier_chi_lo`、単位なし）。`multiblock_polar_tier` 専用。nominal \(h_r=(s_{\max}-r_{\rm match})/nr\) と \(N_t=nz/2^t\) に対して \(s_{\rm tr}=\chi_{\rm lo}h_rN_t/\pi\) を既存 center radial face へ最近傍 snap する。snap 後は \(\chi_{\rm fine}\in[0.70,0.85]\)、\(\chi_{\rm coarse}\in[1.40,1.70]\) 必須。
  - `polar_tier_fan_sectors: Literal[6,12]`（既定 `12`；単位なし）。origin fan の triangle 数。`polar_tier_min_tier_columns` と同値でなければ `ConfigError`。
  - `polar_tier_min_tier_columns: int`（既定 `12`；単位なし）。innermost regular tier の angular columns。`nz/polar_tier_min_tier_columns` は 2 の整数べきでなければならない。
  - `polar_tier_fan_first_ring_radius_cm: float` [cm]（既定 `0.0`；有限、`>=0`）。`multiblock_polar_tier` 専用。`0.0` は AUTO \(s_1=h_r/\sin(\pi/N_0)\)、正値は fan first-ring radius \(s_1\) の明示 override。各 tier は区間 \([s_{\rm inner},s_{\rm outer}]\) に対し \(n=\max(1,\operatorname{round}((s_{\rm outer}-s_{\rm inner})/h_r))\)、\(h'_r=(s_{\rm outer}-s_{\rm inner})/n\) の tier 内一様 ladder を用い、\(s_{\rm inner}+kh'_r\) で両端へ exact landing する（追加 snap ring は作らない）。construction gate は全 tier で \(0.5h_r\le h'_r\le1.5h_r\)、全 ring で \(s\Delta\theta/h_r\ge0.5\)、fan altitude で \(s_1\sin(\pi/N_0)\ge0.9h_r\) を要求し、radial nestedness の参照間隔は \(\min_t h'_{r,t}\)。
  - `polar_tier_hydro_enabled: bool`（既定 `False`）。Phase III-b の fail-loud hydro gate。`Numerics.hydro.enabled=True` と `topology_scheme="multiblock_polar_tier"` の組合せは、この key が `True` であり、かつ `rz_momentum_scheme="area_weighted_symmetric"`, `aw_compatible_force_work=True`, `av_model="csw_edge"` / `"csw_edge_csw98"` / `"mimetic_tensor_v1"`, `subzonal_pressure_enabled=True` の mixed-cell force trio が揃う場合だけ有効。それ以外は `ConfigError`。既定 False は legacy path と HDF5 schema を変更しない。
  - `polar_tier_belt_thickness_frac: float`（default `0.0` = legacy Chebyshev belt construction BIT-EXACT; valid `[0.0, 0.9]`. `>0` sets each transition belt's radial thickness to `frac*dr_local` centered on the legacy belt center (adjacent tier rows absorb the change; ConfigError if a row would go non-positive). Raises the five-triangle role-2/3 sliver margin against converging-shock inversion while keeping pure-Lagrangian physics（W2 2026-07-30; discontinuous as frac→0+, experiment knob. Measured caveat: single-row template altitude is template-internal — thickening alone does NOT raise the role-2/3 sliver altitude; use `polar_tier_belt_rows` for that））
  - `polar_tier_dendrite_enabled: bool`（default `False` = bit-exact legacy topology. Opt-in static polar dendrite（P4-A, 2026-07-31）: the polar cap（W=16 master cells = 15° per pole, both poles bitwise-mirrored）is angularly coarsened per level to the measured survival widths — shell 0.9375°（unchanged）, T1 1.875°, T2 7.5°, T3–T5 15° — via two additional θ-transition rows（`S_theta` consuming T1's first radial row joining 16→8 cap intervals; `S_theta2` consuming T2's first row joining 4→2）built from the existing five-triangle rosettes plus 1:1 quads, all derived from integer master-label join descriptors（gap/overlap/ratio>2 = ConfigError before allocation）. Topology becomes 13 blocks / 69,338 cells / 69,371 nodes at 288×192（layout-derived, asserted）; checkpoints do not carry across the setting. v1 fail-loud contract: requires Ntheta=192, belt_rows=1, thickness_frac=0, pole-cap map off; mutually exclusive with `rezone_solver="m1_tmop"` and `euler_window(s)`（unqualified on nonuniform ladders）. The transition rows are TRANSITION_BELT-role and are protected by the descriptor-driven band-ALE belt bands whose lines include the Chebyshev center chains（bare runs die at first shock contact on unprotected rosettes, as any unprotected five-triangle row does）. Measured status（2026-07-31, 8 iterations）: the legacy t=3.229×10⁻¹⁰ tier-3 axis wall is REMOVED（15° axis cells）, but the shallowest transition's own axis rosette undergoes uniform terminal compression at t=3.176×10⁻¹⁰ — a net −1.6% vs the non-dendrite production config; root-cause consultation in progress（pentagon-vs-quad terminal-compression gap; subzonal is fully implemented for triangles but its spread-driven merit is correctly silent under uniform compression））
  - `polar_tier_dendrite_s_theta_rows_below: int`（default `5`; valid `[1,40]`; number of T1 rows below the S_theta transition（T1b, on the 8-ladder）. Placement sweep（2026-07-31）: 5（r≈13.4µm）is optimal — higher placements degrade（14.7µm: 3.145）and cross into the shock-crossing-kill regime beyond ≈15-16µm（16.4µm: 3.019）））
  - `polar_tier_native_pentagon: bool`（default `False` = bit-exact legacy rosettes; requires `polar_tier_dendrite_enabled=True`（ConfigError otherwise）. Opt-in native-pentagon transitions（2026-07-31 campaign, in progress）: every TWO_TO_ONE join in the dendrite descriptor set is emitted as ONE native five-vertex cell `[coarse m, coarse m+1, fine k+2, fine k+1, fine k]`（legacy rosette outer-perimeter winding, orientation gate `area2>0` preserved）instead of five triangles around a Chebyshev center — the center nodes cease to exist, removing the sliver-altitude wall class at its root. Corner storage stride for the whole configuration widens 4→8（`core::corner_stride_for_config`; identical to `pentagon_belt_shell`'s stride; v4 checkpoint schema records the stride — restart requires an exact stride match, pre-v4 checkpoints and v1 legacy checkpoints are rejected fail-loud）. Kernel support status（measured 2026-07-31, NUMERICS §3.2.9d）: the full production trio runs end-to-end on pentagons — AWS pressure via the statically condensed virtual fan, five-corner Q_k subzonal pressure with common-mode projection, Q_k planar nodal mass, five-edge CSW98（missing-neighbor limiter fallback）, and the pentagon acoustic length min(2A/P, fan altitude) in every CFL mode. Measured: bare native run（no band ALE, production trio, 288×192）survives unprotected shock passage through every radial transition and dies at the terminal axis bulk-compression epoch（t≈3.107×10⁻¹⁰, same wall as the rosette OFF baseline 3.110×10⁻¹⁰）— the historic bare-rosette shock-contact death（≈1.9–2.0×10⁻¹⁰）is structurally removed. Pending: band-ALE respace on native transitions（rejected fail-loud until the pentagon swept-remap stage lands — fraction-transported corner masses, GCL and bow-tie path-subdivision gates）; restart（pre-v4/v1 checkpoints and stride mismatches reject fail-loud）; contract rank/stiffness observability gates and the optional affine-null damping operator）
  - `polar_tier_pole_cap_m: int` / `polar_tier_pole_cap_alpha: float`（defaults `0` / `0.0` = OFF, bit-exact; valid `{0}∪[4,48]` / `[0,1]`; active iff both positive. Capped equal-solid-angle pole map: the first `m` master-grid θ cells blend toward the equal-μ ladder with a quintic smoothstep (first-cell widening ≈`1+α(√m−1)`), evaluated as one continuous map in master coordinates so all rings (shell, tiers, zipper intermediates) stay aligned; exact endpoints and bitwise south mirror untouched. Measured (2026-07-31): the construction q-gate (isoperimetric 0.20 floor, binding at outer-shell axis cells) caps admissible widening at β≈1.5, and the cap's interior compression makes the five-triangle belt slivers dynamically more brittle — net-negative on the production topology; retained as an experiment surface for belt-template-aware pole designs（W16））
  - `polar_tier_belt_rows: int`（default `1` = legacy single-row five-triangle belt BIT-EXACT; allowed `{1, 2, 3}`. `>1` replaces each 2:1 transition belt with a multi-row gradual join spread over `rows` radial rows (rows=2: 4→3→2 column template, requires the belt's coarse column count divisible by 4; rows=3: 6→5→4→3 zipper, divisible by 6 — ConfigError otherwise). Each rows>1 belt absorbs one radial row from each adjacent tier that can donate (donor keeps ≥1 row; the innermost 1-row tier never donates), growing the span from the legacy inter-tier gap to ≈gap+2Δr so per-row height reaches tier-spacing scale; `polar_tier_belt_thickness_frac>0` is rejected together with rows>1 (conflicting span policies). Rationale is measured (2026-07-30): subdividing the legacy span alone dropped the absolute minimum altitude (0.081 vs 0.106 µm) and left the converging-shock death time unchanged — absolute margin governs, so the span must widen. NUMERICS §2 polar-tier construction has the template details. Changes topology counts (n_cells/n_nodes) — old checkpoints are incompatible for this opt-in setting（W6B 2026-07-30, experiment knob））
  - `multiblock_cart_core_n_c: int`（既定 `nz/4`；有効範囲：`>=4` かつ `nz=4*n_c`；単位なし）。Cartesian half-core の片側分割数で、half-square/fan boundary は \(4n_c\) segments を持つ。`topology_scheme="multiblock_cart_core_polar_shell"`、`"multiblock_half_butterfly_5block"`、または `"multiblock_half_butterfly_trifan_cap_5block"` の場合のみ有効。`"multiblock_polar_tier_cart_center"` では派生値 `N_theta(cut)/4` が正であり、本キーは未設定（`-1`）のままにするか派生値と厳密一致させる（不一致は `ConfigError`）。
  - `multiblock_cart_core_bridge_layers: int`（既定 `max(4, round(n_c/8))`；有効範囲：`>=1`；単位なし）。Cartesian half-core と polar shell の間の Hermite bridge 層数、または 5-block fan radial layer count。`topology_scheme="multiblock_cart_core_polar_shell"`、`"multiblock_half_butterfly_5block"`、`"multiblock_half_butterfly_trifan_cap_5block"`、または `"multiblock_polar_tier_cart_center"`（`>=1` 必須）の場合のみ有効。
  - `polar_tier_cart_cut_ring: int`（既定 `-1`；`topology_scheme="multiblock_polar_tier_cart_center"` 専用で `>0` 必須 — 他 scheme で `-1` 以外を指定すると `ConfigError`）。polar-tier 本体の外向き正準 node-ring counter（fan first ring = 0、tier row と belt row を外向きに加算、shell 内端 ring が最大値）における切断 ring。cut ring は kept 側の内端 node ring（= 将来の seam ring）になる。許容条件: 構造化 tier 内部の ordinary ring（全 belt support row から 3 row 以上離れ、transition join footprint 外）、その tier の \(N_\theta\) が 4 の倍数、fan より上かつ shell より下。違反は最近傍の許容 ring を列挙する `ConfigError`。dendrite 有効構成の切断は W3c の schedule descriptor 着地まで fail-loud 拒否。
  - `polar_tier_center_kind: Literal["cart_box","trifan_cap"]`（既定 `"cart_box"`；`topology_scheme="multiblock_polar_tier_cart_center"` 専用）。`"cart_box"` は従来の Cartesian center を bit-exact に維持する。`"trifan_cap"` は \(N_{\rm cap}=n_c\)、\(N_\theta=4n_c\) の equiangular concentric cap（apex は厳密に \((0,0)\)、第 1 row は三角形、以後は四角形）を同じ Hermite bridge の内側へ接続する。他 scheme で非既定値を指定すると `ConfigError`。
  - `multiblock_cart_core_bridge_grading: Literal["uniform","quintic_log","log"]`（既定 `"uniform"`；単位なし）。`topology_scheme="multiblock_cart_core_polar_shell"` または `"multiblock_polar_tier_cart_center"` の場合のみ指定可能。`"quintic_log"` は `multiblock_transition_scheme="hermite_bridge"` を必須とし、core spacing から最初の shell spacing まで bridge layer を quintic-log grading する。`"log"` は seam の measured kept-side gap を anchor として内向きに進み、各 step の adjacent spacing ratio を `[1/multiblock_cart_core_bridge_ratio_max, multiblock_cart_core_bridge_ratio_max]` に clamp してから \(\max(\mathrm{floor},\chi r)\) に従う。\(\chi\) は全 layer で cap radius に厳密に着地するよう求解され、`multiblock_polar_tier_cart_center` + `trifan_cap` の場合のみ有効。
  - `multiblock_cart_core_bridge_spacing_floor: float` [cm]（既定 `0.0`；有限、`>=0`）。`multiblock_cart_core_bridge_grading="log"` を用いる `multiblock_polar_tier_cart_center` 専用の絶対 bridge spacing floor。
  - `multiblock_cart_core_bridge_ratio_max: double`（既定 `1.05`；有限、`>1`；単位なし）。`multiblock_cart_core_bridge_grading="log"` を用いる `multiblock_polar_tier_cart_center` 専用の adjacent bridge-spacing ratio bound。
  - `multiblock_theta_cap_widen_factor: float`（既定 `1.0`；有効範囲：有限かつ `>=1`；単位なし）。`topology_scheme="multiblock_cart_core_polar_shell"` 専用。両極に隣接する theta column をこの係数で広げ、内部 column を一様に狭めて column 数と全角度を保つ。`1.0` は bit-frozen equiangular ladder を保持する。
  ...
  - `multiblock_outer_svec_tangent_balance: bool`（既定 `true`）。multiblock outer-shell の各 non-pole node で、隣接する corner Svec pair の接線方向和を geometry recompute ごとに射影して I1-B S3-T3 G1 constant-state seam-GCL closure を保つ。strong drive では outer arc の接線復元力を毎 step 消去して pole-adjacent outer cells を不安定化するため、boundary-acceleration projection への置換が入るまでは drive deck で `false` にすること。
...
- 共通：
  ...
  - `rezoning` / `numerics.ale: dict`
    - `enabled: bool`（既定 False；2D_RZ ALE 用。使うときは True を明示する。1D_SPH では使用しない）
    - `mesh_mode: Literal["fixed","reale_v2"]` (default `"fixed"`). `"fixed"` preserves the existing mesh paths. `"reale_v2"` is the host-side Stage-2 pilot: after each accepted full Lagrangian step commits, it advects cell-centroid generators with cell-averaged nodal flow, applies the constrained-Voronoi/CVT target builder on the current outer-boundary-plus-axis domain, conservatively remaps hydro fields, and installs the rebuilt CSR topology before the next step; in this mode every cell has 16 vertex slots (`kMeshTopoCellStorageSlotsMaxGeneral`) from initialization on. The pilot is accepted only for `Main.dimension="2D_RZ"`, one MPI rank, exactly one material, no PLIC or per-material conservation state, hydro enabled, radiation/conduction/laser/burn disabled, and `Mesh.topology_scheme="multiblock_polar_tier"` with `polar_tier_native_pentagon=True`; every other combination is a `ConfigError`. A target, tessellation, topology, or remap rejection leaves the accepted Lagrangian state unchanged for that cycle and emits `[reale_v2] SKIP ... skipped_total=N`; successful commits emit `[reale_v2] COMMIT ...`. The mode makes no bitwise-reproducibility claim. The default value is frozen-config elided; legacy default completion supplies `"fixed"`.
    - `TENRYU_REALE_REZONE_INTERVAL` (environment-only; default `1`; valid range: positive integer) is the rezone trigger ceiling under `mesh_mode="reale_v2"`. A rezone fires when (a) any node has moved at least half the local cell scale, `sqrt(A)/2`, since the last committed rezone, (b) the hydro dt bound has halved relative to its first post-commit value, or (c) this many steps have elapsed since the last commit. An evaluation without a matching displacement reference (the first of the run, or a node-count change since the last snapshot) fires unconditionally, logged as reason `first`. The default `1` preserves the historical every-step behavior. The value is parsed once and invalid input fails fast; this knob has no namelist or frozen-config field.
    - `reale_core: Literal["exact","legacy"]` (default `"exact"`). Selects the tessellation authority used by `mesh_mode="reale_v2"`; the exact Delaunay/restricted-Voronoi core is the production default. `"legacy"` is accepted only when `mesh_mode="fixed"`, where the key is inert. Combining `"legacy"` with `mesh_mode="reale_v2"` is a `ConfigError` because the legacy builder emits no boundary provenance and every rezone fails the carrier gates. The default is frozen-config elided and legacy frozen-config completion supplies `"exact"`. The historical `TENRYU_REALE_CORE` environment variable is retired and ignored.
    - `rezone_min_dt_s: float` (default `1.0e-14` s; valid range `>= 0`). **PRODUCTION REQUIREMENT**, in the same parameter class as `Numerics.dt.max_s`, not a derived numerical tolerance. After staged remap and carrier projection, the ReALE transaction evaluates the staged edge-AV and node-crossing bounds before installing any staged arrays. A staged minimum `<= rezone_min_dt_s` rejects through the ordinary `staged_dt` skip path, with no rollback machinery because no install has occurred; `0.0` disables evaluation and negative values are a `ConfigError`. Per design §5/D5, “the requirement is deck-owned”; the dt requirement must not be folded into the weld criterion. The default is frozen-config elided and legacy default completion supplies `1.0e-14`.
    - `tess_gpu_dual: bool` (default `False`). Opt-in GPU offload of the exact per-triangle circumcenter construction inside the restricted-Voronoi dual build (`mesh_mode="reale_v2"` exact core). The device path reproduces the host expansion arithmetic term sequence byte-for-byte (per-triangle host fallback for anything the fixed-capacity device path cannot certify), so tessellation output is bitwise identical to the host path; `False` keeps the pure host path. Requires a CUDA device at rezone time when enabled (fails loud, no silent host downgrade). The default is frozen-config elided and legacy completion supplies `False`.
    - `tess_gpu_restrict: bool` (default `False`; requires `tess_gpu_dual=True`, else `ConfigError`). Opt-in GPU certified classification of restricted-Voronoi dual nodes against the domain polygon (winding with error-bound-certified signs on the device-resident dual expansions; any uncertain or boundary-touching node falls back to the unchanged host classification), bitwise identical to the host path by construction. The default is frozen-config elided and legacy completion supplies `False`.
    - `dvclp_solver_rev: int` (default `0`; valid values `{0, 1}`, anything else a `ConfigError`). DV-CLP Newton-stage solver revision. `0` is the legacy bit-path: the semismooth-Newton KKT solve uses the diagonal-pivot dense LDL^T with drop-on-zero-pivot recovery, byte-for-byte unchanged. `1` additionally runs a deterministic column-pivoted Householder QR over the active gradient columns at every Newton refactor entry and drops linearly dependent rows before factorization, so systematic rank deficiency (e.g. pure-z difference rows closing graph cycles, which the duplicate-support dedup cannot see) never reaches the LDL^T and the solve path stops depending on FMA rounding (NUMERICS §15.4a; design doc `dvclp_newton_quality_20260813` §7 P-B). Row drops cannot affect correctness — feasibility scans, alpha, and certificates evaluate edges, not rows — only Newton solution quality. The default is frozen-config elided and legacy completion supplies `0`.
    - `reale_lloyd_max: int` (default `4`; valid range `[0, 64]`, anything else a `ConfigError`). Lloyd (CVT compactness) iteration ceiling per rezone in `build_generator_targets` (`mesh_mode="reale_v2"`). Each iteration pays at least one full tessellation build through the sweep-blend admissibility transaction; the d1 certified skip and the d2 representation-spacing floor still exit earlier when they certify. `0` disables Lloyd compactness (advection-only targets). The default reproduces the historical fixed ceiling bit-for-bit and is frozen-config elided; legacy completion supplies `4`. Exposed 2026-08-16 for the S1 rezone-economics lever (design doc `rezone_semantic_levers_20260816` §S1); changing it is a mesh-quality/performance trade owned by the user.
    - `reale_short_edge_collapse_rel: float` (default `3.0e-2`; values `<= 0` disable). Relative geometric short-edge band at the ReALE install boundary: after the staged tessellation mesh is validated and before its first remap/install consumer, an edge `(u,v)` is collapsed when `l^2 < reale_short_edge_collapse_rel^2 * min(A_h(u), A_h(v))`, where `A_h` is the minimum planar area of the staged cells incident on the node. Master/master merges are forbidden, boundary-carrier pairs require an equal carrier-edge id, and the deterministic representative priority is master, slave, then other, with lowest node id breaking ties. The weld-stage geometric band and DCEL tolerated path remain opt-in instrumentation and are inert by default. The default is frozen-config elided; legacy completion supplies `3.0e-2`.
    - `reale_subdomain_rezone: bool` (default `False`). Opt-in ReALE subdomain path: classifies and dilates disturbed cells, fills and merges connected-component hulls, tessellates each component against its current-mesh boundary loop, splices it into the preserved global CSR with hanging interface vertices, and remaps only the selected region plus its one-ring donor support. Preserved cells and nodes retain identity and state except for geometric corner-mass repartition on preserved cells that gain hanging vertices. Components touching the true boundary inherit the persistent physical carrier; internal interface carriers are temporary and are discarded at splice. `False` uses the unchanged full-domain path. The default is frozen-config elided; legacy completion supplies `False`. See NUMERICS §3.3.18.
    - `reale_subdomain_frac_max: float` (default `0.6`). Maximum dilated disturbed-cell fraction admitted by the subdomain path. A larger fraction, or unavailable face adjacency, falls back to the unchanged full-domain rezone. The default is frozen-config elided; legacy completion supplies `0.6`. See NUMERICS §3.3.18.
    - `reale_overlay_additivity_tol: float` (default `1.0e-4`; values `<= 0` disable bounded acceptance). When an old- or new-overlay additivity audit fails its strict bound, the audit is tolerated only if the worst relative error is at most this value and the failing-cell count is no more than 0.1% of the corresponding cell count; otherwise the remap is skipped as before. A non-positive value restores the previous strict rejection behavior exactly. The default is frozen-config elided; legacy completion supplies `1.0e-4`. See NUMERICS §3.3 and the ReALE defect dossier §12-P2.
    - `reale_corner_mass_reset: bool` (default `True`). After each successful ReALE conservative remap and before carrier projection, rebuilds the new mesh's active corner masses as `m_ci = m_c * V_ci / sum_i(V_ci)` using the same RZ corner-subpolygon volumes as the compatible subzonal-pressure force, and zeros padded slots. `False` preserves the previous transported corner masses byte-for-byte. The default is frozen-config elided; legacy completion supplies `True`. See NUMERICS §3.3.16 and the ReALE defect dossier §9.
    - `reale_velocity_max_principle: bool` (default `False`). After conservative ReALE velocity remap and before carrier projection, clamps each new nodal velocity component to the relaxed local range of old-node velocities within `5.0e-5` cm, widening once to `1.0e-4` cm when fewer than three donors are found. The componentwise pad is 5% of the donor range and vanishes for a degenerate range; nodes with fewer than three donors after widening remain unchanged. `False` preserves the previous remapped velocities byte-for-byte. The default is frozen-config elided; legacy completion supplies `False`. See NUMERICS §3.3.17 and the ReALE defect dossier §11. default off (2026-08-17): measured to worsen the Voronoi-hydro drift by interfering with the remap's stabilizing role — dossier §11.
    - `reale_dt_trigger_factor: float` (default `0.5`; valid range `(0, 1]`, anything else a `ConfigError`). Factor in the ReALE dt-halving rezone trigger: after a commit establishes the post-rezone hydro-dt reference, a new rezone triggers when `dt_hydro <= factor * dt_ref` (`[reale_trigger] reason=dt`). The default multiplies by the same `0.5` the trigger previously hard-coded, so decisions are bit-identical. Frozen-config elided; legacy completion supplies `0.5`. Design doc `rezone_semantic_levers_20260816` §S3a.
    - `reale_dt_trigger_cooldown: int` (default `0` steps; valid range `[0, 100000]`, anything else a `ConfigError`). Minimum committed-rezone age (in steps, compared as `steps_since >= cooldown`) additionally required before the dt-halving trigger may fire; the ceiling/disp/first triggers are unaffected. `0` adds no constraint (the conjunct is vacuous on that path), reproducing current behavior exactly. Frozen-config elided; legacy completion supplies `0`. Design doc `rezone_semantic_levers_20260816` §S3b.
    ...
    - `runtime_controller: dict`（既定 `{"monitor_enabled": False, "monitor_every": 1, "shell_rows": 4, "controller_shell_rows": 4, "cap_columns": 2, "q_soft": 0.42, "q_hard": 0.22, "q_recover": 0.60, "h_soft": 8.0, "h_hard": 4.0, "h_recover": 12.0, "soft_persistence": 2, "recover_checks": 3, "winslow_sweeps": 4, "winslow_omega": 0.2, "beta_monitor_soft": 0.12, "beta_monitor_hard": 0.25, "beta_mass": 0.12, "beta_front": 0.12, "beta_theta": 0.15, "g_max": 1.22, "front_width_cells": 2.5, "cap_fraction": 0.05, "cap_normal_fraction": 0.025, "controller_enabled": False, "commit_rollback_enabled": True, "activation_front_mode": "min", "activation_front_margin_hs": 2.0, "activation_time_s": -1.0, "cadence_soft": 2, "cadence_hard": 1, "cadence_recovery": 4, "pre_step_enabled": True, "failures_hard_force": 4, "failures_big_repair": 8, "escalation_max_failures": 12}`；W2 の runtime mesh-health monitor、W3a target-construction library、W3b transactional controller、W3c post-crossing activation gate。`monitor_enabled: bool`; `monitor_every: int >= 1` evaluations; `shell_rows: int >= 0`; `controller_shell_rows: int >= 0`; `cap_columns: int >= 0`; quality thresholds must satisfy `0 < q_hard < q_soft < q_recover <= 1`; horizon thresholds must satisfy `0 < h_hard < h_soft < h_recover`; `soft_persistence: int >= 1`; `recover_checks: int >= 1`。W2 監視領域は core+bridge と innermost shell rows、出力は `[ale-state]` および optional additive `/diagnostics/ale_state/{step,t_s,state,q_min,h_min,q_min_cell,h_min_cell}`。`monitor_enabled=False` では kernel launch、reference snapshot、history group のいずれも発生しない。W3a は target construction library、W3b は monitor state cadence に従って W3a target を reference-barrier transaction へ渡し、W3c は cadence 前に sector-front crossing または明示時刻で activation を gate する。NUMERICS §3.3.3）
      - `controller_shell_rows: int`（既定 4；有効範囲 `>=0`；controller-era node mandate と active-cell mask に含める innermost shell rows。row/ring 0 は seam であり controller era では可動だが、button-morph-era mask contract は変更しない）
      - `cap_columns: int`（既定 2；有効範囲 `>=0`；各 axis に隣接して generic 2-D target から除外し、fixed-ray common radial cap target と feasible projection を適用する angular cell columns 数。0 で W7b cap target を無効化）
      - `winslow_sweeps: int`（既定 4；有効範囲 `[1,16]`；virtual fixed Jacobi sweep count）
      - `winslow_omega: float`（既定 0.2；有効範囲 `(0,1]`；virtual Winslow relaxation）
      - `beta_monitor_soft`, `beta_monitor_hard: float`（既定 0.12, 0.25；各有効範囲 `[0,1)`；soft/hard monitor-target blend weight）
      - `beta_mass`, `beta_front: float`（各既定 0.12；各有効範囲 `[0,0.5]`、かつ和 `<=0.9`；bridge radial composite-monitor weights）
      - `beta_theta: float`（既定 0.15；有効範囲 `[0,0.5]`；weak equal-angle restoration weight）
      - `g_max: float`（既定 1.22；有効範囲 `(1,1.6)`；adjacent radial-spacing ratio cap）
      - `front_width_cells: float`（既定 2.5；有効範囲 `[1,8]`；front Gaussian width in local radial cells）
      - `cap_fraction: float`（既定 0.05；有効範囲 `(0,0.2]`；total target displacement cap relative to local characteristic length）
      - `cap_normal_fraction: float`（既定 0.025；有効範囲 `(0,cap_fraction]`；shock-band normal target displacement cap）
      - `controller_enabled: bool`（既定 False；W3b runtime motion controller。True には `monitor_enabled=True` が必須）
      - `commit_rollback_enabled: bool`（既定 True；W7e future-feasibility commit gate failure で runtime-ALE transaction を rollback。False では同じ warning を出すが commit を維持）
      - `activation_front_mode: Literal["min", "mean"]`（既定 `"min"`；activation gate に使う有効な sector-front の統計量）
      - `activation_front_margin_hs: float`（既定 2.0；有効範囲 `>=0`；`activation_front_mode` で選択した有効な sector-front 統計量が初期 bridge layer-0 core--bridge 境界半径にこの initial shell seam spacing 数を加えた閾値より内側へ進み、bridge 全体を通過してから controller event を許可）
      - `activation_time_s: float`（既定 -1.0 s；sector detector が利用できない場合の明示 activation 時刻。`>0` で `state.t>=activation_time_s` を gate とし、`<=0` では detector なしの controller は inactive のまま）
      - `cadence_soft`, `cadence_hard`, `cadence_recovery: int`（既定 2, 1, 4 steps；各 `>=1`、`cadence_hard <= cadence_soft <= cadence_recovery`）
      - `pre_step_enabled: bool`（既定 True；previous monitor state が `HARD` のとき、同じ cadence bookkeeping で hydro advance 前の event を許可）
      - `failures_hard_force: int`（既定 4；`1 <= failures_hard_force <= failures_big_repair`；この回数以上の連続 attempted transaction failure で hard target blend を強制）
      - `failures_big_repair: int`（既定 8；`failures_hard_force <= failures_big_repair <= escalation_max_failures`；この回数以上では hard target blend を強制し、両 displacement cap と virtual Winslow sweep count を 2 倍にする。sweep count は最大 32）
      - `escalation_max_failures: int`（既定 12；`failures_big_repair <= escalation_max_failures`；この回数の連続 attempted transaction failure で controller を disengage）
    ...
    - `mesh_epoch_enabled: bool`（default False; accumulate accepted pre-hydro mesh repairs into the driver retry snapshot）
    - `mesh_epoch_max_per_step: int`（default 16; valid range `>= 1`; maximum committed mesh-maintenance epochs per physical step）
    ...
    - `euler_window.t_on_s: float` / `euler_window.t_off_s: float`（default `0.0` / `-1.0`; the window engages only while `t_on_s <= t` and (`t_off_s < 0` or `t < t_off_s`). When the mode is active but no window is time-active the ALE step is a pure-Lagrangian no-op — control never falls through to another rezone solver（2026-07-30））
    - `euler_window.role: str`（default `""`; allowed `{"", "axis_survival_core"}`. `"axis_survival_core"` designates the persistent origin-centered spherical Eulerian core（axis-wall contract 2026-07-31, verdict ledger A58）: requires `shape="spherical_disk"`, `cr=cz=0`, `rad_in=0`, `t_on_s=0`, persistent `t_off_s<0`, `conservative_remap_enabled=True`, and an empty `euler_windows` list. `rad_out` becomes the REQUESTED plateau radius（snapped OUTWARD to a complete structural unit — center-fan rings, tier rows, and native-pentagon transition rows are never split）; `transition_width` becomes the minimum feather width. Plateau nodes are held bitwise at their captured initial coordinates every step（static-capture χ from initial radii, C1 smoothstep feather, north-compute/south-copy via the construction-exported mirror node map）; the mandatory transaction runs ahead of all other ALE operators with all-or-nothing semantics — rejection triggers snapshot restore + dt retry（fatal diagnostics on exhaustion）, never a silent Lagrangian fallback. This role — and only this role — lifts the dendrite×euler_window exclusivity（the qualified nonuniform-ladder path; plain rectangle/annulus windows on dendrite remain rejected）. Measured status（2026-07-31, verdict ledger A59–A62）: on the native-pentagon production topology the disk（requested 11.96µm → snapped s_p=12.15µm, 3058 plateau cells）STRUCTURALLY REMOVES every inner axis wall（S_theta/tier/fan — zero inner failures across all dt paths and radii）; the run is deterministic with the exact gate（step 5116, t=3.447×10⁻¹⁰, cell 1537）×2. The surviving wall is the SHELL-block polar-axis column（rows 4–8, cols 0–1）at the focus epoch — radius-independent（s_p=16.2µm gives the bit-identical death; 22.0µm is slightly worse via focal pile-up on the larger frozen boundary）. Historic 3.787/3.816 "bounce-crossing" numbers are PLOT_EVERY_S dt-clip convention samples（terminal death times carry ±10% dt-path chaos between conventions/configs; only within-convention deterministic gates are comparable）. Shell-column survival design: consultation #3's matched shell-cap dendrite was implemented（`Mesh.shell_polar_cap_dendrite` + `shell_cap_rows_2x`, opt-in default-off byte-inert）and REFUTED by measurement（A64）: the 2→1 transition row's axis pentagon dies at strong-shock transit wherever placed（death cell tracks rows_2x×176+1; earlier when farther out）, and the degenerate full-C2 chain violates the construction q≥0.20 floor at the outer shell cap（worst q=0.154）— the viable-placement window is empty. The unifying insight: transition-row axis pentagons are transit-fragile as a class（the native-bare 3.107 death was the S_theta one; the spherical core saves inner rows by freezing them）. The shell polar-column wall remains open; the machinery stays as an experiment surface（qualification tests: tests/hydro/test_axis_core_mask.cu, 5/5 green））
    - `euler_window.shape: "spherical_disk"`（additional allowed value; only with `role="axis_survival_core"`）
    ...
    - **削除済み（2026-08-03）**：旧 axis-survival-core time-gated release は廃止され、`role="axis_survival_core"` では `t_off_s=-1` が必須。
    - `axis_core_transition_passage_enabled: bool`（既定 `False`；consult-12 rank-2 の位相認識拘束 ALE passage v1（台帳 A104→E23 — 反跳衝撃波が pentagon 遷移帯を横切る際、anchored は隣接反転・released は subzonal 自壊する実測に対する「第三状態」）。有効時、plateau 内の各 TRANSITION_BELT を独立 patch (belt+両側 1 cell row) として捕捉し、CONVERGENCE→ARMED (中心 outflow 反転)→PASSAGE (patch 毎の §5.4 shock sensor が内縁 6 行以内) の状態機械で、PASSAGE 中は patch node に 6 回固定 Jacobi 平滑 (kernel-center 志向・二重バッファ決定論) の品質 target を quintic feather (境界 d=0 固定/d=1 0.5/d≥2 1) で合成し、α ladder {1..2⁻⁸} を patch 別 masked admissibility+swept gate (max|ΔV_f|/min V ≤ 0.25) で backtrack、既存 prescribed-target transaction 経路 (全体 admissibility+保存 remap+rollback) で all-or-nothing 適用する。α<1/16 連続 2 回 (衝撃波 4 行以内) で ESCALATE (rank-1 topology morph 要) をログ。`axis_core_transaction_mode` は static/always_moving/clearance_replay と併用可。既定 OFF は捕捉・kernel とも追加ゼロ）
    - `axis_core_ring_release_enabled: bool`（既定 `False`；Demand-driven ring release: on a proven geometry-infeasible step (typed G1 failure or window-transaction rejection) the axis-core plateau boundary steps one structural unit inward and the step retries; tuning-free (mesh unit ladder + frozen admissibility floors); persisted across restart via AXIS_CORE_RELEASED_UNITS.）
    - `band_ale.enabled: bool`（default False = bit-exact inert; BL-ALE stage 1（W5 2026-07-30）. Triggered, co-moving, band-local ALE: the run stays pure-Lagrangian except thin structural bands — per transition belt, the belt cells plus one adjacent tier row on each side; per axis-adjacent column, the first/last θ column pairs of structured blocks — which engage when the band's min planar `A/A0` (vs first-seen area) crosses `aspect_trigger` and are then χ-blended toward co-moving targets: each band ring moves to its own instantaneous angular-mean radius (θ preserved); axis-band nodes get a column-wise z Laplacian with endpoints pinned. The blended target passes the candidate-mesh admissibility oracle, reanchors the reference, and runs the conservative remap — the same accepted-rezone path as `euler_window`; inadmissible candidates skip the band that step. Requires `conservative_remap_enabled=True` and `Main.dimension="2D_RZ"` with a multiblock topology (a single-block structured mesh is additionally accepted for `bands="estimator"` only — the 2026-08-26 single-block generalization; in that mode the per-step structured conservative-remap phase is bypassed and band transactions are the only conservative-remap consumer, mirroring the multiblock arrangement); mutually exclusive with `rezone_solver="m1_tmop"` and with `euler_window`/`euler_windows`. Frozen-config emits the block only when enabled）
    - `band_ale.aspect_trigger: float`（default `0.25`; valid `(0, 1]`; engage threshold on the band minimum of planar `A/A0` for non-Estimator bands）
    - `band_ale.release_hysteresis: float`（default `1.5`; valid `> 1`; non-Estimator bands disengage when the band minimum recovers above `aspect_trigger*release_hysteresis`）
    - `band_ale.chi: float`（default `0.5`; valid `[0, 1]`; per-step blend strength toward the band target）
    - `band_ale.bands: str`（default `"belts_axis"`; allowed `{"belts", "axis", "belts_axis", "shell", "belts_axis_shell", "estimator"}`; which band families are armed. `"shell"` arms only the shock-following polar-shell band, `"belts_axis_shell"` arms all three legacy families, and standalone `"estimator"` arms only the refinement-estimator-driven polar-shell band and requires `Numerics.diagnostics.refinement_estimator.enabled=True` and (v5) `Numerics.diagnostics.refinement_autopilot.enabled=True` (the front hold consumes the tracker); the Estimator band engages/releases on within-band radial-spacing nonuniformity and respaces interior rings to uniform fractions of the current endpoint radii, but remains inactive while any expanded-window row mean reaches the shock-hold threshold. Measured guidance (2026-07-30): the axis z-Laplacian variant is dose-dependently destructive on GXII-scale implosions (relief intensity anti-correlates with survival; drive-layer/neighbor-column damage) — production candidate is `"belts"`）
    - `band_ale.estimator_band_hold_mach: float`（default `0.30`; finite and valid `>=0`; physical compression hold for Estimator-band respace installs: the band holds while any window cell's radial velocity jump exceeds this Mach fraction of its sound speed, using the same measure as the pole-theta and catchment holds; `0` disables; applies to the multiblock estimator band only; the single-block lattice band does not implement the Mach hold and ignores this key.）
    - `band_ale.estimator_band_cut: float`（default `0.5`; valid finite `(0,1)`; threshold applied to each polar-shell radial row's mean `state.refine_error`）
    - `band_ale.estimator_band_shock_hold: float`（default `0.90`; finite, greater than `estimator_band_cut`, and `<=1.0`; the Estimator band stays inactive for an evaluation when any row-mean `state.refine_error` in its inward/outward-expanded window is `>= estimator_band_shock_hold`）
    - `band_ale.estimator_band_front_hold_margin_rows: float`（default `16.0`; valid `>=0`; v5 front hold: the Estimator band also stays inactive while the refinement tracker reports a TRACKED/ARMED front whose median polar-shell row lies within this many rows of the expanded window `[first_ring, last_ring]`. Catches FORMING fronts whose row means sit in the `[estimator_band_cut, estimator_band_shock_hold)` gap (measured 0.83–0.895 on the hot-shell surrogate, ledger A455–A458); requires the tracker, hence the autopilot requirement on `bands="estimator"`）
    - `band_ale.estimator_band_axis: str`（default `"auto"`; allowed `{"auto", "i", "j"}`; resolves the LAYER axis of the Estimator band on single-block structured meshes: `"auto"` selects `"i"` (radial layers) when `Mesh.logical_mesh_2d` is a spherical-polar family and `"j"` (z layers) otherwise; explicit `"i"`/`"j"` override the rule. Band rows are then per-line means over the layer lines of that axis, the spacing metric `eta_h` uses adjacent line-mean layer-coordinate gaps, and the respace target moves nodes only along the layer axis (transverse coordinates untouched). Ignored on multiblock topologies (the polar-shell radial-ring convention is fixed). The refinement tracker resolves its single-block column tables with the same rule so front rows are comparable to band rows）
    - `band_ale.compose_with_rezone: bool`（default `False`; single-block only, requires `bands="estimator"`. When True, the single-block ALE step runs the STANDARD triggered flow first (winslow rezone + its remap, untouched semantics) and then evaluates the Estimator band and applies its transactions on the post-rezone mesh — feature maintenance composed with plume fold management (measured motivation: the graded-band-preserving triggered-rezone proof runs of 2026-08-27). With False the band branch bypasses the standard flow exactly as before）
    - `evacuated_cell.enabled: bool`（default `False` is bit-inert; when enabled, mass-anchored conversion turns evacuated cells into inactive vacuum. The OFF threshold is `m_off=max(off_mass_fraction*m_ref, rho_vacuum_policy_g_per_cc*V_ref)`, the ARM threshold is `max(arm_mass_fraction*m_ref, 100*m_off)`, and conversion requires consecutive-evaluation hysteresis plus the laser-insignificance, coupling, and recipient gates. All extensives transfer deterministically and conservatively to face-area-weighted active neighbors with a 64-eps audit and the `[evac-cell-txn]` ledger; converted cells are excluded from hydro forces/artificial viscosity, CFL, and conduction (exact-zero face coefficients), while FLD deliberately retains them. Domain-corner cells are excluded; enabling is fail-closed to single-block `2D_RZ` with `Numerics.ale.enabled=True`, `Numerics.diagnostics.conduction_energy_rate_export.enabled=True`, and no SNB nonlocal conduction）
    - `evacuated_cell.every_n_steps: int`（default `50`; valid `>=1`; policy evaluation cadence in committed hydro steps）
    - `evacuated_cell.arm_mass_fraction: float`（default `1.0e-6`; finite and valid with `0 < off_mass_fraction < arm_mass_fraction < 1`; reference-mass fraction used by the ARM threshold）
    - `evacuated_cell.off_mass_fraction: float`（default `1.0e-8`; finite and valid with `0 < off_mass_fraction < arm_mass_fraction < 1`; reference-mass fraction used by the OFF threshold）
    - `evacuated_cell.rho_vacuum_policy_g_per_cc: float`（default `1.0e-10`; finite and `>0`; policy density in g/cm³ used with the reference cell volume in the mass thresholds）
    - `evacuated_cell.off_hold_evaluations: int`（default `2`; valid `>=1`; consecutive OFF-eligible evaluations required before conversion）
    - `evacuated_cell.laser_ne_over_ncrit_max: float`（default `1.0e-3`; finite and `>0`; maximum estimated `n_e/n_crit` accepted by the laser-insignificance gate）
    - `evacuated_cell.laser_wavelength_nm: float`（default `351.0`; finite and `>0`; laser wavelength in nm used to estimate the critical density）
    - `evacuated_cell.coupling_fraction_max: float`（default `1.0e-8`; finite and `>0`; maximum laser-plus-conduction coupling energy relative to the neighbor-patch energy）
    - `evacuated_cell.max_cells_per_event: int`（default `4`; valid `>=1`; rejects an event if more cells would convert）
    - `evacuated_cell.rematerialize_enabled: bool`（default `True`; part of the policy's correctness: a converted vacuum slot has no pressure back-reaction, so the converging flow closes it geometrically; re-materialization conservatively withdraws mass/energy from the active face neighbors (area-weighted, per-donor clamped, 64-eps audited, `[evac-cell-remat]` ledger) and revives the cell as a thin live cell before geometric failure）
    - `evacuated_cell.rematerialize_after_evaluations: int`（default `10`; valid `>=1`; timed trigger — an inactive slot re-materializes unconditionally after this many policy evaluations, in addition to the volume-threshold and predicted-closure (one-cadence linear volume extrapolation) triggers）
    - `evacuated_cell.rematerialize_volume_fraction: float`（default `0.05`; finite and valid in `(0,1)`; volume trigger — re-materialize when the slot volume falls below this fraction of the reference cell volume）
    - `evacuated_cell.rematerialize_neighbor_change_max: float`（default `5.0e-2`; finite and valid in `(0,1)`; per-donor relative withdrawal clamp — the refill target is the donors' mass-weighted mean density, and each donor's withdrawal is clamped to this fraction of its mass (partial fill; the achieved `fill=` fraction is ledgered, and fills below 1e-3 are skipped as insufficient)）
    - `evacuated_cell.rematerialize_dwell_evaluations: int`（default `5`; valid `>=1`; a re-materialized cell is exempt from re-conversion for this many evaluations (hysteresis against convert/re-materialize chatter)）
    - `evacuated_cell.closure_contact.enabled: bool`（default `True`, inert while `evacuated_cell.enabled=False`; the void-closure contact state machine: a converted slot whose closing faces approach within an arming gap is tracked per node pair every step, joined by a momentum-conserving impact projection when a pair reaches the gap floor (relative kinetic energy deposited to ion internal energy of the active corner-mass-weighted neighbors, 64-eps audited, `[evac-cell-contact]` ledger), then held by a normal-acceleration constraint in both hydro stages; frictionless (tangential free) and unilateral (releases on sustained tension)）
    - `evacuated_cell.closure_contact.gap_floor_fraction: float`（default `0.02`; valid in `(0,1)`; the engaged gap floor g0 as a fraction of the reference closing-direction thickness）
    - `evacuated_cell.closure_contact.gap_arm_fraction: float`（default `0.04`; valid in `(0,1)` and required `> gap_floor_fraction`; the arming gap — per-pair gaps and their 2-evaluation linear prediction arm the slot when crossing this）
    - `evacuated_cell.closure_contact.live_mass_gate: float`（default `0.05`; valid in `(0,1)`; a LIVE closure-lineage cell converts (and arms) when its mass falls below this fraction of the reference mass while its volume is below the classifier — the wedge/live-thin closure route that mass-only triggers miss）
    - `evacuated_cell.closure_contact.live_volume_gate: float`（default `0.02`; valid in `(0,1)`; the volume classifier for the live closure gate, as a fraction of the reference volume）
    - `evacuated_cell.closure_contact.refill_min_mass_fraction: float`（default `0.05`; valid in `(0,1)`; minimum viable re-materialization: the clamped refill must reach this fraction of the reference mass or the slot stays inactive）
    - `evacuated_cell.closure_contact.refill_min_density_ratio: float`（default `0.2`; valid in `(0,1)`; minimum viable re-materialization: the refill density must reach this ratio of the donors' median density — together with the mass fraction this stops the refill/re-ablation mass pump）
    - `evacuated_cell.closure_contact.release_force_c: float`（default `1.0e-3`; finite `>0`; release threshold scale — an engaged pair releases when its constraint multiplier is tensile beyond `release_force_c * A_face * p_scale` for the persistence count）
    - `evacuated_cell.closure_contact.release_persistence_stages: int`（default `2`; valid `>=1`; consecutive tensile evaluations required before a pair releases）
    - `evacuated_cell.closure_contact.reengage_gap_margin: float`（default `0.01`; valid in `(0,1)`; a released slot re-arms when the minimum pair gap closes below `g0*(1+margin)`）
    - `evacuated_cell.closure_contact.mortar_position_drift_beta: float`（default `0.02`; finite in `[0,0.1]`; low-gain held-mortar position-drift correction, with `0` preserving the rate-only projection path bitwise）
    - `evacuated_cell.closure_contact.surface_engage_enabled: bool`（surface-measured engagement (migration phase 4); default false.）
    - `evacuated_cell.closure_contact.lcp_apply_enabled: bool`（coupled LCP impulse application and device row hold (migration phase 6); default false; requires `surface_engage_enabled=true`.）
    - `evacuated_cell.closure_contact.axis_edge_collapse.enabled`（bool, default `False`; enable the axis-edge-collapse monitor for a contact-adjacent axis cell whose on-axis edge is closing irreversibly (stage A: measurement and predicate only).）
    - `evacuated_cell.closure_contact.axis_edge_collapse.ulp_count`（double, default `4096`; edge-length floor in units of the larger ULP of the two axis node z coordinates.）
    - `evacuated_cell.closure_contact.axis_edge_collapse.h_ref_fraction`（double, default `1e-6`; edge-length floor as a fraction of the frozen local axis spacing reference.）
    - `evacuated_cell.closure_contact.axis_edge_collapse.release_hysteresis`（double, default `4`; disarm requires the gap to exceed this multiple of the retirement floor while opening.）
    - `evacuated_cell.closure_contact.axis_edge_collapse.persistence_window`（int, default `8`, allowed `2..8`; number of recent per-step samples in the closing-persistence window.）
    - `evacuated_cell.closure_contact.axis_edge_collapse.persistence_min_closing`（int, default `7`, allowed `1..persistence_window`; closing samples required within the window.）
    - `evacuated_cell.closure_contact.axis_edge_collapse.repair_recurrence_steps`（int, default `16`; steps within which consuming most of a repair's gained clearance classifies the repair as futile.）
    - `evacuated_cell.closure_contact.axis_edge_collapse.repair_futility_fraction`（double, default `0.25`; remaining fraction of gained clearance at or below which the repair is futile.）
    - `evacuated_cell.closure_contact.flank_tangential_strip.enabled`（bool, default `False`; enable the flank tangential-strip monitor — stage S1: measurement and classification only.）
    - `evacuated_cell.closure_contact.flank_tangential_strip.untangler_enabled`（bool, default `True`; after all equidistribution line-search trials fail, enable the deterministic host-side local feasible-set/Jacobian-barrier untangler fallback.）
    - `evacuated_cell.closure_contact.flank_tangential_strip.band_layers`（int, default `2`, allowed `1..3`; active strip node rows above the axis row.）
    - `evacuated_cell.closure_contact.flank_tangential_strip.band_halfwidth_j`（int, default `6`, allowed `1..32`; half-width of the monitored j window around the slot cell.）
    - `evacuated_cell.closure_contact.flank_tangential_strip.arm_quality_ratio`（double, default `0.25`; arm when a band corner's angle quality falls below this fraction of its frozen epoch reference.）
    - `evacuated_cell.closure_contact.flank_tangential_strip.release_quality_ratio`（double, default `0.60`; release requires recovery above this fraction.）
    - `evacuated_cell.closure_contact.flank_tangential_strip.min_progress_factor`（double, default `10.0`, valid `>1.0`; accept a strip trial when its band quality ratio improves by at least this factor.）
    - `evacuated_cell.closure_contact.flank_tangential_strip.lead_steps`（int, default `32`; arm when the log-slope prediction reaches the hard floor within this many steps.）
    - `evacuated_cell.closure_contact.flank_tangential_strip.release_persistence_steps`（int, default `32`; consecutive accepted steps required for release.）
    - `evacuated_cell.closure_contact.flank_tangential_strip.release_shear_number`（double, default `0.02`; release also requires `dt*max|du_t/dn|` below this.）
    - `evacuated_cell.closure_contact.flank_tangential_strip.slip_handoff_ratio`（double, default `0.5`, valid `>0`; informational `|delta_s|/h_s` threshold for the later slip-patch caller.）
    - `evacuated_cell.closure_contact.flank_tangential_strip.slip_patch_enabled`（bool, default `False`; master gate for the host-side flank slip-patch transaction.）
    - `evacuated_cell.closure_contact.seam_interface_owner_enabled`（bool, default `False`; when the dormant scaffold's volume allowance is exhausted, convert the slot cell into a non-volumetric seam interface owner instead of aborting: the cell leaves the mesh-geometry and volume-floor policing sets while the contact machinery keeps owning the seam.）
    - `band_ale.estimator_band_in_rows: int` / `band_ale.estimator_band_out_rows: int`（defaults `4` / `4`; valid `>=1`; rows added on the inner/outer sides of the outermost connected interval whose row-mean `refine_error` reaches `estimator_band_cut`; the Shell boundary guard and fewer-than-three-rings inactive rule still apply）
    - `band_ale.estimator_band_eta_on: float`（default `1.5`; finite and greater than `estimator_band_eta_off`; Estimator engage threshold for `eta_h=max_j(h_j)/median_j(h_j)`, with `h_j=abs(r_j-r_{j+1})` from outer to inner mean-radius rings）
    - `band_ale.estimator_band_eta_off: float`（default `1.15`; finite and valid with `1 < estimator_band_eta_off < estimator_band_eta_on`; Estimator release threshold for `eta_h`）
    - `band_ale.estimator_band_per_column: bool`（default `False`; master switch for the Phase-A per-column, per-spoke Estimator operator. `True` requires `bands="estimator"`. With zonal pressure-drive perturbation enabled it selects the per-column production path; otherwise the unchanged angle-mean path remains authoritative and the per-column path runs in diagnostic shadow）
    - `band_ale.estimator_band_pc_filter_halfwidth: int`（default `3`; valid `[1,3]`; half-width of the mask-normalized binomial physical-edge-radius filter, with even reflection at both polar axes）
    - `band_ale.estimator_band_pc_slope_limit: float` / `band_ale.estimator_band_pc_slope_reject: float`（defaults `0.35` / `0.50`; finite, `slope_limit>0`, and `slope_reject>=slope_limit`; normal clamp and hard per-column degradation thresholds for regularized edge slope）
    - `band_ale.estimator_band_pc_curvature_limit: float`（default `0.50`; finite `>0`; per-column degradation limit for the physical-radius edge second difference in local-spacing units）
    - `band_ale.estimator_band_pc_chi_max: float`（default `0.25`; finite `(0,1]`; maximum per-column transaction blend）
    - `band_ale.estimator_band_pc_chi_step: float`（default `0.10`; finite `>0`; maximum chi increase per committed estimator event and first-engagement chi）
    - `band_ale.estimator_band_pc_sigma_floor: float`（default `0.25`; finite `(0,1)`; an applied transaction below this oracle sigma is retrospectively classified as starvation at the next evaluation, counted as rejected, held for the configured cooldown, and resets the adaptive chi state）
    - `band_ale.estimator_band_pc_coverage_full: float` / `band_ale.estimator_band_pc_coverage_min: float`（defaults `0.80` / `0.50`; finite and `0 < coverage_min <= coverage_full <= 1`; Phase A takes no action below `coverage_min`, never falls back to angle-mean under asymmetric input, and applies global-patch semantics to the entire well-formed set above the minimum; `coverage_full` records the later full-coverage threshold）
    - `band_ale.estimator_band_pc_cooldown_events: int`（default `2`; valid `>=1`; minimum number of estimator evaluations between committed per-column transactions）
    - `band_ale.estimator_band_pc_phase_b: bool`（default `False`; enables the Phase-B four-sample traveling/persistent discriminator, geometric front tube, conservative incident-cell containment, and patchwise per-column action. `False` retains the Phase-A global-hold decision path）
    - `band_ale.estimator_band_pc_tube_dilate_rows: int`（default `5`; valid `>=0`; full radial cell rows added on both sides of each Phase-B front-tube seed）
    - `band_ale.estimator_band_pc_tube_dilate_cols_extra: int`（default `2`; valid `>=0`; additional angular cell columns added when the Phase-B front tube touches either polar axis）
    - `band_ale.estimator_band_pc_ambiguous_hold_fraction: float`（default `0.20`; finite in `[0,1]`; Phase B escalates to a global hold when the ambiguous-column fraction is strictly greater than this value. The four-sample ridge history is runtime-only and resets at `run_start`, including restart after a mesh-epoch swap）
    - `band_ale.axis_target: str`（default `"z_laplacian"` = bit-exact original column z-smoothing; allowed `{"z_laplacian", "respace"}`. `respace` applies the initial-fraction radial redistribution per column. Measured (2026-07-30): ALL four axis-band variants — z-Laplacian and respace, whole-column and argmin-segment-local — degrade survival relative to `bands="belts"` on GXII-scale implosions (whole-column fights the in-transit shock; even thin segments' every-step remaps perturb the belt protection). Axis bands are an experiment surface, not production）
    - `band_ale.axis_segment_halfwidth: int`（default `4`; valid `[1, 64]`; axis-band targets of both modes apply only inside rows `[argmin−K, argmin+K]` around the band's minimum-ratio cell (deterministic smallest-index tie-break), window endpoints acting as per-step pins（W15 2026-07-30））
    - `band_ale.axis_shell_block_enabled: bool`（default `False`; when enabled, axis bands also cover the polar-shell block itself, targeting drive-layer axis-column crush. `False` preserves the legacy shell-block skip and counter）
    - `band_ale.sigma_linesearch_enabled: bool`（default `True`; when enabled, band transactions apply the largest admissible prescribed-target fraction by halving sigma from 1 to 1/64; `False` preserves all-or-nothing candidate acceptance）
    - `band_ale.transaction_energy_closure_enabled: bool`（default `False`; Deposit each applied band transaction's remap KE loss as heat (KE-fixup, chi=1). Default false is bit-identical.）
    - `band_ale.center_target: str`（default `"line"` = the ring-line interpolated Chebyshev-center target, byte-exact original; allowed `{"line", "volume_fraction"}`. `volume_fraction` solves each pentagon center for initial triangle-volume-fraction preservation（log objective + admissibility barrier + Lagrangian tether, fixed 8 damped Newton iterations; review §6.4 center-only design）. Measured (2026-07-31): net-negative during shock transit（champion 3.216 vs 3.229; enforcing initial partitions fights the transient physical distribution）— experiment surface, not production）
    - `band_ale.axis_repair_{enabled, eta_on, eta_off, cap_rel}`（defaults `False`/`0.85`/`0.95`/`0.05`; disabled bit-exact. η-triggered impulsive axis repair: tier axis-adjacent cells engage on RELATIVE ring-scale lag（η = s_cell/median ring scale, per-cell hysteresis）and receive an area-gradient inflation nudge capped per event, via the band transaction. Measured (2026-07-31): v1 nudge form is net-negative on the production config（3.194 vs 3.229; 2-step engage/release limit cycles pump remaps）— experiment surface; the review's full 2D target-Jacobian patch objective is the remaining untested fidelity）
    - `band_ale.closure_catchment_enabled: bool`（default `False` = bit-exact inert; 2026-08-28, front-conforming lane. Closure-adjacent quasi-Eulerian catchment band for tier_cart meshes with an active central pseudo-core pool: a structured two-block walker over the unpooled CENTRAL_CORE and BRIDGE rows builds a per-step prescribed radial-respace target（birth-proportion redistribution with uniform fallback, θ normalization to clean spokes with an axis-exact collar blend）that absorbs the Lagrangian closure gap at the pool interface. Arms physically（piston-adjacent row max node speed ≥ 1e5 cm/s, latched）and requests proactive ring absorption when the innermost consecutive intervals fall below the resolution-relative operating floor（0.5× min birth spacing）. Runs through the standard prescribed-target transaction（admissibility oracle + σ line-search + machine-zero mass/energy ledger）. Requires the tier_cart multiblock topology with a unique CENTRAL_CORE and BRIDGE block; holds（no-op with a logged reason）when the pooled row count leaves no walkable region）
    - `band_ale.closure_catchment_forced_active: bool`（default `False`; increment-1 test switch that bypasses the physical arming latch）
    - `band_ale.closure_catchment_s_catch_cm: float` / `band_ale.closure_catchment_s_protect_cm: float`（defaults `6.0e-4` / `1.0e-3` cm; finite `>0` with `s_catch < s_protect`; the catchment region cap measured from the pool interface, and the protected outer anchor cap — both applied RELATIVELY to the last covered node ring so collapsed states remain walkable）
    - `band_ale.closure_catchment_spacing_floor_cm: float`（default `3.0e-6` cm; finite `>0`; hard spacing floor of the redistribution; infeasible floors fall back to uniform spacing）
    - `band_ale.closure_catchment_ratio_max: float`（default `1.05`; finite `>1`; adjacent-spacing ratio cap of the redistribution; non-convergent ratio passes fall back to uniform）
    - `band_ale.closure_catchment_nu_max: float`（default `0.25`; finite `(0,1]`; per-transaction swept-fraction pre-cap; a prescaled target that wants more motion sets the reapply flag for another capped bite in the same step）
    - `band_ale.closure_catchment_max_bites: int`（default `1`; valid `>=1`; maximum ν-capped sub-transaction bites per step（deck: TENRYU_SMOKE_CCATCH_BITES））
    - `band_ale.closure_catchment_shock_hold: float`（default `0.30`; compressive radial velocity jump threshold in local sound-speed units: the hold engages while any cell of the walk rows shows `u_r,inner - u_r,outer > threshold * c_s` — a shock transiting the guarded region; `<= 0` disables）
    - `band_ale.closure_catchment_eta_h_arm: float` / `band_ale.closure_catchment_eta_h_full: float`（defaults `0.80` / `0.65`; valid `0 < eta_h_full < eta_h_arm <= 1`; smoothstep5 activation ramp on the relative spacing defect `eta_h`, exactly `0` at or above arm and `1` at or below full）
    - `band_ale.closure_catchment_eta_m_arm: float` / `band_ale.closure_catchment_eta_m_full: float`（defaults `0.75` / `0.60`; valid `0 < eta_m_full < eta_m_arm <= 1`; the same ramp on the relative interface-mass defect `eta_m`）
    - `band_ale.closure_catchment_reset_eta: float`（default `0.90`; valid `(0, 1]`; EMA/accumulator reset level）
    - `band_ale.closure_catchment_support_core_rows: int` / `band_ale.closure_catchment_support_taper_rows: int`（defaults `4` / `2`; valid `core_rows >= 1` and `taper_rows >= 0`; interface-adjacent support window of the common-mode correction and its smooth taper）
    - `band_ale.closure_catchment_accum_frac: float`（default `0.02`; valid `(0, 1)`; displacement accumulator: a transaction fires only when the accumulated common-mode correction exceeds this fraction of the local spacing）
    - `band_ale.closure_catchment_rearm_drop: float`（default `0.10`; valid `(0, 1)`; rearm hysteresis after a fired transaction）
    - `band_ale.pole_theta_enabled: bool`（default `False` = bit-exact inert; 2026-08-28, consult-adjudicated（docs/design/front_conforming_ale_20260826.md §7.11）. Transactional tangential（θ）re-equidistribution of near-pole node rows on tier_cart meshes, the general counter to the post-stagnation polar azimuthal cell collapse（axis-jet shear pancaking that only the corner-geometry/subzonal dt channel sees）. A per-step host controller scans POLAR_SHELL + POLAR_TIER + active BRIDGE cells（pooled/derefined/hydro-inactive masked; quads gathered from the CSR with radial/angular edge pairs classified by node-id evidence）into per-column H = h_θ/(s̄·Δθ*)（target-normalized subzonal angular altitude）and the target-relative Jacobian condition number κ; each pole is detected, sectored, latched, and line-searched INDEPENDENTLY（equator symmetry is an audit, never an imposed constraint）. On fire, marked-cell node rows（decomposed canonically into (block, owned node row) + halo）receive a relaxed equal-θ map（deadband, per-node move clamp, deterministic monotonicity projection with a 0.02·Δθ* minimum gap; axis spokes are fixed anchors; the pool interface ring is in scope; radius is preserved — the spherical-legal M1 form; the curve-preserving reparameterization with Legendre-protected modes is the required M2 upgrade before aspherical production）. The map composes into whatever prescribed-target transaction runs that step（one target, one remap; consume-once on acceptance）; a standalone pole_theta band engages only when no other band fired. Requires a multiblock topology with unique POLAR_SHELL and BRIDGE blocks）
    - `band_ale.pole_theta_routine_enabled: bool`（default `False`; v2 routine polar lane master switch; measured 2026-08-30: in every exercised regime the routine lane is either unnecessary（pre-stagnation stays in the LSB noise band without it）or harmful（during stagnation it re-equalizes pitches against the physically draining polar channels and pumps theta asymmetry through its pass band）, while the hard lane, catchment, and pool carry all demonstrated mandates; it is kept for future regimes that demonstrate need）
    - `band_ale.pole_theta_phys_lp: int` / `band_ale.pole_theta_phys_lc: int`（defaults `0` / `4`; valid `0 <= lp < lc <= 32`; retained/cutoff Legendre orders of the low-pass physical reference band; the routine lane acts only on the high-pass pitch defect `d = ln(dtheta/dtheta_phys)`, so deformation inside the retained band is invisible to it）
    - `band_ale.pole_theta_noise_floor: float`（default `1e-5`; finite and `>0`; noise normalization of the solid-angle L8 row defect; rows activate via smoothstep5 between `2x` and `4x` this floor, with an exact no-op below `2x`）
    - `band_ale.pole_theta_noise_ceiling: float`（default `1e-3`; structural ceiling of the routine lane: activation is band-pass, exactly zero at/above `2x` the ceiling, so defects far above the noise scale are left to physics and the hard lane）
    - `band_ale.pole_theta_h_arm: float` / `band_ale.pole_theta_h_fire: float` / `band_ale.pole_theta_h_hard: float` / `band_ale.pole_theta_h_release: float`（defaults `0.70` / `0.55` / `0.35` / `0.80`; finite with `0 < h_hard < h_fire < h_arm < h_release <= 1`; H thresholds: arm latch, normal fire（needs 2 consecutive evaluations + cooldown）, hard fire（immediate, overrides cooldown）, release）
    - `band_ale.pole_theta_kappa_arm: float` / `band_ale.pole_theta_kappa_fire: float` / `band_ale.pole_theta_kappa_hard: float`（defaults `2.5` / `3.5` / `6.0`; finite with `1 < arm < fire < hard`; condition-number companions to the H thresholds）
    - `band_ale.pole_theta_alpha: float` / `band_ale.pole_theta_alpha_hard: float`（defaults `0.30` / `0.60`; finite `0 < alpha <= alpha_hard <= 1`; relaxation of the equal-θ target for normal / hard fires）
    - `band_ale.pole_theta_deadband_frac: float`（default `0.05`; finite `>=0` and `< move_limit_frac`; nodes within this fraction of Δθ* of their equalized angle do not move — the deadband uses the qualified target pitch, never the collapsing current scale）
    - `band_ale.pole_theta_move_limit_frac: float` / `band_ale.pole_theta_move_limit_hard_frac: float`（defaults `0.35` / `0.75`; finite `deadband < limit <= limit_hard <= 1`; per-node |δθ| clamps in units of Δθ*（the monotonicity projection may exceed them — ordering outranks the clamp））
    - `band_ale.pole_theta_cooldown_s: float`（default `1.0e-12` s; `>=0`; minimum time between normal fires per pole; hard fires ignore it）
    - `band_ale.pole_theta_cooldown_base_s: float`（default `1.0e-12` s; `>=0`; hard-lane base physical-time cooldown between committed coupled fires; futility debt doubles it per unit of debt, capability reaches exact zero at debt `>= 3`, and novelty of the defect signature erases debt; `0` disables）
    - `band_ale.pole_theta_predict_window_s: float` / `band_ale.pole_theta_predict_horizon_s: float`（defaults `2.0e-12` / `5.0e-12` s; `>0`; the log-collapse rate λ = −d ln H/dt is least-squares fitted over the window and pre-fires when H < 0.75 and the extrapolated time to h_fire is below the horizon）
    - **pole-theta v2 semantics**: the sector/map machinery fires only on the hard thresholds (`pole_theta_h_hard`, `pole_theta_kappa_hard`) with the hard parameters; `pole_theta_h_fire`, `pole_theta_kappa_fire`, `pole_theta_alpha`, `pole_theta_move_limit_frac`, `pole_theta_cooldown_s`, `pole_theta_predict_window_s`, `pole_theta_predict_horizon_s`, and `pole_theta_deadband_frac` are retained for config compatibility but unused by the v2 lanes.
    - `band_ale.pole_theta_halo_columns: int` / `band_ale.pole_theta_halo_rows: int`（defaults `2` / `2`; `>=0`; taper halo appended on the equator side of the grown sector, and owned-node-row halo around the marked rows）
    - `band_ale.pole_theta_post_h_floor: float`（default `0.65`; finite `(0,1)`; recorded M1 acceptance target for post-fire H — enforced controller-side（non-improving fires escalate to hard + widened halo）, not yet inside the σ line-search）
    - `band_ale.pole_theta_shock_hold: float`（default `0.30`; compressive radial velocity jump threshold in local sound-speed units: the hold engages while any cell of the pole's scan half shows `u_r,inner - u_r,outer > threshold * c_s` — a shock transiting the guarded region; `<= 0` disables）
    - `band_ale.belt_target: str`（default `"ring_mean"` = bit-exact original co-moving target (each ring to its own instantaneous angular-mean radius); allowed `{"ring_mean", "respace"}`. `respace` keeps each belt-band ring at its INITIAL-fraction position between the two pinned lines' current mean radii（radial scaling per node, θ preserved exactly）— compression still happens but is shared across all band rows instead of concentrating in the weakest one. Measured (2026-07-30, five-triangle 288×192): belt-only respace with `aspect_trigger=0.45`, `release_hysteresis=3.0`, `chi=0.9` keeps all four transition belts alive through shock transit (engaged min A/A0 held at 1e-3–1e-1) and moves first mesh death off the belts entirely（champion 3.110e-10 → 3.229e-10 s, death at an unprotected axis-adjacent tier cell）. Note the hysteresis interplay: respace is effective enough per step to cross a narrow release band and chatter — use release_hysteresis ≥ 3 with it）
    - `band_ale.shell_window_in_rows: int` / `band_ale.shell_window_out_rows: int`（defaults `8` / `4`; valid `>=0`; inward/outward half-widths, in units of the main polar block's initial mean radial spacing, around the current maximum-centered-gradient density front. Fewer than three selected node rings leaves the Shell band inactive for that update）
    - `band_ale.shell_boundary_guard_rows: int`（default `2`; valid `>=0`; excludes the outermost block rows adjacent to the domain outer boundary from the Shell radius window, regardless of `r_front`）
    - `band_ale.shell_min_spacing_frac: float`（default `0.5`; the Shell band engages when either its minimum planar `A/A0` crosses `aspect_trigger` or the minimum difference between consecutive selected mean ring radii falls below this fraction of the block's initial mean radial spacing. It disengages only when both thresholds recover above their respective values multiplied by `release_hysteresis`）
    - `band_ale.shell_front_metric: str`（default `"grad_rho"`; allowed `{"grad_rho", "min_spacing"}`; selects the Shell window centre metric. `"grad_rho"` follows the maximum radial mean-density gradient, while `"min_spacing"` follows the midpoint of the globally closest consecutive current mean-radius ring pair）
    - `band_ale.shell_target: str`（default and currently only allowed value `"respace"`; places each selected polar-shell ring at its initial-fraction position between the selected window's pinned endpoint rings, with radial displacement and the common `chi` blend）
    ...
    - `axis_variational_projection_enabled: bool` (default False; axis-band escalation opt-in. When AxisSpinePlusLocal fails, the retry ladder may dispatch `AxisVariationalProjection`, a deterministic projection-style half-space feasibility operator, before `InteriorMultiNodeProjection` and `FullWinslow`. NUMERICS_03 §3.3.5a; ARCHITECTURE_06-10 §6.1)
    ...
    - `axis_band_managed_remap_enabled: bool` (default False; managed axis-band controller opt-in. False preserves the legacy ALE/retry path.)
    ...
    - `conservative_remap_order: str`（既定 `"first_order_donor"`；許可値 `{"first_order_donor", "second_order_van_leer"}`。`"first_order_donor"` は PR B donor-cell flux を bit-exact に保持し、`"second_order_van_leer"` は PR C の Van Leer slope + Barth-Jespersen bounded face reconstruction を使用。tri_fan の fan-touching faces は Stage 3 では first-order donor に fallback し、pure quad-to-quad faces のみ second-order reconstruction を使う）
	    ...
	    - `central_pseudo_core_activation_time_s: double [s]` (default 0.0; must be finite and `>= 0`. The pooled central cap builds only once `t >= central_pseudo_core_activation_time_s`; 0 means build at leg start.)
	    ...
	    - 旧 1D 専用 ALE キー群は削除済み。新しい 1D solution-adaptive ALE は `Numerics.ale1d` で制御し、既定では無効。
  - `floors: dict`
...
- `eos: dict`
  ...
  - `ideal_gas: dict`（任意。受け付けるキーは `gamma` だけ。省略すると `gamma=5/3`。`model="power_law_te"` と併用すると ConfigError）
    ...
    - 2T理想気体EOS（NUMERICS §1.1.5 (a)）：
      ...
      - \(\bar{Z}\) は `zbar` モデルに従う
      - `cv_e_override` 指定時は \(C_{v,e}\) を上記公式の代わりに定数値を使用する（下記 `eos.cv_e_override`）
  - `cv_e_override: Optional[float]`（既定 None；`eos` の直下に書く — `ideal_gas` の中には書けない。指定時は電子比熱を定数 \(C_{v,e}\) [erg/(cm³·eV)]（体積あたり）として固定する。**検証テスト用**：Marshak wave や Su-Olson 等の解析解が定数 \(C_v\) を要求する場合に使用。指定時は `gamma` による \(C_{v,e}\) 計算を上書きする。`eos_T_ref_eV` を指定しなければ温度依存性なし。圧力は \(P_e = (\gamma-1)\,\rho\,e_e\) をそのまま使用。NUMERICS §6.1 の \(\beta_c = 4\,a_{eV}\,T_{e,c}^3 / C_{v,e}\) に直接適用される）
  - `eos_T_ref_eV: Optional[float]`（既定 None；`cv_e_override` と併せて正の値を指定すると、1T の物質 EOS 変換を \(e_e=\alpha_0T_e^4/\rho\)、\(\alpha_0=\)`cv_e_override`\(/(4T_{ref}^3)\)、\(T_{ref}=\)`eos_T_ref_eV` で行う（比熱が \(T^3\) に比例する解析解の検証用）。NUMERICS §6.1 参照）
...
- `multigroup_diffusion: dict`（`mode="multigroup_diffusion"` で使用。NUMERICS §6.7 参照）
  ...
  - `hydro_coupling: Literal["none","conservative_advection","gamma_r_43"]` (default `"gamma_r_43"`). `none` preserves historical frozen radiation density during mesh motion; its adjusted energy-budget epsilon is not physical closure. `conservative_advection` carries each group cell energy `E_g V` through accepted hydro half-steps without radiation-pressure force/work; supported only for 1D Lagrangian FLD on the host split loop, without ALE1D or persistent loop. An environment gamma override is rejected for this option. `gamma_r_43` retains the existing 1D FLD radiation-pressure force and force-work-conjugate field payment. See NUMERICS §6.7 for equations, limits, and ledger interpretation.
  ...
  - `outer_accel: string`（既定 `"auto"`；許容値 `{"auto","none","anderson","grey"}`；`"auto"` は 1D で `"grey"`、2D_RZ で `"none"` に validate 時解決。`"grey"` は 1D FLD 外側反復の多群-灰色合成加速（NUMERICS §6.7、2026-09-24; 収束解は加速なしと同じ; 2D_RZ では ConfigError）。`"anderson"` は opt-in Anderson acceleration of the FLD outer fixed-point iteration; the converged exit is always the raw Newton output。2D_RZ に加えて 1D_SPH にも適用（2026-09-14 移植、`fld_anderson.cuh`; 有効時は 1D 外側ループのパイプライン化を使わない; 常駐ループは非対応で通常ループに戻る）。NIF DS デッキ `examples/nifds/liquid_d2_1d.py` は `"anderson"`）
  - `limiter_evaluation: string`（既定 `"predictor"`；許容値 `{"predictor","iterate"}`；1D FLD の流束制限子を評価する放射場。`"predictor"` は最初の外側反復の放射場で評価して step 内固定、`"iterate"` は毎反復評価し直す従来方式。NUMERICS §6.7、2026-09-24）
  ...
  - `fleck_form: Literal["be","exp_phi1"]`（既定 `"be"`、2026-07-16 導入 — β_sec 後続裁定 §12 の係数レバー）: Fleck 因子の時間形状。`"be"`（既定・bit 保存） = f = 1/(1+z)（従来の backward-Euler 形）。`"exp_phi1"` = f = (1−e^{−z})/z = φ₁(−z) — 固定輻射スカラー緩和の厳密保持率（0<f≤1、stiff 極限 z·f→1 で交換維持）。fleck_beta と直交（z の β には tangent/secant がそのまま入る）。**1D FLD のみ**（persistent path 含む; 2D fleck kernel は独立実装で be 固定）。設計 = docs/design/fleck_exp_source_20260716.md。表不透明度（tmat/table_nlte/ionmix）のセルの NLTE 係数カーネルも exp_phi1 形を実装する（2026-09-24。2026-09-23 まで受理して黙って無視、09-23〜24 は `ConfigError`）。多材料デッキで LTE の表・周波数依存 Marshak 不透明度が支配するセルは、単一材料の表と同じく Planck 平均の吸収係数による群共通の Fleck 因子を使う
...
- `sn_transport: dict`（`mode="sn_transport"` で使用。NUMERICS §6.8 参照）
  ...
  - `spatial_scheme: Literal["diamond_difference","linear_characteristic","linear_discontinuous"]`（既定: 1D（`Main.dimension="1D_SPH"`/`"1D_CYL"`）は `"linear_discontinuous"`、2D_RZ は `"linear_characteristic"`（2026-09-25）。`"linear_discontinuous"` は 1D 専用（2D_RZ は `ConfigError`）: 強度・放出・散乱源・電子温度をセル内で線形（2 節点）に持ち、セル内の温度分布をステップ間で保持する。光学的に厚いセル（\(\sigma\Delta r\gtrsim1\)）の拡散極限の流れが正しく、放出は節点温度まわりの Newton 線形化、放出結合は吸収率密度に対する GMRES（灰色低次系の前処理）、散乱は整合 P1 補正の反復で解き、物質と放射が同じエネルギーを交換する（NUMERICS §6.8.4）。純吸収体のビーム減衰はセル光学厚さの 2 次精度（セル当たり \(1/(1+\tau+\tau^2/2)\)）で、光学厚さ \(\gtrsim1\) のセルを通る非散乱ビームの減衰が支配的な問題は `"linear_characteristic"` の方が正確（その厳密減衰はセル当たり \(e^{-\tau}\)）。`"diamond_difference"` は deprecated regression option として DD 経路の後方比較にのみ残す。`inner_acceleration="anderson"` は `"linear_discontinuous"` と併用不可（`ConfigError`）で、`dsa_enabled`・`inner_graph_unroll`・`outer_tol_stagnation_factor`・`diffusion_fallback_mode`・`tau_diffusion_on/off` は `"linear_characteristic"` にのみ効く。`"linear_discontinuous"` での意味: `max_outer_iterations` = 節点温度の Newton 反復数、`outer_tol` = 節点温度の相対変化の許容値（収束しなければ driver の再試行を要求）、`inner_tol` = 各 Newton 反復で GMRES 残差を初期値から縮める比（散乱の反復はその 0.1 倍まで）、`max_inner_iterations` = Newton 反復 1 回あたりの Krylov 反復数と輸送 1 回あたりの散乱反復数の上限）
  ...
  - `grey_preconditioner: Literal["auto","on","off"]`（既定 `"auto"`；2026-09-25 導入）: `spatial_scheme="linear_discontinuous"` の放出結合 GMRES の灰色低次系（整合 P1）前処理。`"auto"` は Newton 反復ごとに、節点の前処理の利得 \(\gamma_j=\bar\kappa_j\bar\sigma_{a,j}/\mathrm{rem}_j\) の最大が 0.1 を超えるときだけ前処理を使い、それ以下では前処理なしで解く（P1 系の組み立て・分解・求解を省く。収束解と許容値は同じで、Krylov の反復列が変わるので解の丸めが変わる。NUMERICS §6.8.4）。`"on"` は常に使う（2026-09-25 より前の動作）、`"off"` は使わない。`"on"`・`"off"` は `spatial_scheme="linear_discontinuous"` でのみ有効（他の scheme で与えると `ConfigError`。他の値も `ConfigError`）。frozen config に書く（この版より前の checkpoint の frozen config には既定値 `"auto"` を補う）。
  - `inner_graph_unroll: int`（既定 `5`；1D_SPH source iteration CUDA graph の unroll 数。残差はこの回数ごとに確認する。`1` は graph capture を無効化する。散乱がゼロ（\(\sigma_s\) が全要素で 0）の外側反復では 1 回のスイープで確定するので使わない（NUMERICS §6.8、2026-09-24）。有効範囲：`>= 1`）
  ...
  - `boundary: dict`
    ...
    - `outer_r: Literal["vacuum","reflect","marshak"]`（既定 `"vacuum"`；`"vacuum"` は outer incoming intensity zero、`"reflect"` は 2D_RZ の閉円筒/1D-z reduction 用。1D_SPH では `"reflect"` は `ConfigError`。`"marshak"` は 1D_SPH: 内向き半区間へ \(\psi^-=2F_{inc,g}\) を入射（`spatial_scheme` は `"linear_characteristic"` または `"linear_discontinuous"`。`"linear_discontinuous"` では flux 駆動（`sn_transport.marshak.flux_erg_per_cm2_s`）を \(\psi^-=F_{inc}/\sum_{\mu<0}w|\mu|\) として入射し、離散入射流束が指定 flux に一致する）。駆動は `Radiation.boundary.marshak_Tr_eV > 0`（黒体・multigroup 可）または `sn_transport.marshak.flux_erg_per_cm2_s > 0`（灰色 `groups=1`、`flux_pulse_duration_s` 対応）の排他的二択、両方指定・両方ゼロは `ConfigError`。NUMERICS §6.8 1D_SPH Marshak 参照）
...
- `profile: dict`（既定：ビーム個別設定で上書き可；全ビーム共通のビーム強度プロファイル）
  - `model: Literal["gaussian","super_gaussian","flat_top","table","custom"]`（既定 `"gaussian"`。`"table"` のキーは `LaserBeam.profile` と同じ）
  ...
  - `radius_um: float`（flat_topビーム半径 [µm]；`model="flat_top"` 専用。有効範囲：`> 0`。従来の書き方 `w0_um` も同じ半径として受理する（両方の指定は ConfigError））
...
- `lasermesh: dict`（`enabled` キーは無い — 指定すると未知キーとして扱う）
  ...
  - `ghost_corona: dict`（ゴーストコロナ設定。1D_SPH シャープギャップ対応。NUMERICS §5.7.5参照）
    ...
    - `transition_resolved_nhat: float`（既定 0.9；有効範囲：`> 0`；resolved-corona 判定閾値 \(\hat{n}_{\text{resolved}}\)。`enabled=True` なら常に使用（ゴーストプロファイルの減衰、NUMERICS §5.7.5.2。2026-09-23〜）し、`transition_enabled=True` ではハンドオフの遷移モデルにも使う）
    - `transition_resolved_cells: int`（既定 3；有効範囲：`≥ 1`；ゴーストが消え、ハンドオフがベースラインに復帰するのに必要な連続亜臨界セル数 \(N_{\text{required}}\)。`enabled=True` なら常に使用（NUMERICS §5.7.5.2）し、`transition_enabled=True` ではハンドオフの遷移モデルにも使う）
    - `transition_density_exponent: float`（既定 1.0；有効範囲：`≥ 0`；密度バイアス指数 \(\alpha_\rho\)。0 で密度バイアス無効。`transition_enabled=True` 時のみ使用）
- `raytrace: dict`
  - `integrator: Literal["auto","leapfrog","characteristic"]`（既定 `"auto"`。`"leapfrog"` = 可変ステップ Verlet の行進（NUMERICS §5.3.2、1D では球のみ）。`"characteristic"` = 1D_SPH の `mode="raytrace_2d"` 専用で、各レイを保存量（球・円柱の断面の角運動量 \(B\)、円柱の軸方向速度、平板の横方向速度）の特性曲線に沿って区間（半径節点・hydro セル面・事象）ごとに適応 Gauss–Kronrod 求積で積分する（NUMERICS §5.3.6。ステップ幅の離散化誤差がなく、転回点への接近で作業量が増えない。`cfl_ray`・`ds_adapt_*` は使わない。円柱・平板は (g)）。1D_SPH/`raytrace_2d` 以外で指定すると `ConfigError`、`Mesh.geometry_1d` が球以外で `"leapfrog"` を指定すると `ConfigError`。`"auto"` は 1D_SPH/`raytrace_2d` で `"characteristic"`、それ以外で `"leapfrog"` に解決され、frozen config には解決後の値を書く。その他の値（旧予約の `"rk2"`/`"rk4"` を含む）は `ConfigError`。2026-09-24 導入）
...
- `raytrace_skip: dict`（レイトレースSkip最適化；プラズマ条件がゆっくり変化する場合に前回結果をパワースケーリングして再利用。NUMERICS §5.9参照）
  - `enabled: bool`（既定 False — 2026-08-07 に既定を無効化。`laser.mode="radial_absorption_1d"`、熱い電子モデル有効時、2D の CBET 有効時は、有効にしても省略しない）
  ...
  - `crit_guard: float`（既定 0.01 [無次元]；有効範囲：`[0, 1)`；臨界帯の幅。いずれかのセルで、キャッシュ時と現在の \(\hat n=n_e/n_{crit}\) が帯域 \(\hat n_{margin}-\)`crit_guard`（\(\hat n_{margin}\) は `Laser.lasermesh.critical_margin`）を横断した場合（両向き）は強制再計算。臨界面近傍では \(\hat n\) の微小変化が terminate の有無を左右するため、横断があれば Skip は安全でない。0 でも横断の判定は \(\hat n_{margin}\) で行う。NUMERICS §5.9.4参照）
...
- `profile: Optional[dict]`（ビーム強度プロファイル；未指定時はLaser全体の `profile` を使用）
  ...
  - `r_max_um: float`（custom用・必須；`Laser.profile` と同じく \([0, r_{max}]\) の 2001 点でテーブル化）
...
- `radiation_thermal_subcycle: bool`（既定 `False`；True のとき、`Radiation.imc.two_stage=False` の Radiation 演算子で compressed cell が \(T_{e,floor}+0.5\) eV 以内まで落ちた場合に、熱力学状態を復元して \(n_{sub}=2,4,8\) の順に再試行する。この Radiation 演算子は thermal substep ごとに `Radiation -> Qei -> Conduction` を評価し、電子・イオン交換と電子熱伝導はこの中で 1 step に 1 回だけ行う（Hydro の 2T エネルギー更新は交換しない、伝導の standalone Strang 位置は呼ばない; 2026-09-23 以前は Hydro も交換していた）。NUMERICS §2.1, §11.2 参照）
- `materials: dict`（per-material state/schema introduction; top-level `Materials(...)` とは別名前空間）
  - `per_material_conservation_enabled: bool`（既定 `False`；per-material conservation arrays の master switch。False では `State.mass_per_material`, `Ee_per_material`, `Ei_per_material`, `Te_per_material`, `Ti_per_material` と valid flags は size 0 のまま。**2D_RZ 専用** — 1D_SPH では `ConfigError`（2026-09-23、理由の記述を 2026-09-25 に更新）。1D はセルごとに電子温度とイオン温度を 1 つずつ持ち、複数の材料を含むセルは混合物として閉じる。材料別の熱力学（材料別 EOS 射影・Qei・伝導）は各材料の電離度を核電荷 Z（または `Materials.zbar` の `fixed_value`）とし、1D の電離モデル（Thomas–Fermi・表）を使わない。また 1D の流体・レーザー・輻射・燃焼はセルのエネルギーだけを更新する。1D で材料別の温度を持たせるには、材料別の電離・閉包・流体の仕事の分配・輻射とレーザーの吸収の分配・ALE1D の材料別再配置を新たに設計する必要がある）
  ...
  - `hdf5_emit_derived_per_material: bool`（既定 `False`；`per_material_conservation_enabled=True` のとき、スナップショットの `hydro/per_material/v1/` に材料別の導出量 `rho_derived`・`ee_derived`・`ei_derived`・`Te_derived`・`Ti_derived`・`Pe_derived` も書く（出力専用。§7.2））
...
- `hydro: dict`
  ...
  - `pressure_tension_cutoff: bool`（既定 `False`；`True` で 1D の各 EOS 閉包の直後に、全圧 \(P_e+P_i\) が `pressure_tension_cutoff_value` を下回るセルの両種の圧力を同じ比で縮めて全圧をその値にする（張力カットオフ；値 0 なら両種とも 0 で、床付き相では pdV 仕事をしない）。流体は表 EOS の cold curve の張力を支えられない — PROPACEOS 由来の液体 D2 表は初期状態で −7 kbar かつ \(\partial P/\partial\rho|_T<0\) で、界面と丸め誤差の指数的不安定を駆動する。温度・エネルギーは変えず、力・仕事・人工粘性は同じ床付き圧力を読む。`Main.dimension="1D_SPH"` 専用。NUMERICS §3 参照。2026-09-15）
  - `pressure_tension_cutoff_value: float`（既定 `0.0` [dyn/cm²]；有効範囲 `<= 0`、有限。`pressure_tension_cutoff=True` のときの全圧の下限）
  - `T_start_inactive_cells: str`（既定 `"passive_fill"`；許可値 `{"passive_fill", "rigid_wall", "cold_equilibrium"}`；`T_start_eV > 0` のときの非活性セルの扱い。`"passive_fill"` = 従来（圧力寄与ゼロ、隣接セルのいずれかが活性ならノードが動く、低密度フィル専用）。`"rigid_wall"` = ノードは両側のセルが活性のときだけ動き、非活性セルは剛体壁として体積・エネルギーを保持、電子イオン緩和は非活性セルでもマスクしない（多材料の高密度界面向け、閾値は初期温度のすぐ上に置く）。`"rigid_wall"` は `Main.dimension="1D_SPH"` 専用。NUMERICS §2.1.1参照。`"cold_equilibrium"` = hydro のマスクを行わず、`T_start_eV` を低温平衡構成則（電子側の自由エネルギー補正 \(w(T_e)C(v)\)、NUMERICS §1 (b)）の電子温度遷移の終点として使う。1D_SPH・2T・表 EOS（sesame/ionmix/tmat）専用で、全非 void 材料に `eos.cold_reference` が必須、`pressure_tension_cutoff=True`・persistent loop とは併用不可、`qei_heat_capacity="table"` が必須（2026-09-16）。`compatible_energy=True` とは 2026-09-26 から併用でき、互換エネルギー更新が cold エネルギーの変化を電子の熱量座標から返して全エネルギーを丸め誤差まで保存する（NUMERICS §1 (b)））
  - `cold_equilibrium: dict`（`T_start_inactive_cells="cold_equilibrium"` の数値パラメータ。`transition_begin_fraction: float`（既定 `0.5`；\(T_a=\)fraction\(\times T_{start}\) から \(T_{start}\) まで 5 次スムーズステップで補正を 0 にする）/ `density_core_ratio: float`（既定 `1.10`；\(|\ln(\rho/\rho_0)|\le\ln\)core で補正が全効）/ `density_outer_ratio: float`（既定 `1.50`；\(\ln\)outer で補正が 0）/ `inverse_max_iterations: int`（既定 `80`；\(q_e\to T_e\) の安全化 Newton の反復上限）。NUMERICS §1 (b) 参照）
  ...
  - `trial_volume_cfl_shrink_fraction: float`（既定 `0.5` [無次元]；有効範囲：`(0, 1)`；trial-volume diagnostic が failure を検出した場合に報告する suggested retry timestep 係数 \(\Delta t_{suggested}=f_{shrink}\Delta t\)。現状ではログ専用）
  ...
  - `regime_aware_corner_j_guard_enabled: bool` (default `False`; per-cell mesh regime metadata (2D_RZ) for the pre-hydro corner-J trigger. False allocates no regime buffer and preserves the legacy global threshold path. True classifies active cells after StepStart geometry refresh and lets the pre-hydro trigger read per-cell `CellRegime.trigger_scale_threshold`. NUMERICS §3.2.13a)
  - `axis_margin_guard_enabled: bool` (default `False`; axis-face replacement predicate (2D_RZ). False uses the generic corner-J check for all cells. True makes `i=0` cells use the five-condition axis-margin predicate while non-axis cells keep the generic corner-J predicate. NUMERICS §3.2.13a)
  ...
  - `total_energy_remap_2d_rz: bool`（既定 `False`；2D_RZ conservative ALE remap の total-material-energy recovery path。True では electron/ion internal energy を別々に remap せず、corner-mass nodal kinetic energy と material internal energy の和を conservative scalar として remap し、post-remap node projection 後に同じ corner-mass nodal kinetic energy を差し引いて `e_e/e_i` を bounded `Y_e^int` tracer から復元する。CSR multiblock `topology_scheme="multiblock_half_butterfly_trifan_cap_5block"` では RZ corner-mass-consistent velocity projection、CSR hydro mass-positivity limiter、KE-realizability nodal velocity limiter、report-first `E_floor_injected` accounting を使う。False では legacy `e_e/e_i` separate remap path を選ぶ。structured `polar_center_treatment="tri_fan"` および未検証 CSR topology では True は ConfigError）
	  ...
	  - `av_model: Literal["scalar_vnr_legacy","csw_edge","csw_edge_csw98","csw_edge_plus_tensor_limited","mimetic_tensor_v1"]`（既定 `"scalar_vnr_legacy"` for all topologies; no topology-default magic. `"csw_edge"` selects the Phase 4 DRACO/HYDRA-class edge AV surface and must be paired with `subzonal_pressure_enabled=True`. `"csw_edge_csw98"` selects the CSW-1998-faithful median-mesh edge AV (I1-B Stage-G W1; NUMERICS §3.2.9c): median-mesh S per Eq. 16, strict compression switch, logical-line continuation-edge limiter (Eq. 18), force-coupled AV CFL. It runs the same compatible force/work path and is valid with OR without `subzonal_pressure_enabled` (column A/B isolation). `"csw_edge_plus_tensor_limited"` is a Stage G placeholder and is rejected at validation with `ConfigError("Stage G tensor AV is not yet implemented; use csw_edge")`。`"mimetic_tensor_v1"` selects the Campbell–Shashkov nonsymmetric mimetic tensor AV (consult-4 §7 v1; NUMERICS §3.2.9e): cell-local polygonal mimetic stiffness \(K_c=\mu_c[(g_Rg_R^T+g_Zg_Z^T)/A_c+P_c]\) applied per velocity component, corner forces on the compatible corner-force path (planar momentum, AW RZ work pairing), edge-AV replaced entirely (mutually exclusive with the CSW families). v1 REQUIRES the AW compatible trio: `aw_compatible_force_work=True`（すなわち `rz_momentum_scheme="area_weighted_symmetric"`, `subzonal_pressure_enabled=True`）かつ 2D。それ以外は `ConfigError`）
	  - `corner_mass_convention: string` (default `"kinematic_basis_rz_v1"`; Selects the corner-mass partition of cell mass onto its four corners for the compatible discretization (node mass/KE/projection/energy-audit share it). bbsw_radial_v0: the logical-index radial-linear BBSW lump (exact R-weighted Q1 lump on i-aligned radial rectangles; degenerates to an equal split on axis-column cells). kinematic_basis_rz_v1: m_k = M·int(N_k R J)/int(R J) by 2x2 Gauss — the exact R-weighted lump of the Q1 kinematic basis for ANY bilinear quad. P1 triangle-degenerate cells use the exact P1 closed form w_k=(r_k+Σr)/(4Σr) under kinematic_basis_rz_v1. Default kinematic_basis_rz_v1 as of the Wave-2D G4 epoch (2026-07-27); bbsw_radial_v0 remains selectable and is the permanent resolution for legacy frozen configs that predate the key. The multiblock exact-subpolygon path is not governed by this knob. The `BbswRadialV0` and `KinematicBasisRzV1` conventions apply to STRUCTURED topologies only; multiblock topologies use the canonical exact-subpolygon partition when subzonal pressure is enabled and the exact-subpolygon fallback when it is disabled, with no convention switch. Vocabulary lock (A124(b), 2026-08-15): the "canonical exact-subpolygon partition" is the Σ-normalized form (`compute_quad_corner_masses_partitioned_subpolygon`; mass-closed to roundoff on every quad), the "exact-subpolygon fallback" is the parent-volume-normalized form (`compute_quad_corner_masses_exact_subpolygon`; equal to roundoff on non-folded quads, NOT mass-closed on folded quads); the runtime selector is the Lagrangian-invariant flag (the subzonal/hourglass 4-key OR), not `subzonal_pressure_enabled` alone and not the AV model. Per-topology contract: docs/design/a124b_corner_mass_contract_20260815.md.)
	  ...
	  - `axis_projection.{enabled, shadow_only, q_on, q_floor, log_every_n_steps, patch_halfwidth}`（defaults `False`/`True`/`0.20`/`0.05`/`0`/`2`; disabled is bit-exact zero-cost. Axis geometric-admissibility layer at the corrector pre-commit point (2D_RZ multiblock polar family): monitors tier axis-adjacent cell columns' predicted corner-area ANISOTROPY (min corner / cell mean, compression-neutral since 2026-07-31), logs `[axis-proj-shadow]` crossings of `q_on`; with `shadow_only=False` an exact active-set QP computes the smallest mass-weighted pos_v correction holding endangered corners above `q_floor×(cell mean)` (≤12 candidates, ≤8 deterministic iterations, axis nodes δu_R=0), gated by an exact nonlinear recheck and a `dE≥0` energy backstop; removed kinetic energy deposits into patch ion internal energy (exactly mass-closed) with an `E_axis_projection` ledger and applied/rejected counters. Measured status (2026-07-31, GXII-scale implosion): the shadow layer is a validated zero-perturbation death-precursor detector (~20-step lead on the axis-wedge inversion), but the terminal collapse is a macroscopic flow-driven event (countering it needs O(100 erg)/step kinetic injection) — the active layer cannot stop THAT wall and every unsafe correction is rejected; retained as production-safe machinery for weaker perturbation-class hazards and diagnostics（W17a-e））
	  - `subzonal_dt_limiter_enabled: bool` (default `True`; gates the subzonal-pressure frequency dt bound (an upper-bound stiffness estimate from corner masses/volumes). `False` removes only the dt limiter — the subzonal force/work physics is untouched. Escape hatch for venues where the bound is spuriously tight (e.g. polar-tier post-remap corner states, 2026-07-29); the champion-era reference binaries predate this limiter entirely.)
	  - `rz_momentum_scheme: Literal["volume_weighted","area_weighted_symmetric"]`（既定 `"volume_weighted"`；`"area_weighted_symmetric"` は FIX2-W1/W2 の structured single-block および multiblock CSR 2D_RZ momentum operator。通常は `Main.dimension="2D_RZ"`, `av_model="scalar_vnr_legacy"`, `subzonal_pressure_enabled=False` が必須だが、`aw_compatible_force_work=True` の opt-in では `av_model="csw_edge"`/`"csw_edge_csw98"`/`"mimetic_tensor_v1"` と `subzonal_pressure_enabled=True` の compatible trio を許可する。この trio は `multiblock_polar_tier` の mixed quad/triangle cells を support する。multiblock tri-fan-cap topology は v1 では引き続き `ConfigError`。opt-in が False の内部エネルギーは既存の geometric `P dV` 非compatible 更新を保持する。NUMERICS §3.2.5）
	  - `aw_compatible_force_work: bool`（既定 `False`；True は `rz_momentum_scheme="area_weighted_symmetric"`, `av_model="csw_edge"` / `"csw_edge_csw98"` / `"mimetic_tensor_v1"`, `subzonal_pressure_enabled=True` の三条件を必須とし、違反は `ConfigError("Numerics.hydro.aw_compatible_force_work requires rz_momentum_scheme=\"area_weighted_symmetric\" with av_model=\"csw_edge\", \"csw_edge_csw98\" or \"mimetic_tensor_v1\" and subzonal_pressure_enabled=true")`。BBSW/AW force を compatible corner-force/work buffers に通す。momentum は planar の \(f_{AW}+f_{sz}+f_{AV}\) を planar node mass で消費し、内部 work は true-RZ 測度で測る（pressure は \(2\pi r_{node}\) pre-scale バッファ、subzonal は corner ごとの \(2\pi r_{node}\) 重み、edge-AV は \(\Delta(2\pi r\,\mathbf{u})\) 対）。外部駆動 traction は momentum と境界台帳のみに入り、cell 内部 work には入らない。NUMERICS §3.2.9b の representation split 参照。`multiblock_polar_tier` では `polar_tier_hydro_enabled=True` も必須で、triangle active slots と fixed-origin force constraint を使用する。False は legacy AW path を bit-frozen に保持し、frozen config では既定値を省略する。NUMERICS §3.2.9b）
	  ...
	  - `pentagon_affine_null_enabled: bool` / `pentagon_affine_null_kappa: float`（defaults `True` / `0.03`; kappa valid `[0,1]`. Affine-null damping on native-pentagon cells only（NUMERICS §3.2.9d）: \(f^{AN}=-\kappa_{AN}(c_s/h_c)M u^{\perp}\) with \(u^{\perp}\) the mass-weighted non-affine velocity residual of the five vertices（fixed-order 6×6 Cholesky fit; a cell whose mass-weighted affine Gram is not positive-definite contributes zero damping and is counted via the `[pentagon_an]` telemetry line — A218(a) cell-wise admissibility gate, 2026-08-10）, added into the subzonal corner force before the single mirror-invariant \(k=2\) closure. Exactly zero for every affine field（translation/rotation/dilation/shear）, zero force and torque sums, non-negative compatible heat. Meshes without pentagon cells are unaffected（legacy byte surface preserved; keys frozen-config elided at defaults）. Qualification: tests/hydro/test_pentagon_affine_null.cpp（annihilation, sums, dissipation, rank-4 observability χ≥1e-3）; production κ chosen by the measured sweep（see verdict ledger A56））
  ...
  - `csw98_degenerate_side_floor_rel: float`（既定 `1.0e-2` [無次元]；`av_model="csw_edge_csw98"` の辺長退化 guard \(\eta\)。\(l_e^2<\eta^2 A_c\) の辺は零長面として粘性運動量 flux を伝達せず、\(\eta\le0\) で guard 無効。NUMERICS §3.2.9c 参照）
  - `csw98_damper_impulse_beta: float`（既定 `0.0` [無次元]；`av_model="csw_edge_csw98"` の side-force impulse clamp \(\beta\)。\(|F_e|\Delta t\le\beta\mu_e|d\mathbf{v}_e|\), \(\mu_e=m_0m_1/(m_0+m_1)\) を満たすよう force を縮小し、\(\beta\le0\) で clamp 無効。bound を満たす場合は scale が厳密に 1.0 で force は不変。2026-08-20 A377 で既定を 0.15→0.0 に再裁定 (構造格子強駆動 launch で能動と実測・freestream deck は明示 opt-in)。NUMERICS §3.2.9c 参照）
  - `csw98_axisline_av_mode: Literal["off","d1prime"]`（既定 `"off"`；`"d1prime"` は AW planar csw98 の軸線 edge を復帰し、速度 jump を \(e_z\) へ射影してから switch/limiter/impedance/force/work/CFL を評価する D1' constraint-space operator \(F=P F(Pu)\)。`TENRYU_AW_AXISLINE_AV=1` は優先して非射影 pre-D1 operator を復帰する。既定 OFF は現行 D1 exclusion と bit-identical。NUMERICS §3.2.9c 参照）
  - `csw98_axisline_d1prime_cfl_enabled: bool`（既定 `true`；Enable the damping-eigenvalue (lambda) AV-CFL dt bound for axis-line d1prime edges. When false, no AV dt candidate is contributed there; stability rests on the damper impulse clamp.）
  - `csw98_limiter_shock_floor_enabled: bool`（既定 `false`；Floor the 2D csw98 edge-AV limiter attenuation at `csw_shock_limiter_floor` on compressive sides (not weighted by the limiter: a smooth compression keeps the floor as well; the 1D kernel weights its floor by 1 - psi since 2026-09-25, NUMERICS §3.1.6). Default false is bit-identical to prior behavior.）
  - `csw98_axisline_work_planar_enabled: bool`（既定 `false`；Book axis-line edge-AV work in the planar metric (2*pi*r pairing nulls on-axis dissipation). Default false is bit-identical.）
  ...
  - `tensor_av_C1: float`（既定 1.0 [無次元]；有効範囲：`[0, 10]`；`av_model="mimetic_tensor_v1"` 専用の線形係数 \(C_1\)（\(\mu_c=\sigma_c\rho_cL_c[a_c+\sqrt{a_c^2+C_1^2c_s^2}]\)）。CSW 系の `av_C1`/`csw_C1` とは分離。既定値のとき frozen config では省略（elide）し、既存 frozen config のバイト同一性を保つ。NUMERICS §3.2.9e 参照）
  - `tensor_av_C2: float`（既定 1.0 [無次元]；有効範囲：`[0, 10]`；`av_model="mimetic_tensor_v1"` 専用の2次係数 \(C_2\)（\(a_c=C_2\frac{\gamma+1}{4}\Delta u_c\)）。CSW 系の `av_C2`/`csw_C2` とは分離。既定値のとき frozen config では省略（elide）。NUMERICS §3.2.9e 参照）
  - `csw_pole_floor_enabled: bool`（既定 `False`；consult-5 beta の極帯 limiter floor。`av_model="csw_edge_csw98"` かつ multiblock topology 必須（それ以外は `ConfigError`）。有効時、multiblock csw98 の force と AV-CFL の limiter 欠損 \(s_e=1-\psi_e\) を \(\max[s_e,\ \sigma_0 W_{pole}(\theta_e)S_e]\) に床上げする。\(W_{pole}\) は極からの角距離 \(d=\min(\theta,\pi-\theta)\) の cosine feather（\(d<\theta_0\) で 1、\(\theta_0+\theta_f\) まで減衰）、\(S_e\) は当該 edge の隣接 2 セル全 face の \(s\) の固定順 max（厳密 affine 流では全 \(s=0\) → floor=0 で uniform-affine ゼロ粘性ゲートを保存）。既定 OFF は既存 csw98 経路をバイト同一に保つ。frozen config は有効時のみ 4 キーを emit（default-off elide）。NUMERICS §3.2.9c 追補・VERIFICATION §18 参照）
  - `csw_pole_floor_sigma0: float`（既定 1.0 [無次元]；有効範囲 `[0,4]`；床の強度 \(\sigma_0\)。qualification は 1.0 から開始し、shock/entropy 指標が許容後にのみ低減）
  - `csw_pole_floor_theta0_rad: float`（既定 0.033 [rad]；有効範囲 `[0,0.5]`；full-floor の極角半幅（≈2 列 @192 角セル））
  - `csw_pole_floor_thetaf_rad: float`（既定 0.033 [rad]；有効範囲 `[0,0.5]`；cosine feather 幅）
  - `csw_rz_lift_enabled: bool`（既定 `False`；consult-5 §2.2 の RZ-congruent lift の edge 版（台帳 A79）。`av_model="csw_edge_csw98"` + `aw_compatible_force_work=True` + multiblock topology 必須（それ以外は `ConfigError`）。有効時、csw98 edge AV の**配分**を非対称化（\(n_0\mathrel{-}=(\bar R_e/R_0)F,\ n_1\mathrel{+}=(\bar R_e/R_1)F\)、\(\bar R_e=(R_0+R_1)/2\)）し、true-RZ **work 対**を \(w=-2\pi\bar R_e\,F\cdot\Delta v\) に置換する — 圧縮で厳密 \(w\ge0\)・boost 不変・RZ 測度合力ゼロ（NUMERICS §3.2.9c 追補）。軸接触 edge（\(R_i=0\)）と guard 違反 edge は従来対のまま。既定 OFF は既存経路をバイト同一に保ち、frozen config は有効時のみ 2 キーを emit）
  - `csw_rz_lift_guard_ratio: float`（既定 4.0 [無次元]；有効範囲 `[1,64]`；\(\bar R_e>g_R\min(R_0,R_1)\) の edge を lift 対象外にする軸近傍 guard。軸接触 edge の厳密散逸化は零測度ノードのため構造的に不可能（A79）— guard 内は従来対を温存し、残存欠陥として文書化）
  - `csw_axis_mirror_limiter: bool`（既定 `False`；consult-6 §3.13 の根治候補（台帳 A82）。multiblock csw98 endpoint limiter の継続探索 miss（3 分岐）のうち endpoint が軸ノード（\(R\le\)`axis_eps_cm`）の場合に、中立 1.0 の代わりに解析的パリティ ghost（\((R,Z)^g=(-R,Z)\), \((u_R,u_Z)^g=(-u_R,u_Z)\)）で反射エッジの継続比を既存ルーチンで評価し stencil を完全化する。structured 経路の aw_axis_slave 反射と同じ引数順規約。非軸 miss は従来どおり中立 1.0。`av_model="csw_edge_csw98"` + multiblock 必須。既定 OFF はバイト同一、frozen config は有効時のみ emit）
  - `csw_pole_desens_enabled: bool`（既定 `False`；consult-6 §3.14 の有界 front 脱感作（台帳 A87 — entropy ledger で AV work の極前線発火 per-mass 7→164× が堆積者と実測確定）。multiblock csw98 の force/AV-CFL の limiter 欠損を \((1-\alpha W_{pole}S_e)\) 倍に減衰（センサーは csw_pole_floor と共有・逆方向適用）。`csw_pole_floor_enabled` と相互排他。`av_model="csw_edge_csw98"` + multiblock 必須。既定 OFF バイト同一・frozen config は有効時のみ emit）
  - `csw_pole_desens_alpha: float`（既定 0.25 [無次元]；有効範囲 `[0,0.9]`；脱感作強度 α。受理基準（consult-6）: post-shock ringing 新発なし・shock 幅 ≥2 radial cells・RH jump 基準内・極エントロピー過剰の α 単調減・赤道不変）
  - `csw_pole_desens_theta0_rad: float`（既定 0.033 [rad]；full-window 極角半幅）
  - `csw_pole_desens_thetaf_rad: float`（既定 0.033 [rad]；cosine feather 幅）
  - `csw_polar_slaving_enabled: bool`（既定 `False`；consult-7 P\* の極 cohort limiter slaving（台帳 A89→E13 — 全 remedy 実測後の裁定: 制御変数は前線 AV 活性化の角度コヒーレンス。D4 大域 limiter-off が唯一の治癒実測・wake 輸送は RZ hoop 測度閉塞・振幅系は thermostat 補償で反証）。有効時、POLAR_SHELL block の shock-normal edge instance（同一 radial 行の θ 隣接セル間 face — 位相的判定）を cohort（極 p × 行 i × 役割 role∈{toward,away}·pole-canonical）に束ね、limiter 非依存の前線 gate \(\chi=[-\Delta u\cdot S]_+/(c_s\|S\|)\) の C¹ smoothstep（\(\chi_0\)〜\(\chi_1\)）の core max を G とし、strict 圧縮 (\(\Delta u\cdot S<-10^{-12}c_s\|S\|\)) な core メンバー \(\ge2\) のとき \(\psi^\*=\min\psi^{raw}\)（−0 正規化）を採り、one-sided 射影 \(a^{eff}=a^{raw}+\lambda[\max(a^{raw},a^\*)-a^{raw}]\)（\(\lambda=\)strength·\(w_q\)·G、\(a=1-\psi\)）で \(\psi^{eff}\le\psi^{raw}\) を保証（AV は増えるのみ・コヒーレント raw cohort で厳密不活性）。ψ_eff は **force と AV-CFL の両方**が消費（single-copy 規則 — CFL の粘性信号速度 \(du\cdot(1-\psi)\cdot\)proj は適用 force の bound であるべき）。OFF は host 分岐で既存カーネルを原文起動（byte 同一）。core 外 q は feather \(w_q=(1-\eta)^2(1+2\eta)\)、\(q\ge k_{outer}\) は不変。診断: modified/cohorts/nonfinite カウンタ（nonfinite は fatal）+200 step ログ。`av_model="csw_edge_csw98"` + multiblock + 2D_RZ 必須・`csw_pole_floor`/`csw_pole_desens` と相互排他。frozen config は有効時のみ 6 キーを emit。NUMERICS §3.2.9g・受理基準は consult-7 §8（壁遅延のみは不合格 — s1 縞根絶+0.7ns 生存+極外不変））
  - `csw_polar_slaving_full_columns: int`（既定 4；有効範囲 `[1,32)`；full-slaving core の列数 k_core（q=0..k_core−1 を w=1 で束ねる））
  - `csw_polar_slaving_outer_columns: int`（既定 6；有効範囲 `(full,64)`；feather 端 k_outer（q∈[k_core,k_outer) が smoothstep feather・以遠は厳密不変））
  - `csw_polar_slaving_chi_on: float`（既定 0.08 [無次元]；前線 gate の下端 χ₀ — これ未満の圧縮は slaving なし）
  - `csw_polar_slaving_chi_full: float`（既定 0.20 [無次元]；gate 上端 χ₁ — 以上で full slaving。`0<=χ₀<χ₁<=10` 検証）
  - `csw_polar_slaving_strength: float`（既定 1.0；有効範囲 `[0,1]`；診断 ladder 用の部分強度。production は 1.0（部分強度は limiter 差動を残すため最終 remedy ではない — consult-7 §4.12））
  - `csw_polar_slaving_min_columns: int`（既定 96；有効範囲 `[8,4096]`；P1 block 一般化 (2026-08-02) — POLAR_SHELL/POLAR_TIER の構造化 quad block のうち `n_j >= min_columns` を slaving 対象にする（既定で 192/192/178 の 3 block・1392 cohorts。TRANSITION_BELT (pentagon) と粗環 (88/44/22/12 列) は除外 — 粗環の Δθ では極冠 slaving が広角相関になるため保守的に不適用）。動機: 殻縞根絶後、収束前線が未 slaved の tier ring (block1) 極列 j=2 を同機構で圧潰させる migration を実測（A90/A91））
  - `csw_polar_slaving_av_stiffness_cfl_enabled: bool`（既定 `False`；consult-9 §3 の演算子剛性 AV-CFL（台帳 A95→E16a — P* が引き上げた AV 係数で細長軸セルの陽的安定 dt が運転 dt を下回り period-2Δt 体積呼吸が発生する実測 (P1L/OFFL2/DT3B 行列) への第一 remedy）。有効時、engaged cohort の shock-normal instance について force 組立と同一の凍結量（同 \(\rho_e, c_{s,e}\) 調和平均・同 S ベクトル (aw_planar)・同 \(a^{eff}\)）から閉形式 2×2 velocity-tangent \(\mathsf D_{c,e}=\rho_e a^{eff}[G^{\prime}q\,nn^T-G\,nS^T+(Gq/s)(I-nn^T)]\)（\(G=\alpha s+\sqrt{\alpha^2s^2+\beta^2}\), \(\alpha=\)av_quadratic\((\gamma+1)/4\), \(\beta=\)av_linear\(\,c_{s,e}\)；\(s\to0\) は Lipschitz bound \(2\rho a\beta|S|\)）を canonical 端点向きで face 毎に合算（片側 face は単側 — cap 境界で正常）し、spectral norm \(\kappa_e\)、節点 Gershgorin 行和 \(\lambda_n=2\sum\kappa/M_n\)（\(M_n\)=momentum 更新の実分母 `node_planar_mass` — このため `rz_momentum_scheme="area_weighted_symmetric"` 必須）、決定論 max で \(\Lambda\) を得て \(\Delta t\le\sigma/\Lambda\) を既存 dt 集約へ合流する。engaged cohort 無しなら無拘束。診断: 200 step 毎に Θ=Λ·dt・winner node。`csw_polar_slaving_enabled` 必須。既定 OFF は配列・カーネル・診断とも追加ゼロ。v1 の deferred: predictor 段再評価 soft-fail（本 deck は full-step retry 無効）・η_nf 実積分器認証試験。NUMERICS §3.2.9g 追補参照）
  - `csw_polar_slaving_av_stiffness_sigma: float`（既定 0.8；有効範囲 `(0,1]`；no-flip 安全率 σ_nf（consult-9 §3.2.3、η_nf=1 前提の初期生産値））
  - `wake_heat_flux_enabled: bool`（既定 `False`；consult-6 §3.2–3.12 の遅延 wake 限定・角度方向人工熱流束（台帳 A88 — 前線 AV 振幅調整の両方向反証後に残る唯一の consult-6 remedy）。有効時、各 step の hydro corrector 成功後に (i) wake 履歴センサー \(S_c=\max[S^{AV},S^{kin}]\)（\(S^{AV}=\mathrm{clamp}((a_c-0.01)/0.09)\)、\(a_c\) は列内 max 正規化の比 AV 加熱率；\(S^{kin}\) は radial face の圧縮 Mach \(M\) と圧力 jump \(J\) の smooth 積）と front hard mask \(B_c=[a_c>0.01]\lor[M>0.05\land J>0.10]\)、(ii) 遅延 2 状態メモリ \(\eta\)（fast attack・\(\tau_\eta=3t_{cross}\) decay）/ \(\zeta\)（旧 \(\eta^n\) 起源・\(\tau_{lag}=0.5t_{cross}\)・\(\tau_\zeta=3t_{cross}\)）、(iii) gate \(G_c=W_{pole}\,\zeta^n(1-S_c)^2(1-B_c)\)（現前線で厳密 0）、(iv) 角度方向 face のみの保存的伝導交換 \(C_f=A_f\kappa_f/d_f\)、\(\kappa_f=C_E\rho_f c_{v,f}c_{s,f}\ell_f G_f\)（\(A_f=\pi(R_0+R_1)L_f\)、\(\rho_f,c_{v,f},\ell_f\) 調和平均・\(c_{s,f},G_f\) は min）、per-cell 拡散数 cap \(s_c=\min(1,0.45/\Lambda_c)\) を \(\min(s_c,s_d)\) で face 適用（凸結合 — 新極値なし）。運動量・KE 不変、エネルギーは \(U=m e\) の pairwise 反対称更新で厳密保存。更新後に EOS 再 closure + 音速再計算。決定論: 固定順 face リスト + CSR gather（atomicAdd 不使用）。v1 制約: `2D_RZ` + `av_model="csw_edge_csw98"` + multiblock 必須、`restart_from` / `band_ale` 拒否（\(\eta,\zeta\) は cell 付着 — 生産 disk 構成では殻 remap が恒等のため厳密に material 付着。§3.4 輸送は follow-up）。CENTRAL_CORE と axis_survival_core disk plateau セルは除外・材質境界（dominant material 不一致 / PLIC 界面）越えの flux は 0。既定 OFF は確保・演算ゼロでバイト同一、frozen config は有効時のみ 5 キーを emit。NUMERICS §3.2.9f 参照）
  - `wake_heat_flux_CE: float`（既定 0.10 [無次元]；有効範囲 `[0,1]`；伝導係数 \(C_E\)。qualification sweep {0.05, 0.10, 0.20}。0 はセンサー/履歴のみ更新し flux を適用しない診断モード）
  - `wake_heat_flux_theta_a_rad: float`（既定 0.0327 [rad] ≈ 2Δθ@192；\(W_{pole}\) smoothstep の full-window 極角半幅）
  - `wake_heat_flux_theta_b_rad: float`（既定 0.0982 [rad] ≈ 6Δθ@192；\(W_{pole}\) が 0 に達する極角。`theta_a < theta_b <= pi` を検証）
  - `wake_heat_flux_global_theta: bool`（既定 `False`；True で \(W_{pole}\equiv1\)（全角度適用）。consult-6 §3.5 の第一 qualification 形態）
  ...
  - `csw_shock_limiter_floor: float`（既定 0.65 [無次元]；有効範囲：`[0, 1]`；\(\chi^{lim}\) の下限を \(f(1-\psi)\chi^{raw}\) とする。\(\psi\) は Caramana-Shashkov-Whalen (1998) 式 (12) の Christensen limiter（隣接セルとの速度勾配比から作る連続量）で、隣接セルが圧縮していない衝撃波の足元・後端では \(\psi=0\)（下限 \(f\chi^{raw}\)）、速度が線形な圧縮では \(\psi=1\)（下限 0）。NUMERICS §3.1.6 参照）
  ...
  - `bulk_viscosity_C: float`（既定 `0.0` [無次元]；有効範囲：`[0, +∞)`；1D_SPH 専用。cell-centered viscous pressure \(Q_{bulk}=C_{bulk}\rho c_s |(u_{j+1}-u_j)/\Delta r_i| \Delta r_i\) を既存 `Qvisc` に加算する。**圧縮セルのみ**（\(du<0\)）で作用し、post-shock 圧縮域の音響振動を直接減衰する（2026-07-26 是正: 旧仕様の膨張側適用は反散逸＝エントロピー減少であり撤廃（カーネルレビュー指摘））。`0` で無効。2D_RZ では未使用。NUMERICS §3.1.6 参照）
  ...
  - `plasma_viscosity` (dict, default disabled) — Braginskii plasma shear
    ...
    - `enabled` (bool, default `False`) — master switch. Disabled runs are
      bit-identical to pre-electron ion-only behavior.
    ...
    - `species` (str, `"ion"` | `"electron"` | `"both"`, default `"ion"`) —
      which Braginskii channel(s) feed π = −η_eff W. `"ion"`: ion-only legacy
  ...
  - `boundary`：次元に応じて型が異なる（ARCHITECTURE §4.1 Config::HydroConfig 参照）。
    ...
    - **2D_RZ**：`boundary: dict`（per-face 指定）
      ...
      - `z_bottom`/`z_top` の dict 形式は v1 では `{"type":"state_supply", "rho_g_per_cc": float, "u_z_cm_per_s": float, "T_eV": float}` のみをサポートし、`rho_g_per_cc`, `u_z_cm_per_s`, `T_eV` はすべて必須である。任意キー `drive_t_end_s`（float [s]、既定 +inf = 従来どおり恒久供給、有限値は > 0 必須）は**有限駆動窓**を宣言する: `t < drive_t_end_s` の間だけ面が SOURCE として振る舞い（zonal override・material v_z 復元・エネルギー tally・HLLC ghost 供給値・伝導 Dirichlet 熱源・remap 境界流入・供給エネルギー再閉包）、窓終了後は剛体壁として継続する（mesh/velocity の壁 pinning・CFL/anti-hourglass の壁扱いは元々無条件で不変、HLLC ghost はホスト側で REFLECT に解決 — interior copy では pinned 壁を物質が透過するため）。frozen echo は有限値のときのみキーを出力する（JSON は inf を担持できず、欠損 = 既定 inf で roundtrip 厳密）。FLD z 境界 `"state_supply"` と有限窓の併用は本版では `ConfigError`（FLD 供給閉包に窓後 fallback が未実装）。z-face 境界セルの `rho/mass/Te/Ti` を供給値へ戻し、`Te=Ti=T_eV` として既存 EOS closure により `ee/ei/Pe/Pi` を再計算する。境界セルの material `v_z` は各 step の boundary override 後に `u_z_cm_per_s` へ復元・保持される。ALE 有効時に mesh normal を z_min/z_max に固定し mesh node `v_z` をゼロにしても、これは mesh anchoring のみであり material `v_z` をゼロ化しない。`state_supply` は r-face/1D 境界では `ConfigError` とする。文字列 `"state_supply"` は曖昧さ回避のため不可で、dict 形式を要求する。ALE open-flow remap と reservoir/ghost tally はこの supplied material `u_z_cm_per_s` を用いる。
  - `boundary_pressure: Optional[callable]`（`boundary="pressure"` 時の駆動圧力 \(P_{drive}(t)\) [dyne/cm²]。シグネチャ：`P(t_s: float) -> float` [dyne/cm²]。初期化時に FrozenTable1D 化（サンプリング: 時刻 \(t_k = k\,h\)（\(h = 2^{-40}\) s ≈ 0.909 ps、\(k = 0, \ldots, \lceil t_{end}/h \rceil\)）で評価し、各区間で弦と中点値の差が局所値の \(10^{-6}\) を超える間は 2 分割して中点を加える（最小間隔 \(h/2^7\) ≈ 7.1 fs）。標本時刻も細分の判定もその時刻付近の関数値だけで決まるので、\(t_{end}\) の違う実行でも重なる範囲の表は同一（2026-09-24 に \([0, t_{end}]\) の 10000 等間隔点から変更）。基準点が \(2^{20}\) を超える長時間（\(t_{end} >\) 約 0.95 µs）では \(h\) を 2 倍ずつ広げて \(2^{20}\) 以下に収め、その場合だけ表は \(t_{end}\) の 2 冪の区分に依存する。線形補間テーブルとして凍結）。**端点外挿規則**：\(t < 0\) および \(t > t_{end}\) では \(P = 0\) とする（free境界と等価）。2D_RZ では `r_outer` 境界のみに適用。`boundary="pressure"` で未提供の場合は `ConfigError`）
  - `pressure_drive_perturbation: dict` controls the optional zonal multiplier applied to the pressure drive. The frozen form stores the resolved Legendre list and sets `random_rms=0.0`, so restart parsing does not redraw the random spectrum.

    | Key | Type | Default | Description |
    |---|---|---|---|
    | `Numerics.hydro.pressure_drive_perturbation.enabled` | bool | `False` | Enables the zonal pressure-drive perturbation; enabling an empty profile is a `ConfigError`. |
    | `Numerics.hydro.pressure_drive_perturbation.legendre_modes` | list[list[int, float]] | `[]` | Explicit `[l, amplitude]` pairs with `1 <= l <= 16`; the frozen value is the full resolved explicit-plus-random list. |
    | `Numerics.hydro.pressure_drive_perturbation.ring_spots` | list[list[float, float, float]] | `[]` | Gaussian ring triples `[theta0_rad, sigma_rad, amplitude]`, with `theta0_rad` in `[0, pi]` and `sigma_rad > 0`; at most four rings. |
    | `Numerics.hydro.pressure_drive_perturbation.random_seed` | int | `0` | Deterministic splitmix64/Box-Muller seed retained as provenance in frozen configs. |
    | `Numerics.hydro.pressure_drive_perturbation.random_l_min` | int | `2` | Inclusive minimum random Legendre degree; the range must satisfy `1 <= random_l_min <= random_l_max <= 16`. |
    | `Numerics.hydro.pressure_drive_perturbation.random_l_max` | int | `8` | Inclusive maximum random Legendre degree. |
    | `Numerics.hydro.pressure_drive_perturbation.random_rms` | float | `0.0` | Target angular RMS of the random Legendre component; values greater than zero activate deterministic resolution, while frozen configs write `0.0`. |
- `conduction: dict`
  - `enabled: bool`（既定 True；電子熱伝導の有効化。False の場合：電子熱伝導を完全にスキップ（`dt_cond=∞`）。`ion_conduction`、`ion_f_lim` および `f_lim` は無視される（イオン熱伝導も行わない）。NUMERICS §4参照）
  ...
  - `ion_conduction: bool`（既定 False；Braginskii のイオン熱伝導 \(\kappa_i=3.906\,n_iT_i\tau_i/m_i\)。電子の熱伝導とその記帳の後に、イオン温度を後退 Euler の三重対角系（640 セル以下は 1 ブロックの並列巡回縮約、それより長い線は cuSPARSE の gtsv2。電子の `solver` によらない）で 1 回解き、動いたエネルギーを \(e_i\) に記帳してから 1D 流体の EOS 閉包で \(T_i,P_i\) を閉じ直す。面の伝導率は `face_kappa_policy` に従い、面流束は `ion_f_lim` で制限する。1D（`Main.dimension="1D_SPH"`、`Mesh.geometry_1d` の平面・円柱・球）かつ 2T のみ。2D、1T、`Numerics.materials.per_material_conservation_enabled=True`、`eos.hydro_backend="mie_gruneisen"` の材料は `ConfigError`（2026-09-26。それ以前はキーを読むだけで何もしていなかった）。\(Z=1\) で \(\kappa_i\approx\kappa_e/(25\sqrt A)\) と小さいが、衝撃波の収束直後の中心のように \(T_i\gg T_e\) の領域ではイオンが熱を運ぶ主体になる。persistent loop は非対応（通常の経路へ戻る）。NUMERICS §4.6 参照）
  - `ion_f_lim: float`（既定 1.0 [無次元]；有効範囲：`(0, +∞)`、有限；イオン熱流束の制限係数。面ごとに \(\phi=1/(1+|q_{SH}|/q_{\max})\)、\(q_{\max}=f\,n_iT_i\sqrt{T_i/m_i}\)（面の \(\rho,T_i\) の算術平均と \(A\) の調和平均）。1.0 は上限をイオンの自由流束そのものにする（Hoffman et al. 2015 の標準の計算も同じ上限を係数 1 で使うが、\(\min(\kappa_i\nabla T_i, q_{stream}/f)\) の形の制限。調和型のこちらは \(|q_{SH}|=q_{\max}\) 付近で流束が最大半分まで小さい）。`ion_conduction=True` のときだけ使用。NUMERICS §4.6 参照）
  ...
  - `nonlocal_model: Literal["none","snb"]`（既定 `"none"`；SNB (Schurtz–Nicolaï–Busquet) 型非局所電子熱輸送の opt-in。`"none"` は従来の局所 Spitzer-Härm + flux limiter 経路を bit 恒等で保存。`"snb"` は群別拡散 H_g 方程式（1D: cusparse 群バッチ三重対角 / 2D: Kershaw 9-point 対称化 CSR + 群バッチ Jacobi-PCG）で SH 流束を非局所補正し、既存 `f_lim` cap を外側安全 cap として合成する（cap 発火は診断計上）。**1D_SPH（planar/cyl/sph）と 2D_RZ の両次元で利用可。いずれも `Main.temperature_model="2T"` + `conduction.enabled=True` + `conduction.solver="sts"` が必要**。2D は Kershaw 9-point 対称化ステンシル CSR + 群バッチ Jacobi-PCG（cusparse 三重対角の 2D 対応物）。`per_material_conservation_enabled=True` との併用、1T、`solver!="sts"` は `ConfigError`。`snb_efield="local"` は 2D で `ConfigError`（fail-closed、1D のみ）。設計 doc `docs/design/2d_snb_port_spec.md`（1D: `docs/design/snb_nonlocal_1d_20260710.md`）、NUMERICS §4.4（1D）・§4.5（2D_RZ）参照）
  - `snb_n_groups: int`（既定 24；有効範囲：`[2, ∞)`；`nonlocal_model="snb"` 時のみ使用。SNB エネルギー群数。群端は毎伝導ステップ β̂=E/(k_B max T_e) の幾何級数 [0.1, snb_E_max_over_Te]（+ [0, 0.1] の最下群）で再構築。24 群幾何で連続カーネル比 ≤0.8%（設計 doc §7C）。NUMERICS §4.4（1D）・§4.5（2D_RZ）参照）
  - `snb_E_max_over_Te: float`（既定 20.0；有効範囲：`(1, ∞)`；`nonlocal_model="snb"` 時のみ使用。群構造の最大エネルギー E_max/(k_B max T_e)。20 で未収容 tail ≤1.7e-5（未収容分は純 SH のまま扱われ保存を破らない）。NUMERICS §4.4（1D）・§4.5（2D_RZ）参照）
  - `snb_mfp: Literal["geometric_r2","original"]`（既定 `"geometric_r2"`；`nonlocal_model="snb"` 時のみ使用。群 mfp 変種。`"geometric_r2"` = Sherlock 2017 Eq(1) の幾何平均 φ 補正形 λ_g=2√2 β̄_g² λ_0、λ_0=T_e²/(4πn_e√(Z̄φ)e⁴lnΛ)、φ=(Z̄+4.2)/(Z̄+0.24)（Brodrick r=2 等価、高 Z で VFP 一致改善）。`"original"` = Schurtz 2000 Eq(3)+(23) の λ_g=2β̄_g²λ_B/√(Z̄+1)。NUMERICS §4.4（1D）・§4.5（2D_RZ）参照）
  - `snb_efield: Literal["none","local"]`（既定 `"none"`；`nonlocal_model="snb"` 時のみ使用。電場停止長による輸送 mfp 補正（Schurtz Eq 32–35）。既定 `"none"`：`geometric_r2` の r=2 較正は電場抑制を織込み済みで、重ねると二重計上になる（Sherlock 2017 §VII の裁定）。`"original"` 変種と併用する場合の推奨は `"local"`。NUMERICS §4.4（1D）・§4.5（2D_RZ）参照。**2D_RZ では "local" は ConfigError**）
...
- `positivity: dict`
  ...
  - `Te_min_eV: float`（既定 `1e-3`；有効範囲：`> 0`；`Mesh.floors.Te_floor_eV` のエイリアス。**v1.0 で非推奨（deprecated）**。使用時は WARNING 出力。実効値は `max(Mesh.floors.Te_floor_eV, Numerics.positivity.Te_min_eV)`（`Mesh` ブロックが `Numerics` より先に呼ばれる通常の順序の場合）。推奨は `Mesh.floors.Te_floor_eV` のみを使用すること（`Numerics.floors.Te_floor_eV` も同じ値を書き換える — `Mesh.floors` の項参照）
- `safety: dict`
...
- `diagnostics: dict`
  - `refinement_estimator: dict`（設計 doc §18 C1 の read-only modified-Lohner 検出場。hydro state へは feedback しない）
    ...
    |-----|------|---------|-----------------------|
    | `Numerics.diagnostics.refinement_estimator.enabled` | bool | `False` | True の場合のみ cadence 評価・ログを有効化し、評価済み host field が全セル長なら additive snapshot dataset `hydro/refine_error` [dimensionless] を出力する。False では dataset を作成しない |
    | `Numerics.diagnostics.refinement_estimator.every` | int | `25` | 評価間隔 [steps]。`>= 1` |
    | `Numerics.diagnostics.refinement_estimator.filter_eps` | float | `0.01` | modified-Lohner ripple filter。finite and `> 0` |
    | `Numerics.diagnostics.refinement_estimator.detect_cutoff` | float | `0.8` | theta 60--120 deg の front-band membership threshold。finite and in `(0, 1)` |

  - `conduction_energy_rate_export: dict`（設計 doc §18.3 の conduction operator-level 診断出力）

    | Key | Type | Default | Validation / behavior |
    |-----|------|---------|-----------------------|
    | `Numerics.diagnostics.conduction_energy_rate_export.enabled` | bool | `False` | True の場合、conduction 前後の authoritative `ee` 差と演算子後の `rho` から net electron-conduction energy rate per volume を計算し、additive snapshot dataset `hydro/conduction_e_rate` [erg/cm3/s] を出力する。floor/clamp injection を構成上含み、False では dataset を作成しない |

  - `refinement_autopilot: dict`（設計 doc §18.1--18.2 stage-1 の front tracker / decision engine。`mode="shadow"` は判断をログへ出すだけで run state を変更せず、`mode="arm_exit"` は commit checkpoint と decision record を残して run を停止する）

    | Key | Type | Default | Validation / behavior |
    |-----|------|---------|-----------------------|
    | `Numerics.diagnostics.refinement_autopilot.enabled` | bool | `False` | True かつ `refinement_estimator` の cadence 評価 step のみ `[refine_trk]` / `[refine_apilot]` を出力し、`mode` で stage-1 の動作を選ぶ |
    | `Numerics.diagnostics.refinement_autopilot.mode` | str | `"shadow"` | exactly `"shadow"` or `"arm_exit"`。`shadow` は log-only、`arm_exit` は firing-window 接近時の checkpoint request、decision record、clean stop を有効化する |
    | `Numerics.diagnostics.refinement_autopilot.ckpt_lead_h` | float | `8.0` | `arm_exit` が checkpoint request を開始する lead [parent cells]：`d_clear_h <= window_hi_h + ckpt_lead_h`。finite and `> 0` |
    | `Numerics.diagnostics.refinement_autopilot.e_on` | float | `0.88` | median column peak E の SEARCH→TRACKED on threshold。finite and in `(0, 1)` |
    | `Numerics.diagnostics.refinement_autopilot.e_off` | float | `0.60` | TRACKED→SEARCH off threshold。finite and in `(0, 1)`、`e_off < e_on` |
    | `Numerics.diagnostics.refinement_autopilot.assoc_cut` | float | `0.30` | stitched column の component membership cut。finite and in `(0, 1)`、`assoc_cut <= strong_cut` |
    | `Numerics.diagnostics.refinement_autopilot.strong_cut` | float | `0.60` | innermost component の peak E 要件。finite and in `(0, 1)` |
    | `Numerics.diagnostics.refinement_autopilot.gap_bridge` | int | `5` | block rim を跨ぐ component gap tolerance [cells]。`>= 0` |
    | `Numerics.diagnostics.refinement_autopilot.persist` | int | `3` | state promotion/demotion に必要な連続 observation 数。`>= 1` |
    | `Numerics.diagnostics.refinement_autopilot.n_q_plan` | float | `3.3` | planning AV-layer width [cells]。finite and `> 0` |
    | `Numerics.diagnostics.refinement_autopilot.chi_design` | float | `0.018` | design birth-quality target。finite and in `(0, 1)` |
    | `Numerics.diagnostics.refinement_autopilot.s_rep_cm` | float | `16.1e-4` | replacement interface radius [cm]。finite、`s_rep_cm > handoff_cm > 0` |
    | `Numerics.diagnostics.refinement_autopilot.handoff_cm` | float | `4.5e-4` | 2D→1D handoff radius [cm]。finite、`s_rep_cm > handoff_cm > 0` |
    | `Numerics.diagnostics.refinement_autopilot.window_lo_h` | float | `12.0` | preferred firing window lower bound [parent cells]。finite、`window_hi_h >= window_lo_h > 0` |
    | `Numerics.diagnostics.refinement_autopilot.window_hi_h` | float | `24.0` | preferred firing window upper bound [parent cells]。finite、`window_hi_h >= window_lo_h > 0` |
    | `Numerics.diagnostics.refinement_autopilot.history` | int | `9` | accepted front-observation ring-buffer length。`>= 3` |
    | `Numerics.diagnostics.refinement_autopilot.cov_min` | float | `0.5` | minimum tracked-column fraction。finite and in `(0, 1]` |
  ...
  - `dt_breakdown_history_enabled: bool`（既定 True；True の場合、history HDF5 に `/diagnostics/dt_breakdown_history/`, `/diagnostics/cfl_winner/`, `/diagnostics/per_row_mass/`, `/diagnostics/av_max/` を追加する。診断専用で、CFL・AV・境界条件・状態更新の数値経路は変更しない。`Output.history_every=0` でも本フラグが True なら history HDF5 は診断出力のため作成される。1D では dt 内訳の群（群ごとに `cycle`・`t_s` の軸を持つ）だけを毎ステップ追記し、`t`・`cycle`・`dt` とその他の系列の行は `Output.history_every`／`history_every_s` の間隔で書く（2026-09-23 以前は本フラグが全系列の行を毎ステップ書かせていたため、毎ステップの行が要る 1D デッキは `history_every=1` を明示する）。2D では従来どおり本フラグが全系列の行を毎ステップ書かせる）
  ...
  - `mesh_quality_min: dict`（既定 `{"enabled": False}`；True の場合のみ accepted committed post-ALE mesh の all-run achieved minimum diagnostics を計算し、history HDF5 に `/diagnostics/mesh_quality_min/v1/` を additive/backward-compatible group として出力する。The additive field set includes corner/Gauss/RZ-volume minima, negative RZ-volume count, and mesh-conditioning edge-length/altitude/condition-number diagnostics. Multiblock evaluation uses CSR cell-node lookup and `cell_orientation_sign` for committed RZ-volume sign. General-polygonal meshes (`corner_stride != 4`: ReALE v2, pentagon-belt, native-pentagon polar tier) are evaluated on the same CSR walk with per-cell active nverts using the admissibility gate's polygon primitives: corner-J / RZ-volume / edge-length minima and the negative-RZ-volume count are populated, while the Gauss-point / altitude / condition-number diagnostics are quad-only and remain at their datasets' identity values (min-diagnostics +inf, condition 0) on decks that never observe them. False では per-step device-to-host mesh copy と host loop を実行せず、group も作成しない）
  - `shock_approach: dict` (default `{"enabled": False, "every": 50, "target_radius_cm": 0.0, "bins": 192, "h_cell_cm": 0.0, "sectors": 0, "modal_l_max": 4, "sector_confidence_nu": 2.75, "sector_guard_crossings": 9.0}`; read-only shock-ahead diagnostic. When `enabled=True`, every `every` steps it theta-averages cell pressure radially to detect the `|dp/ds|` ridge and writes the fitted inward speed, predicted arrival time at `target_radius_cm`, and crossing-time lead to the `[shock_approach]` log. It changes neither the physical state nor the HDF5 schema. Valid range: `every>=1`, `bins>=16`, finite `h_cell_cm>=0`, and, when enabled, finite `target_radius_cm>0`. `h_cell_cm=0` uses `s_max/bins`. The per-sector extension follows `docs/design/asym_runtime_ale_controller_20260721.md` §2.)
    - `sectors: int` (default `0`; valid values are `0` or even integers `>=4`; `0` preserves the legacy theta-averaged-only path, while positive values enable that many equal-theta sector trackers.)
    - `modal_l_max: int` (default `4`; valid range `[0,4]`; maximum Legendre degree for the solid-angle-and-confidence-weighted fit of `ln(s_f)`.)
    - `sector_confidence_nu: float` (default `2.75`; valid range `>0`; confidence multiplier in `t_arr - nu*sigma_t`.)
    - `sector_guard_crossings: float` (default `9.0`; valid range `>=0`; number of local cell-crossing times subtracted from each sector arrival bound.)
  - `mesh_attribution: dict`
    ...
    - `enable_leave_one_out_replay: bool`（既定 False；現状では API stub のみで shadow replay は未実装）
  - `mesh_degeneracy_forensics: dict`
    - `enabled: bool`（既定 False；repeated pre-commit `mesh_quality_*` / `in_hydro_*` rejection の JSONL forensic dump を有効化する master flag。False では sample capture と JSONL write は行わず、既存 path は bitwise default-off）
    - `corner_j_source_budget_enabled: bool`（既定 False；True の場合のみ corner-J source budget を forensic JSONL に追加する。pressure / scalar Q / anti-hourglass / predictor / corrector / ALE rezone / remap の線形化 \(\Delta J_q\) attribution を出力する。False では追加 snapshot と budget 計算を行わない）
    - `corner_j_source_budget_include_1_ring: bool`（既定 False；予約フラグ。True は対象 cell の 1-ring も budget 対象にする意図を frozen config に保存する。現実装の JSONL emission は既存 forensic trigger cell を対象にする）
    - `velocity_history_enabled: bool`（既定 False；multi-step velocity history JSONL を有効化する master flag。False では追加 sample capture と JSONL write は行わず、既存 path は bitwise default-off）
  ...
  - `production_audit: dict`（production-audit umbrella; commits cc62bad5, f56f8612, 547d894c, f08646b8）
    - `enabled: bool`（既定 `False`；all production-audit infrastructure の master gate）
...
snapshot `hydro/burn_{rate,Q_e,Q_i,eps_cum,n_D,n_T,n_He3,n_He4,n_p}`（1D は `hydro/burn_neutron_cum` も：セルで生まれた中性子の累積数、DD・DT の中性子分枝の和。中性子の発生位置の分布で、`burn_eps_cum` は荷電生成物の沈着先の分布。2026-09-26 additive。checkpoint にも入り再開で引き継ぐ。無い旧 checkpoint からの再開では 0 から数える）、
...
burn 有効 checkpoint 必須（`hydro/burn_n_*` 欠損は起動時 `runtime_error`）。checkpoint は state が持つ比インベントリ \(Y_s\) [1/g] をそのまま `burn_state/specific_inventory`（cell-major の \([n_{cells}\times5]\)、D・T・He3・He4・p の順）にも保存し、再開はこれを優先する（2026-09-26 additive。`hydro/burn_n_*` は数密度 \(Y_s\rho\) で、\(n_s/\rho\) からの再構成は \(Y_s\) と最下位ビットで異なり、再開した run の燃焼の集計が元の run と一致しなかった。無い旧 checkpoint は従来どおり \(n_s/\rho\)）。
...
| ファイル | 命名規則 | 出力頻度 | 用途 |
|---------|---------|---------|------|
| スナップショット | `results/<case>_NNNN.h5` | `plot_every` ステップ毎 または `plot_every_s` 秒毎（OR論理） | 場の可視化・後処理 |
| 時系列 | `results/<case>_history.h5` | `history_every` ステップ毎 または `history_every_s` 秒毎（OR論理、追記）。`/diagnostics/dt_breakdown_history/` などの dt 内訳の群は毎ステップ（§6.4 `dt_breakdown_history_enabled`） | スカラー診断量の時間変化 |
| チェックポイント | `checkpoints/<case>_ckpt_NNNN.h5` | `checkpoint_every` ステップ毎 または `checkpoint_every_s` 秒毎（OR論理） | リスタート（1 ファイル） |

- `NNNN`：出力の種類（スナップショット・チェックポイント）ごとの 4 桁ゼロ詰めの通し番号（0 から数え、出力先に既存のファイルがあれば、再開かどうかによらずその最大番号の次から続ける）。サイクル番号はルート属性 `cycle`、物理時刻はルート属性 `t` に格納する
- **並列 IO 方式**：MPI 実行でも、スナップショット・時系列・チェックポイントはすべて rank 0 だけが 1 つのファイルに書く（MPI-IO は使わない）。1D の MPI 実行では、書く前に各 rank の担当区間を rank 0 に集める。旧形式のランク別チェックポイント（`<case>_ckpt_NNNNNN_rNNNN.h5`）は書かないが、リスタートでは読み込める（§6.4.1 `restart_from`）
- 大きい dataset の圧縮とチャンク分割は ARCHITECTURE §4.9 の `IO::HDF5Writer` を参照
- `<case>`：`Main.name` を優先。未指定（`name` 省略）の場合は namelist ファイル名（拡張子なし）をフォールバックとして使用。全ファイルは `Output.directory` 下の `results/`（スナップショット・時系列）と `checkpoints/` に格納し、`run_info.json` は `Output.directory` 直下に置く
- チェックポイントは `Output.checkpoint_keep_last` 個を保持し、古いものを自動削除

### 7.2 スナップショットHDF5レイアウト（`<case>_NNNN.h5`）
...
│   ├── e_cold                      float64[N_cell]       力学エネルギー [erg/g]（`T_start_inactive_cells="cold_equilibrium"` のときのみ。低温平衡構成則の \(C(v)\)。このモードでは `hydro/ee` は電子熱量座標 \(q_e\) で、物理的な電子エネルギーは `ee + e_cold`）
...
│   └── per_material/v1/            optional。`Numerics.materials.per_material_conservation_enabled=True` の時だけ作成し、disabled 時は group 全体を省略する
...
│       ├── mass                    float64[N_cell, N_mat] 材料別の質量 [g]
│       ├── Ee                      float64[N_cell, N_mat] 材料別の電子内部エネルギー [erg]
│       ├── Ei                      float64[N_cell, N_mat] 材料別のイオン内部エネルギー [erg]
│       └── rho_derived, ee_derived, ei_derived, Te_derived, Ti_derived, Pe_derived
│                                   float64[N_cell, N_mat] optional。`Numerics.materials.hdf5_emit_derived_per_material=True` の時だけ。材料別の密度 [g/cm³]・比内部エネルギー [erg/g]・温度 [eV]・電子圧力 [dyne/cm²] の導出値（出力専用）
│
├── diagnostics/
...
```
/
│  （停止理由は history には書かず、`run_info.json` の `termination_reason` に記録する — §6.4.1 停止条件参照）
...
│   ├── radiation_escaped           float64[N_step]       放射境界流出累積 E_rad_escaped [erg]（E_escape[G]の全群合算。全 rank 合算）
│   ├── radiation_escaped_step      float64[N_step]       その step の放射境界流出 [erg]（2026-08-30 追加）
│   ├── marshak_in                  float64[N_step]       Marshak境界入射累積 E_Marshak_in [erg]（§8.2）
│   ├── marshak_in_step             float64[N_step]       その step の Marshak 境界入射 [erg]
│   ├── pdv_boundary                float64[N_step]       境界PdV仕事累積 E_pdV_bdry [erg]（§10.2）
│   ├── pdv_boundary_step           float64[N_step]       その step の境界 PdV 仕事 [erg]
...
│   ├── floor_injected_step         float64[N_step]       その step のフロア補正注入 [erg]
│   ├── safety_injected             float64[N_step]       安全補正注入累積 E_safety [erg]（診断専用、保存誤差の分子に含めない）
│   ├── safety_injected_step        float64[N_step]       その step の安全補正注入 [erg]
...
│   ├── solver_residual_step        float64[N_step]       その step の Hypre 残差エネルギー [erg]
│   ├── conservation_error          float64[N_step]       相対保存誤差 ε_budget（NUMERICS §10.2 恒等式に基づく）
│   ├── E_volume_in                 float64[N_step]       外部輻射体積源（`Radiation.volume_source_rate`）の入力累積（内部診断）[erg]
│   ├── E_volume_in_step            float64[N_step]       その step の外部輻射体積源の入力 [erg]
...
│   ├── ghost_corona_width          float64[N_step]       1D のゴーストコロナ（`Laser.lasermesh.ghost_corona`）の幅 [cm]（無効・非該当の step は 0；2026-09-23 additive）
...
│
├── radiation/
│   ├── fld_outer_iterations        int64[N_step]         FLD outer (Picard) iterations used this step [count]（FLD 無効時 0；2026-07-16 additive）。thermal subcycle の step は採用した試行の全 FLD 解の合計（2026-09-23）
│   ├── fld_outer_residual          float64[N_step]       FLD outer residual at exit [dimensionless]。step 内に FLD 解が複数あるときは最大値（NaN があれば NaN）
│   ├── fld_outer_converged         int64[N_step]         1 = outer_tol reached, 0 = hit max_outer_iterations。step 内に FLD 解が複数あるときは全解が収束したときだけ 1
│   ├── sn_outer_iterations         int64[N_step]         S_N の外側反復（線形不連続法は節点温度の Newton、他は物質の Picard）の step 内合計 [count]（`mode="sn_transport"` の run だけに出力；2026-09-25 additive）
...
補足（実装互換）：
- 履歴ファイルは書き出しのたびに（既定で 64 行か 30 秒ごと）その場で開いて追記し、閉じる（2026-09-23 から。以前は毎回ファイル全体を複製して追記し、改名していた）。書き出しの合間はファイルは完結しており、実行中に別プロセスで読める。書き出し中に読むと不完全な状態を見ることがあるので、読み手は開けなければ開き直す（書き手は HDF5 のファイルロックを使わないので、読み手が開いたままでも実行は止まらない）。書き出し中に強制終了されて開けなくなったファイルは、次の書き出しで `<name>.unreadable-<n>` へ移して新しい履歴ファイルを始める。
...
│
├── mesh/carrier/v1/               `Numerics.ale.mesh_mode="reale_v2"` のキャリア状態（fixed mode ではグループ全体を省略）
│   │  attrs: valid (int8 bool), domain_active (int8 bool), n_masters (int64)
│   ├── master_stable_id           uint64[n_masters]  capture 時に凍結された master stable ID
│   ├── master_mesh_node           int32[n_masters]   保存 node ordering 上の現 generation node index
│   ├── master_bc_class            uint8[n_masters]   0=outer, 1=axis
│   ├── node_class                 uint8[N_nodes]     0=interior, 1=master, 2=slave
│   ├── node_edge                  int32[N_nodes]     slave の carrier edge（non-slave は -1）
│   └── node_lambda                float64[N_nodes]   slave の snap parameter [0,1]（non-slave は 0）
│
├── metadata/
│   ├── axis_core_released_units    int32 scalar    Demand-driven axis-core ring release の累積構造単位数。欠損（旧 checkpoint）は 0 として警告なしで扱う。0 は無処理。N>0 は初回 axis-core runtime build 前に復元し、`TENRYU_I1B_RING_RELEASE` と `Numerics.ale.euler_window.axis_core_ring_release_enabled` の両方が無効な restart は fail-closed；`TENRYU_I1B_RESTART_FORENSIC=1` のみ loud warning 付きでダウングレード可能
│   ├── merge_tombstone            uint8[N_cells]  Optional 1D persistent row-merge tombstone mask. Present only when at least one entry is nonzero; missing means an empty mask, and the restart loader logs the count when nonzero entries are loaded.
│   └── epoch_swap/v1/             optional additive audit group written by `checkpoint-swap-center`; presence enforces the post-swap BBSW guard
│       ├── checkpoint_a_sha256, segment2_frozen_config_sha256, tool_version
│       ├── cut_ring, kept_cells_C, kept_nodes_N, r_cut_cm, n_c, n_b
│       ├── delta_mass_g, delta_energy_erg, delta_momentum_{r,z}_g_cm_s
│       ├── mass_tolerance_g, energy_tolerance_erg, momentum_{r,z}_tolerance_g_cm_s
│       ├── *_threshold, mirror_tolerance, legendre_tolerance, ledger_kappa
│       ├── measured_max_{speed_cm_s,mach,rho_relative,temperature_relative,mirror_relative,legendre_relative}
│       ├── mirror_z_floor_cm, mirror_v_floor_cm_s, conservation_v_floor_cm_s, momentum_floor_g_cm_s, fresh_v_prenorm_max_cm_s
│       ├── axis_core_released_units_a
│       └── transition/<section>.<key>  differing sanctioned config values as "<parent> -> <segment2>"
│
...
│
├── evacuated_cells/                 排気セル方策+接触状態（グループ不在の旧checkpointは空状態=全再構築で復元）
│   │  スロット毎データセット（n_slots 長）: cell, axis, state, node_a0/b0/a1/b1,
│   │  normal_pair_r0/z0/r1/z1（旧checkpointの slot 単一 normal_r/z は両対へ複製）,
│   │  gap_pair0/1, gap_prev_pair0/1, pair_engaged0/1,
│   │  gap_at_engagement_pair0/1 [cm]（不在時: engaged 対は g0、非 engaged 対は 0 — 侵食不変量の legacy 床）,
│   │  tensile_streak_pair0/1, gap, gap_prev, mass_at_engagement, g0, g_arm, h_perp_ref,
│   │  reproject/release/re-engage カウンタ、熱台帳累積。付随: controller キャッシュ
│   │  （dwell/評価/体積/前回gap）, x_r_ref/x_z_ref, closure_lineage, contact_active_mask。
│   │  デバイス側バッファ（対リスト・active cell リスト）は読み込み後に再構築。
│
└── time_state/                      時間管理・累積診断状態
    ...
    ├── dt_growth_ref            float64     成長制限の参照 dt [s]（事象解決 cap（排気セル接触 cap）を除いた前ステップ dt。
    │                                        旧checkpointで不在の場合は dt として復元 = legacy 挙動。NUMERICS §3.3.12）
    ...
    ├── E_volume_in              float64     外部輻射体積源の入力エネルギー累積値 [erg]（2026-09-23 追加。旧 checkpoint では 0 から）
...
`metadata/epoch_swap/v1` は W-CTR-d の監査証跡である。sanctioned transition set は、A-read で parent 値へ戻す center topology keys、default 値へ戻す全 `Numerics.ale.central_pseudo_core_*` keys、および A の frozen config から継承する `Main.t_end`、Output の plot/history/checkpoint の step/time cadence 6 keys、`Numerics.diagnostics_every` に限る（`Main.name`、`Output.directory` その他は strict）。parent と segment-2 で異なる各 sanctioned key は string dataset `transition/<section>.<key>` に `"<parent> -> <segment2>"` として記録する。同 group を持つ checkpoint は §15 contract により pole-axis BBSW が環境変数優先規則で enabled と解決されれば restart 時に fatal とし、group 欠損の legacy checkpoint または BBSW disabled は受理する。swap の transfer、refusal gate、ledger continuity の規約は `docs/design/epoch_quad_remesh_20260820.md` §§12, 15 を権威とする。

`mesh/carrier/v1` は `mesh_mode="reale_v2"` の checkpoint では generation 0
（`valid=0`, `n_masters=0`）を含めて常に出力し、reader は master node 範囲、
node 配列長、class/boundary-class enum、lambda 範囲、および valid 時の
`n_masters>=3` を検証して復元する。`carrier_ring_pages` と warm
Delaunay cache は保存せず、restart 後は次の rezone install で再構築されるまで
invalid/cold のままとする。

post-rezone `reale_v2` checkpoint の現 generation node 数は root attribute
`n_nodes` を authority とし、`/mesh/x_r` の全要素数が一致しなければ fail loud
する。`/mesh/x_r`, `/mesh/x_z`, `/mesh/v_r`, `/mesh/v_z`,
`mesh/ale_reference/v1/*`, および `mesh/carrier/v1/node_*` はこの進化後 node
数で保存・復元する。一方、`mesh/topology/v3` の block/seam 表は CONFIG から
構築された initial-shaped multiblock metadata のまま round-trip し、block の
owned-node extent は initial node 数に対して検証する。進化後 node 数を使うのは
cell-node CSR index の範囲、`MeshTopology::n_nodes`、および node-sized dataset
extent である。reader は復元した cell-node CSR と座標から、ReALE install と同じ
`mesh::rebuild_runtime_topology_caches` を呼び、`node_flags`、face adjacency、
unique/boundary face payload、reverse node-to-cell CSR と device mirror を再構築する。
構造化 polar-tier runtime cache は無効化し、`general_polygonal=true` とする。

`mesh_mode="reale_v2"` で同 group が欠落する旧 checkpoint は carrier identity を
幾何から一意に再構築できないため、`ConfigError` （`checkpoint predates
carrier IO; restart of a carrier run from this file is not representable — rerun
from the deck or restart a gen-0 checkpoint`）で fail loud する。boundary 再抽出への
fallback/migration は行わない。fixed mode では group 欠落を従来通り無視する。
...
- mesh topology defaults: topology_scheme=`"single_block"`; pentagon_belt_layers=`[]`; multiblock_cart_core_r_c=`s_max/12`; multiblock_cart_core_r_match=`2*r_c` for the cart-core family and `s_max/6` for `multiblock_polar_tier`; multiblock_cart_core_n_c=`nz/4` with `nz=4*n_c` required for the cart-core family; multiblock_cart_core_bridge_layers=`max(4,n_c/8)`; multiblock_cart_core_bridge_grading=`"uniform"`; multiblock_theta_cap_widen_factor=`1.0`; multiblock_transition_scheme=`"hermite_bridge"`; multiblock_cap_p=`6.0` (dimensionless rounded/squircle cap exponent); multiblock_bridge_elliptic_sweeps=`0`; multiblock_bridge_elliptic_omega=`0.5`; multiblock_outer_svec_tangent_balance=`true`; polar_tier_chi_lo=`0.75`; polar_tier_chi_hi=`1.50`; polar_tier_fan_sectors=`12`; polar_tier_min_tier_columns=`12`; polar_tier_fan_first_ring_radius_cm=`0.0`; polar_tier_hydro_enabled=`False`; polar_tier_belt_thickness_frac=`0.0`; polar_tier_belt_rows=`1`; polar_tier_dendrite_enabled=`False`; polar_tier_dendrite_s_theta_rows_below=`5`; polar_tier_pole_cap_m=`0`; polar_tier_pole_cap_alpha=`0.0`; polar_tier_native_pentagon=`False`; polar_tier_cart_cut_ring=`-1`; polar_tier_center_kind=`"cart_box"`（the dendrite/pole-cap/native-pentagon/cart-cut/cap-center opt-ins are frozen-config elided at their defaults, preserving legacy byte identity）. For `multiblock_polar_tier_cart_center` the frozen config emits the polar-tier keys plus `polar_tier_cart_cut_ring`, `multiblock_cart_core_r_c`, `multiblock_cart_core_n_c`, and `multiblock_cart_core_bridge_layers`; non-default `polar_tier_center_kind` is also emitted. `pentagon_belt_layers` is frozen only for `pentagon_belt_shell`; this scheme does not derive or freeze `multiblock_cart_core_*` defaults. Cart-core-only fields are unset/frozen-config elided for `multiblock_polar_tier`; the shared `multiblock_cart_core_r_match` seam key and the eight `polar_tier_*` values are emitted for that opt-in topology. The tri-fan cap and polar-tier topologies are default-off and opt-in only; old implicit `single_block` frozen configs still elide `topology_scheme` and all polar-tier keys. S5 transition fields, `multiblock_cap_p`, `multiblock_theta_cap_widen_factor`, and `multiblock_outer_svec_tangent_balance` are frozen-config elided whenever they equal defaults and are unused, preserving legacy frozen-config byte identity.
...
- cone_shell C4 builder-derived storage defaults: `cone_shell_cavity_cells=0`, `cone_shell_exterior_cells=0`, `cone_shell_tip_fill_layers=0`（いずれも deck key ではなく frozen config には出力しない。builder がそれぞれ `[6,32]`, `[6,32]`, `[6,48]` の topology count へ派生）。
	- rezoning / ale：enabled=False（2D_RZ ALE 用）, reale_short_edge_collapse_rel=3.0e-2, reale_subdomain_rezone=False, reale_subdomain_frac_max=0.6, reale_overlay_additivity_tol=1.0e-4, ale_identity_mode=False, ale_mover_diag=False, ale_preserve_lagrangian_velocity_carry=False, every_n_steps=5, force_rezone_every_n_steps=0, warmup_steps=0, relaxation=0.2, spacing_ratio_threshold=1.5, quality_threshold=0.2, max_iterations=20, max_displacement_fraction=0.5, remap_limiter=`"van_leer"`, remap_ms_midpoint=False, remap_ms_post_check=False, remap_ms_post_max_iter=3, remap_ms_rescale_floor=0.01, ke_fixup=True, ke_conservation_closure=False, ke_conservation_closure_audit=False, ke_closure_redistribute_floor=False, debug_per_remap_log=False, shock_sensor_guard_cells=2, density_jump_threshold=0.1, Te_jump_threshold=0.2, preventive_axis_guard_fraction=0.1, axis_z_motion=`"fixed"`, winslow_axis_kappa=0.7, button_morph={enabled=False, t_start_s=0.0, t_end_s=0.0, max_step_fraction=0.05, every_n_steps=1}, reference_barrier_enabled=False, reference_target=`"none"`, reference_blend_default=1.0, reference_volume_floor_rel=1e-8, reference_corner_j_floor_rel=1e-8, reference_gauss_j_floor_rel=1e-8, reference_linesearch_max_iters=24, reference_force_engage_every_step=False, reference_trigger_axis_margin_enabled=True, reference_trigger_axis_margin_threshold=1e-2, reference_trigger_corner_j_ratio_enabled=True, reference_trigger_corner_j_ratio_threshold=0.5, driver_retry_reference_barrier_enabled=False, driver_retry_reference_barrier_K_axis=4, driver_retry_reference_barrier_eta_axis=0.05, driver_retry_reference_barrier_max_attempts=6, driver_retry_reference_barrier_same_sig_max=3, driver_retry_reference_barrier_cell_window=2, driver_retry_reference_barrier_dt_collapse_rel=1e-3, driver_retry_reference_barrier_lambda_collapse_threshold=1e-3, driver_retry_reference_barrier_lambda_collapse_count=2, driver_retry_reference_barrier_quality_progress_factor=1.25, driver_retry_reference_barrier_quality_progress_count=2, driver_retry_reference_barrier_rezone_freq_warn_fraction=0.20, driver_retry_reference_barrier_rezone_freq_window=200, driver_retry_reference_barrier_chi=0.8, driver_retry_reference_barrier_q_retry=0.5, remap_damage_gate_enabled=False, remap_damage_dmax=0.05, remap_damage_axis_eta=0.02, remap_damage_axis_budget_enabled=False, remap_damage_axis_budget_factor=2.0, predictive_acceptance_enabled=False, predictive_acceptance_axis_floor_fraction=0.0, predictive_acceptance_cell_vol_floor_fraction=0.0, safe_backtrack_enabled=False, safe_backtrack_min_exp=20, safe_backtrack_binary_iters=8, mesh_epoch_enabled=False, mesh_epoch_max_per_step=16, euler_window.enabled=False, euler_window.shape=`"rectangle"`, euler_window.transition_width=0.0, band_ale={enabled=False, aspect_trigger=0.25, release_hysteresis=1.5, chi=0.5, respace_move_cap_frac=0.5, estimator_band_hold_mach=0.30, bands=`"belts_axis"`, belt_target=`"ring_mean"`, center_target=`"line"`, compose_with_rezone=False, axis_target=`"z_laplacian"`, axis_segment_halfwidth=4, axis_shell_block_enabled=False, sigma_linesearch_enabled=True, transaction_energy_closure_enabled=False, estimator_band_cut=0.5, estimator_band_shock_hold=0.90, estimator_band_front_hold_margin_rows=16.0, estimator_band_axis=`"auto"`, estimator_band_in_rows=4, estimator_band_out_rows=4, estimator_band_eta_on=1.5, estimator_band_eta_off=1.15, estimator_band_per_column=False, estimator_band_pc_filter_halfwidth=3, estimator_band_pc_slope_limit=0.35, estimator_band_pc_slope_reject=0.50, estimator_band_pc_curvature_limit=0.50, estimator_band_pc_chi_max=0.25, estimator_band_pc_chi_step=0.10, estimator_band_pc_sigma_floor=0.25, estimator_band_pc_coverage_full=0.80, estimator_band_pc_coverage_min=0.50, estimator_band_pc_cooldown_events=2, estimator_band_pc_phase_b=False, estimator_band_pc_tube_dilate_rows=5, estimator_band_pc_tube_dilate_cols_extra=2, estimator_band_pc_ambiguous_hold_fraction=0.20, closure_catchment_enabled=False, closure_catchment_forced_active=False, closure_catchment_s_catch_cm=6.0e-4, closure_catchment_s_protect_cm=1.0e-3, closure_catchment_spacing_floor_cm=3.0e-6, closure_catchment_ratio_max=1.05, closure_catchment_nu_max=0.25, closure_catchment_max_bites=1, closure_catchment_shock_hold=0.30, closure_catchment_eta_h_arm=0.80, closure_catchment_eta_h_full=0.65, closure_catchment_eta_m_arm=0.75, closure_catchment_eta_m_full=0.60, closure_catchment_reset_eta=0.90, closure_catchment_support_core_rows=4, closure_catchment_support_taper_rows=2, closure_catchment_accum_frac=0.02, closure_catchment_rearm_drop=0.10, pole_theta_enabled=False, pole_theta_routine_enabled=False, pole_theta_phys_lp=0, pole_theta_phys_lc=4, pole_theta_noise_floor=1.0e-5, pole_theta_noise_ceiling=1.0e-3, pole_theta_h_arm=0.70, pole_theta_h_fire=0.55, pole_theta_h_hard=0.35, pole_theta_h_release=0.80, pole_theta_kappa_arm=2.5, pole_theta_kappa_fire=3.5, pole_theta_kappa_hard=6.0, pole_theta_alpha=0.30, pole_theta_alpha_hard=0.60, pole_theta_deadband_frac=0.05, pole_theta_move_limit_frac=0.35, pole_theta_move_limit_hard_frac=0.75, pole_theta_cooldown_s=1.0e-12, pole_theta_cooldown_base_s=1.0e-12, pole_theta_predict_window_s=2.0e-12, pole_theta_predict_horizon_s=5.0e-12, pole_theta_halo_columns=2, pole_theta_halo_rows=2, pole_theta_post_h_floor=0.65, pole_theta_curve_preserving=False, pole_theta_protected_modes=`""`, pole_theta_fit_order=12, pole_theta_shock_hold=0.30, shell_window_in_rows=8, shell_window_out_rows=4, shell_boundary_guard_rows=2, shell_min_spacing_frac=0.5, shell_front_metric=`"grad_rho"`, shell_target=`"respace"`, axis_repair_enabled=False, axis_repair_eta_on=0.85, axis_repair_eta_off=0.95, axis_repair_cap_rel=0.05; frozen-config emitted only when enabled}, evacuated_cell={enabled=False, every_n_steps=50, arm_mass_fraction=1.0e-6, off_mass_fraction=1.0e-8, rho_vacuum_policy_g_per_cc=1.0e-10, off_hold_evaluations=2, laser_ne_over_ncrit_max=1.0e-3, laser_wavelength_nm=351.0, coupling_fraction_max=1.0e-8, max_cells_per_event=4, rematerialize_enabled=True, rematerialize_after_evaluations=10, rematerialize_volume_fraction=0.05, rematerialize_neighbor_change_max=5.0e-2, rematerialize_dwell_evaluations=5, closure_contact={enabled=True, gap_floor_fraction=0.02, gap_arm_fraction=0.04, live_mass_gate=0.05, live_volume_gate=0.02, refill_min_mass_fraction=0.05, refill_min_density_ratio=0.2, release_force_c=1.0e-3, release_persistence_stages=2, reengage_gap_margin=0.01}; frozen-config always emitted}, rezone_solver=`"legacy_winslow"`, m1_gamma_align=0.0, m1_lambda_tether=0.0, m1_theta_reg=0.0, m1_sweeps=8, m1_min_j_dec_rel=0.0, m1_barrier_beta=1e-3, rezone_local_admissibility_linesearch=False, rezone_local_j_floor_rel=1e-8, rezone_local_linesearch_max_halves=8, reject_zero_gauss_j=False, zero_gauss_j_floor_rel=1e-8, lambda_sweep_diagnostic_enabled=False, lambda_sweep_target_cell_c=-1, lambda_sweep_target_cell_i=-1, lambda_sweep_target_cell_j=-1, lambda_sweep_max_exp=20, corner_jacobian_post_tangle_enabled=True, corner_post_tangle_strict_floor_enabled=False, local_boundary_repair_enabled=False, multi_node_boundary_repair_enabled=False, multi_node_interior_repair_enabled=False, axis_variational_projection_enabled=False, multiblock_cross_seam_rezone_enabled=False, multiblock_scaled_reference_enabled=False, multiblock_differential_reference_enabled=False, multiblock_differential_reference_band_count=64, multiblock_differential_reference_smoothing_g0=0.03, multiblock_differential_reference_nu=0.10, multiblock_differential_reference_eps_v=0.03, multiblock_differential_reference_s_cap_min_rel=1e-3, multiblock_differential_reference_xi_seam_tol=1e-9, multiblock_differential_reference_sigma_warn_floor=0.5, multiblock_lagrangian_bulk_center_patch_reference_enabled=False, multiblock_center_patch_ring_max=4, multiblock_center_patch_xi_center=0.0, multiblock_center_patch_halo_layers=2, multiblock_center_patch_vol_on=0.05, multiblock_center_patch_vol_off=0.10, multiblock_center_patch_cornerj_on=0.03, multiblock_center_patch_cornerj_off=0.08, multiblock_center_patch_gaussj_on=0.03, multiblock_center_patch_gaussj_off=0.08, multiblock_path_admissibility_enabled=False, path_admissibility_floor=0.01, dt_rejection_factor=0.5, max_dt_rejections=8, axis_band_managed_remap_enabled=False, axis_band_managed_remap_width=3, axis_band_managed_remap_max_width=6, axis_band_managed_remap_every_hydro_half_step=True, axis_band_managed_remap_margin_trigger=1e-4, axis_band_managed_remap_equal_volume=True, axis_band_managed_remap_include_radiation_groups=True, axis_rezone_enabled=False, axis_rezone_trigger_edge_fraction=0.1, axis_rezone_trigger_min_altitude_fraction=0.1, axis_rezone_eta_floor=1e-2, emergency_cell_deactivation_enabled=False, axis_repair_mode=`"full_winslow"`, remap_scheme=`"legacy_split"`, remap_ms2_limiter=`"van_leer"`, swept_volume_sign_fixed=True, convergence_tol=1e-6; the six M1 keys are emitted by frozen-config serialization only when `rezone_solver="m1_tmop"`; legacy behavior requires `Numerics.profile.legacy_regression@2026-07-27` or an explicit False.
		...
		- Mortar position-drift default: `Numerics.ale.evacuated_cell.closure_contact.mortar_position_drift_beta=0.02` (valid `[0,0.1]`; `0` disables the correction).
		- Surface-engagement default: `Numerics.ale.evacuated_cell.closure_contact.surface_engage_enabled=False`; surface-measured engagement (migration phase 4); default false.
		- LCP-application default: `Numerics.ale.evacuated_cell.closure_contact.lcp_apply_enabled=False`; migration phase 6 coupled contact application; requires `surface_engage_enabled=True`.
		- ReALE corner-mass reset default: `Numerics.ale.reale_corner_mass_reset=True`. Successful ReALE remaps re-anchor active corner masses to the compatible-force RZ subpolygon-volume fractions before carrier projection and zero padded slots. `False` preserves the previous transported corner masses byte-for-byte. The default is frozen-config elided and legacy completion supplies `True` (§6.4, NUMERICS §3.3.16).
		- ReALE remapped-velocity maximum-principle default: `Numerics.ale.reale_velocity_max_principle=False`. Before carrier projection, each new nodal velocity component is clamped to the 5%-relaxed old-node range within `5.0e-5` cm, with one widening to `1.0e-4` cm when fewer than three donors are found. `False` preserves the previous remapped velocities byte-for-byte. The default is frozen-config elided and legacy completion supplies `False` (§6.4, NUMERICS §3.3.17).
		- ReALE subdomain defaults: `Numerics.ale.reale_subdomain_rezone=False`, `Numerics.ale.reale_subdomain_frac_max=0.6`. The opt-in path preserves cells outside the filled disturbed components and falls back to full-domain rezone when face adjacency is unavailable or the dilated fraction exceeds the threshold. Both defaults are frozen-config elided and legacy completion supplies them (§6.4, NUMERICS §3.3.18).
		- ReALE mesh-mode default: `Numerics.ale.mesh_mode="fixed"`. The opt-in `"reale_v2"` pilot and its strict scope are specified in §6.4.7. The default is frozen-config elided, legacy completion supplies `"fixed"`, and no checkpoint or HDF5 schema changes are introduced.
		- ReALE tessellation-authority default: `Numerics.ale.reale_core="exact"`. The legacy builder is rejected with `ConfigError` under `mesh_mode="reale_v2"` and is inert under `mesh_mode="fixed"`. The default is frozen-config elided, legacy completion supplies `"exact"`, and the historical `TENRYU_REALE_CORE` environment variable is retired and ignored.
		- ReALE staged-dt production requirement: `Numerics.ale.rezone_min_dt_s=1.0e-14` s. The pre-install staged edge-AV/node-crossing gate rejects when `dt_staged <= rezone_min_dt_s`; `0.0` disables it and negative values are a `ConfigError`. This deck-owned requirement is not a derived weld tolerance. The default is frozen-config elided and legacy completion supplies `1.0e-14`.
		- ReALE tessellation GPU offload: `Numerics.ale.tess_gpu_dual=False`. Opt-in, bitwise-identical GPU circumcenter batch for the exact tessellator dual stage; OFF keeps the host-only path. The default is frozen-config elided and legacy completion supplies `False`.
		- ReALE tessellation GPU classify: `Numerics.ale.tess_gpu_restrict=False` (requires `tess_gpu_dual=True` when enabled). Opt-in, bitwise-identical GPU node classification for the restrict stage; OFF keeps the host-only path. The default is frozen-config elided and legacy completion supplies `False`.
		- DV-CLP solver revision default: `Numerics.ale.dvclp_solver_rev=0` (valid `{0, 1}`). `0` keeps the legacy Newton bit-path; `1` enables the pre-factorization QR dependent-row elimination (§6.4.7). The default is frozen-config elided and legacy completion supplies `0`.
		- ReALE rezone-economics knobs (2026-08-16, S1/S3): `Numerics.ale.reale_lloyd_max=4` (valid `[0, 64]`; Lloyd/CVT iteration ceiling per rezone, `0` = advection-only targets), `Numerics.ale.reale_dt_trigger_factor=0.5` (valid `(0, 1]`; dt-halving rezone-trigger factor), `Numerics.ale.reale_dt_trigger_cooldown=0` steps (valid `[0, 100000]`; minimum committed-rezone age before the dt trigger may fire). All three defaults reproduce the previous hard-coded behavior bit-for-bit, are frozen-config elided, and legacy completion supplies them (§6.4).
		- Axis-core replay defaults: `euler_window.axis_core_transaction_mode="static"`, `replay_table_path=""`, `replay_tau_lead=4.5e-12`, `replay_tau_splice=1.4e-12`, `replay_beta=1.0`. The replay keys are runtime-inert unless the mode is `"clearance_replay"`; default-valued replay tunables remain elided from normal frozen configs.
		- ALE runtime-controller W2/W3a/W3b/W3c defaults: `Numerics.ale.runtime_controller={"monitor_enabled":False,"monitor_every":1,"shell_rows":4,"controller_shell_rows":4,"cap_columns":2,"q_soft":0.42,"q_hard":0.22,"q_recover":0.60,"h_soft":8.0,"h_hard":4.0,"h_recover":12.0,"soft_persistence":2,"recover_checks":3,"winslow_sweeps":4,"winslow_omega":0.2,"beta_monitor_soft":0.12,"beta_monitor_hard":0.25,"beta_mass":0.12,"beta_front":0.12,"beta_theta":0.15,"g_max":1.22,"front_width_cells":2.5,"cap_fraction":0.05,"cap_normal_fraction":0.025,"controller_enabled":False,"commit_rollback_enabled":True,"activation_front_mode":"min","activation_front_margin_hs":2.0,"activation_time_s":-1.0,"cadence_soft":2,"cadence_hard":1,"cadence_recovery":4,"pre_step_enabled":True,"failures_hard_force":4,"failures_big_repair":8,"escalation_max_failures":12}`. Frozen-config serialization emits the complete dict; legacy default completion fills `controller_shell_rows`, `cap_columns`, the complete W3c/W7e knob set, and the rest of the complete default dict when absent.
		...
		- ALE core1d sub-model defaults: `central_pseudo_core_core1d_enabled=False`, `build_shells=48` `[4,4096]`, `split_append=0` (off) `[0,1024]`, `av_c1=0.5`, `av_c2=4.0`, `cfl=0.25` `(0,1)`, `piston_cap=10.0`, `max_substeps=20000`. Values are injected once into the core-header-free sub-model (`core1d::set_params`) at `ensure_built`; the historical `TENRYU_I1B_CORE_1D_*` envs override when SET. Frozen-config emits the block only when enabled or non-default. NUMERICS §13.
		- ALE absorption-schedule defaults: `central_pseudo_core_spherical_absorb_gasfront=False`, `central_pseudo_core_spherical_absorb_alpha=0.0` (off; otherwise `(0,1)`), `central_pseudo_core_spherical_absorb_pjump=0.0` (off; otherwise `>1`), `central_pseudo_core_mixed_absorb_enabled=False`, `central_pseudo_core_absorb_watch_rows=1` `[1,8]`. The historical `TENRYU_I1B_SPHERICAL_ABSORB_*`, `TENRYU_I1B_MIXED_ABSORB`, and `TENRYU_I1B_ABSORB_WATCH_ROWS` envs override when SET; frozen-config emits the block only when enabled or non-default.
		- ALE remap conservation gate defaults: `remap_mass_closure_reject_tol=0.0` (`>=0`, 0=off) and `rezone_closure_cooldown_steps=50` (`>=1`). Default remains off until the default-on regression sweep passes; the certified I1-B deck sets `remap_mass_closure_reject_tol=1e-8` explicitly. The historical `TENRYU_I1B_REMAP_CLOSURE_REJECT_TOL` and `TENRYU_I1B_REZONE_CLOSURE_COOLDOWN_STEPS` envs override when SET; frozen-config emits the block only when non-default.
		- ALE certified transport/robustness flag defaults: `csr_optionb_coherent_enabled=False`, `csr_optionb_velocity_remap_enabled=False`, `pole_axis_bbsw_enabled=False`, `axis_contact_guard_enabled=False`, `mass_floor_absorb_enabled=False`, `interior_patch_remap_enabled=False`. The historical `TENRYU_I1B_OPTIONB_COHERENT`, `TENRYU_I1B_OPTIONB_VELREMAP`, `TENRYU_I1B_POLE_AXIS_BBSW`, `TENRYU_I1B_AXIS_CONTACT_GUARD`, `TENRYU_I1B_MASS_FLOOR_ABSORB`, and `TENRYU_I1B_INTERIOR_PATCH_REMAP` envs override when SET; frozen-config emits the block only when enabled.
...
- Radiation.multigroup_diffusion diagnostic fields: `state_supply_boundary_policy="local_D_current"` (valid values: `"local_D_current"`, `"harmonic_ghost_D_test"`, `"radial_mean_D_test"`), `diagnostic_radial_fourier_substage_enabled=False`, `cg_inner_tol=1.0e-10` (validation `> 0`), `cg_max_iter=500` (validation `>= 1`). `/diagnostics/fld_substage_audit/v1/` is additive and omitted unless `diagnostic_radial_fourier_substage_enabled=True`; absence is the default reader state, so no existing-reader migration is required.
- 2026-08-25: mesh: bridge grading "log" + spacing floor for multiblock_polar_tier_cart_center (EQR ring cure, design doc §17.1)
- 2026-08-25: mesh: "log" grading v2 is boundary-continuous and ratio-bounded; adds `multiblock_cart_core_bridge_ratio_max`.
- 2026-08-25: adds read-only `Numerics.diagnostics.refinement_estimator.{enabled,every,filter_eps,detect_cutoff}` and the additive optional snapshot dataset `hydro/refine_error`; legacy frozen-config default completion supplies the compiled defaults, with no HDF5 schema change. (W1 shipped detect_cutoff default 0.5.)
- 2026-08-25: upgrades the estimator to the authoritative FLASH4.7 form (mixed second-derivative terms + physical metric factors) and changes the `detect_cutoff` default 0.5 -> 0.8 (FLASH refine_cutoff); since the key is always emitted in frozen configs, checkpoints frozen with 0.5 restart-compare against 0.5 as recorded (pre-production key; no migration). W1.2 makes the metric factors orientation-invariant (absolute center-to-center distances).
- 2026-08-26: adds the default-off `Numerics.diagnostics.refinement_autopilot.*` in-run stitched front tracker and SEARCH/TRACKED/ARMED SHADOW decision logger; it never modifies run state or triggers remesh, and legacy frozen-config default completion supplies all defaults.
- 2026-08-26: adds `refinement_autopilot.mode="arm_exit"` and `ckpt_lead_h`: the armed tracker requests in-window checkpoints, writes `autopilot_decision.json` only when the WOULD_FIRE state and commit checkpoint land on the same step, and terminates with `termination_reason="autopilot_fire"`; no HDF5 schema change is introduced and `run_info.json` gains no new fields.
- 2026-08-26: adds single-block estimator support with a geometry-forked metric (unchanged polar arc factors and rectangular coordinate-difference factors), plus default-off `Numerics.diagnostics.conduction_energy_rate_export.enabled` and the additive snapshot dataset `hydro/conduction_e_rate`; the HDF5 schema version is unchanged.
- 2026-08-26: adds standalone `Numerics.ale.band_ale.bands="estimator"`, its `estimator_band_{cut,in_rows,out_rows}` window controls, and the additive per-transaction `[band-ale-ledger]` log; no checkpoint or HDF5 schema changes are introduced.
- 2026-08-26: replaces the Estimator band's compression-sensitive A/A0 trigger with `eta_h` spacing-nonuniformity hysteresis and replaces its initial-fraction respace target with uniform fractions of the current endpoint radii; other band kinds are unchanged.
- 2026-08-26: adds `Numerics.ale.band_ale.estimator_band_shock_hold=0.90`; the Estimator band remains inactive while any expanded-window row mean reaches the hold threshold, preventing maintenance respacing across the measured A452 traveling front.
- 2026-08-26: adds `Numerics.ale.band_ale.estimator_band_front_hold_margin_rows=16.0` and the tracker-coupled front hold: the Estimator band defers while the refinement tracker's median front row lies within the margin of the window, closing the forming-front gap of the static shock-hold (row means 0.5–0.9); `bands="estimator"` now requires `refinement_autopilot.enabled` (fail-closed).
- 2026-08-26: single-block generalization of the Estimator band: `bands="estimator"` additionally accepts single-block structured 2D_RZ meshes; new key `Numerics.ale.band_ale.estimator_band_axis="auto"` resolves the layer axis (spherical-polar families -> `"i"`, rectangular -> `"j"`); spacing and respace operate on line-mean layer coordinates; the single-block prescribed-target transaction validates the candidate host-side (positive corner Jacobians, quad area, exact RZ volume against the relative volume floor) with accept-or-reject semantics (no sigma-linesearch); in band-ALE mode the single-block per-step conservative-remap phase is bypassed (transactions are the only remap consumer). Validation now rejects `refinement_autopilot.arm_exit` on single-block meshes and rejects `Numerics.conduction.enabled` with non-single-block 2D_RZ topologies (both previously accepted-but-unrunnable at runtime).
- 2026-08-26: adds the default-off `Numerics.ale.band_ale.estimator_band_per_column` master and ten `estimator_band_pc_*` controls for Phase-A per-column membership, physical-radius edge regularization, shared-node per-spoke targets, global front hold, coverage/trigger hysteresis, adaptive chi, retrospective sigma starvation, cooldown, and deterministic decision hashing; asymmetric input never falls back to the angle-mean operator, while perturbation-disabled input keeps that operator authoritative and runs B1X-A in shadow.
- 2026-08-27: adds default-off `estimator_band_pc_phase_b` plus `estimator_band_pc_tube_dilate_rows`, `estimator_band_pc_tube_dilate_cols_extra`, and `estimator_band_pc_ambiguous_hold_fraction` for the four-sample traveling/persistent discriminator, dilated geometric front tube, conservative incident-cell exclusion, patch-width floor, and priority-ordered global-hold escalation; Phase A remains the default and the runtime-only ridge history resets on run start and after an epoch swap.
- 2026-08-27: table-EOS support in the structured conservative RZ remap, transaction-only (v1): band-ALE prescribed-target transactions may now run on tabulated-EOS materials (single material, separate-internal-energy mode). The post-remap closure inverts the per-species (rho,T) tables from the remapped (rho,e_s) — the remapped extensive energies remain authoritative; only temperature floors change energy, ledgered as the actual stored delta. Out-of-table states, nonfinite closures, or any nonzero boundary flux REJECT the transaction with a bitwise state rollback ("[remap-table-eos]" log). The per-step conservative remap, CSR/multiblock remap, total-energy modes, and per-material conservation remain ideal-gas-only (fail-closed asserts). No namelist keys added; the ideal-gas paths are unchanged (separate kernel instantiations).
- 2026-08-27: `Numerics.ale.band_ale.compose_with_rezone=False` — opt-in composition of the single-block Estimator band with the standard triggered-rezone flow (standard flow first, band transactions on the post-rezone mesh; single-block + bands="estimator" only, multiblock rejected). Default off is byte-identical to the previous band-mode bypass.
- 2026-08-27: `Numerics.ale.evacuated_cell` (default disabled) — the evacuated-cell vacuum-conversion policy for ALE-managed laser exteriors: cells whose mass collapses below a reference-anchored threshold (with hysteresis and laser/coupling/recipient gates) are conservatively emptied into face-area-weighted neighbors (ledgered, 64-eps audited) and excluded from hydro/CFL/conduction while remaining in FLD; single-block 2D + ALE + conduction-energy-rate export required, SNB nonlocal conduction rejected (fail-closed). Shadow diagnostics live separately under `Numerics.diagnostics.evacuated_cell_shadow`.
- 2026-08-27: `Numerics.diagnostics.evacuated_cell_shadow` (default disabled) — log-only mass-classification diagnostics ([evac-cell] lines).
- 2026-08-28: `Numerics.ale.evacuated_cell.rematerialize_*` (5 keys, active only inside the disabled-by-default policy) — conservative re-materialization of converted vacuum slots: timed / volume-threshold / predicted-closure triggers, per-donor-clamped partial fill from face-neighbor withdrawal (ledgered `fill=` fraction, 64-eps audited), and a re-conversion dwell. Rationale: a vacuum slot has no pressure back-reaction, so the converging flow closes it geometrically (~16 ps measured on the L2 laser deck); re-materialization ends the vacuum episode before geometric failure.
- 2026-08-28: `Numerics.ale.evacuated_cell.closure_contact.*` (10 keys, default enabled inside the disabled-by-default policy) — the void-closure contact endgame: per-node-pair gap tracking with predicted arming, a momentum-conserving engagement projection (relative-KE loss to ion internal energy, 64-eps audited), a frictionless unilateral normal-acceleration constraint applied in both hydro stages, tension release with hysteresis, minimum-viable-refill gating (stops the refill/re-ablation mass pump), a live-cell closure gate (wedge closures whose mass never crosses the vacuum thresholds), contact-constrained rezone projection, and a capacityless conduction junction through the closed slot. Measured motivation: channel closure at ~38-39 ps is policy-independent on the L2 deck (three realizations) — the closed state needs a contact representation.
- 2026-08-29: Band-ALE v2 adds the catchment common-mode activation lane, the pole-theta noise-gated routine lane and hard-only legacy lane, continuous sigma refinement, the respace displacement cap, and the Estimator physical-compression hold; the `closure_catchment_engage_ratio` / `closure_catchment_release_ratio` keys are removed.
- 2026-08-30: Adds the default-off `Numerics.ale.band_ale.pole_theta_routine_enabled` master switch for the measured-unnecessary-or-harmful routine lane and documents the existing `pole_theta_noise_ceiling=1e-3` structural band-pass ceiling.
...
- schema change log：初期の ALE 整理で旧 1D ALE namelist と `/diagnostics/ale_center/` を削除。V3 で新しい `Numerics.ale1d` を追加し、既定は無効。後続の改修で checkpoint `time_state/ale_last_applied_step` を追加し、旧 checkpoint は -1 で補完するため migration は不要。その後 `Numerics.ale.ke_conservation_closure` を、続いて `Numerics.ale.ke_conservation_closure_audit` を追加した。どちらも既定 False を frozen-config schema 8 migration で補完する。さらに `Numerics.ale.ke_closure_redistribute_floor` を追加し、既定 False を frozen-config schema 9 migration で補完する。history `mesh/ale_rezone_invocations` も追加した。また `Numerics.ale.debug_per_remap_log` を追加し、既定 False を legacy frozen-config default 補完で扱う。Adds `Numerics.ale.predictive_acceptance_*` fields; they are default-off and are added to legacy frozen-config default completion with no HDF5 schema change. Adds `Numerics.hydro.volume_rate_cfl_*`; frozen-config schema 10 fills the default-off values for legacy checkpoint comparison, with no HDF5 schema change. Adds `Numerics.hydro.trial_volume_cfl_*`; frozen-config schema 11 fills the default-off values for legacy checkpoint comparison, with no HDF5 schema change. Adds `Numerics.hydro.corner_jacobian_*`; frozen-config schema 12 fills the default-off values for legacy checkpoint comparison, with no HDF5 schema change. Adds `Numerics.hydro.driver_retry_active_mesh_repair_enabled` and `Numerics.hydro.driver_retry_corner_balance_threshold`; frozen-config schema 13 fills the default-off values for legacy checkpoint comparison, with no HDF5 schema change. Adds `Numerics.ale.corner_jacobian_post_tangle_enabled`; frozen-config schema 14 fills the default-on value for legacy checkpoint comparison, with no HDF5 schema change. Adds `Numerics.ale.local_boundary_repair_enabled`; frozen-config schema 15 fills the default-off value for legacy checkpoint comparison, with no HDF5 schema change. Adds `Numerics.ale.multi_node_boundary_repair_enabled`; frozen-config schema 16 fills the default-off value for legacy checkpoint comparison, with no HDF5 schema change. Adds `Numerics.ale.emergency_cell_deactivation_enabled`; frozen-config schema 17 fills the default-off value for legacy checkpoint comparison, with no HDF5 schema change. Adds `Numerics.hydro.mesh_geometry_soft_fail_enabled`; legacy frozen-config default completion fills the default False value, with no HDF5 schema change. Adds `Numerics.hydro.in_hydro_corner_j_guard_enabled`; legacy frozen-config default completion fills the default False value, with no HDF5 schema change. Adds `Numerics.hydro.regime_aware_corner_j_guard_enabled`, `Numerics.hydro.axis_margin_guard_enabled`, and `Numerics.hydro.axis_guard_band_cells`; legacy frozen-config default completion fills False, False, and 2 respectively, with no HDF5 schema change. Adds `Numerics.ale.multi_node_interior_repair_enabled` and `Numerics.hydro.driver_retry_use_suggested_dt_enabled`; legacy frozen-config default completion fills both default False values, with no HDF5 schema change. Adds default-off `Numerics.diagnostics.mesh_attribution.*` and the separate JSONL stream `mesh_failure_attribution.jsonl`; legacy frozen-config default completion fills the default values, with no HDF5 schema or dt_lineage format change. Adds `Numerics.dt.floor_stall_max_consecutive_steps` (int, default 0, validation >=0); legacy frozen-config default completion fills 0, with no HDF5 schema change. Adds `Numerics.ale.axis_variational_projection_enabled`; frozen-config schema V18 fills the default False value for legacy V17 checkpoints, with no HDF5 schema change. Adds `Numerics.hydro.dispatcher_state_sensitive_bypass_enabled` (bool, default False) and `Numerics.hydro.dispatcher_state_sensitive_repair_cap_per_step` (int, default 3, validation >=1); legacy frozen-config default completion fills both default values, with no HDF5 schema change. Adds 7 new namelist keys under [Numerics.ale]: axis_band_managed_remap_enabled (bool, default False), axis_band_managed_remap_width (int, default 3, validation >= 1), axis_band_managed_remap_max_width (int, default 6, validation >= width), axis_band_managed_remap_every_hydro_half_step (bool, default True), axis_band_managed_remap_margin_trigger (double, default 1e-4, validation > 0), axis_band_managed_remap_equal_volume (bool, default True), axis_band_managed_remap_include_radiation_groups (bool, default True); legacy frozen-config default completion fills all defaults, with no HDF5 schema change. Extends `Numerics.profile.icf_standard_ale.forbidden_when_enabled` with `hydro_driver_retry_active_mesh_repair_enabled_forbidden_value=True`; no HDF5 schema change. Adds 5 new keys under [Numerics.diagnostics] (icf.enabled, icf.rho_inner_threshold_g_per_cc, icf.rho_outer_threshold_g_per_cc, conservation.enabled, ale_provenance_emission.enabled); legacy frozen-config default completion fills False/0/0 with no HDF5 schema change. Adds `Numerics.profile.icf_standard_ale.claim_level` (string, default `"characterization"`, one of `{characterization, pre_plic_smoke, production_comparable}`); legacy frozen-config V19 default completion fills `"characterization"` with no HDF5 schema change. Adds `Numerics.hydro.strategy_first_retry_enabled` (bool, default False) and `Numerics.hydro.strategy_first_max_same_dt_attempts` (int, default 2, validation >=0); legacy frozen-config default completion fills both defaults, with no HDF5 schema change. Adds default-off `Numerics.diagnostics.mesh_degeneracy_forensics.*` and the separate JSONL stream `mesh_degeneracy_forensics.jsonl`; legacy frozen-config default completion fills the defaults, with no HDF5 schema change. Adds default-off `Numerics.diagnostics.mesh_degeneracy_forensics.corner_j_source_budget_*`; legacy frozen-config default completion fills False, with no HDF5 schema change. The 2026-07-26 kernel review adds `Numerics.hydro.crossing_dt_safety` (float, default 0.5), `Numerics.hydro.time_integrator` (string, default "legacy_pc"), `Numerics.hydro.adaptive_av.hysteresis_tau` (float, default 0.0), and `Mesh.grid.grading.mapping` (string, default "legacy_estimated_radius"); legacy frozen-config default completion fills all four defaults, with no HDF5 schema change. 旧historyで不在の場合は unknown として扱い、既存readerのmigrationは不要。The 2026-07-31 laser-march revision adds `Laser.raytrace.ds_adapt_theta_target` (float, default 0.04, `<=0` disables); legacy frozen-config default completion fills the default, with no HDF5 schema change. The 2026-08-03 AV revision: raw 1D_SPH decks omitting `Numerics.hydro.av_type` now resolve to "csw" in the builder (struct default stays "vnr"; frozen configs always emit av_type explicitly, so legacy frozen-config replay is unchanged; no HDF5 schema change). The 2026-08-07 performance revision adds `Numerics.ale1d.min_width_floor` (nested dict: enabled bool default False, floor_cm double default 0.0, target_factor double default 1.25, relief_halfwidth_cells int default 3, max_growth_factor double default 1.8); legacy frozen-config default completion fills the defaults, no HDF5 schema change. The same revision also flips `Laser.raytrace_skip_config.enabled` default True→False (the old True default was universally inert because the pre-fix crit guard vetoed every call on overdense targets — behavior-compatible) and changes the crit guard semantics to a cache-relative band-crossing test (NUMERICS §5.9.4); frozen configs emit the fields explicitly so legacy replay is unchanged, no HDF5 schema change. The 2026-08-10 performance revision adds `Numerics.ale1d.min_width_floor.retrigger_cooldown_steps` (int, default 0, validation >=0): after a floor-triggered rezone attempt that is not applied (any reason), the floor-trigger evaluation is skipped for that many steps, and any applied rezone resets the cooldown; legacy frozen-config default completion fills 0, the in-memory cooldown counter is not checkpointed (a restart re-evaluates the floor once and re-arms on rejection), no HDF5 schema change. The 2026-09-23 revision adds `Numerics.ale1d.emergency_max_dr_ratio` (float, default 1.25, validation >= 1; the mesh-quality trigger used `candidate_dt_penalty_max` as its threshold before), validates `candidate_dt_penalty_max >= 1` and `benefit_min_dt_gain >= 1`, and evaluates the candidate gates those two keys name; legacy frozen-config default completion fills the new key and now also the `min_width_floor` keys, which the 2026-08-07 and 2026-08-10 revisions described but did not complete (a checkpoint older than them failed the restart frozen-config equivalence check); no HDF5 schema change.
Langdon default-on (2026-08-10): `Laser.ib.langdon_model` gains `"auto"` (new default; resolves to `"legacy_vacuum_map"` for enabled 1D_SPH raytrace configs whose beams share one gaussian/super_gaussian/flat_top profile, else `"off"`), the species requirement narrows to `zeff_model="sequential_strip"` only, and the flat_top vacuum-map radius is corrected to w0; frozen configs emit the resolved value, so legacy frozen replay is unchanged. The 2026-09-03 mesh-requirement revision adds `Mesh.resolution_requirement` (dict: enabled bool default True, apply string default "report", zones_per_scale_length int 8, scale_length_factor 0.12, ablation_mass_safety 1.5, formation_ablated_fraction 0.1, absorbed_fraction 1.0, shock_cells_per_separation int 8, shock_event_min_separation_frac 0.05, min_cells_per_layer int 10, zbar_override 0.0, n_bands int 6); the frozen config emits the dict only for decks that wrote the block (absent on both sides otherwise, so legacy checkpoints compare unchanged), and a new run-output file `mesh_requirement.json` is written at run start; no HDF5 schema change. 2026-09-04: calibrated `Mesh.resolution_requirement` defaults (zones_per_scale_length 9) and added `intensity_exponent` (0.4) / `intensity_reference_W_cm2` (1e14) from the mesh-convergence campaign; frozen emission unchanged (block-gated); no HDF5 schema change.
- add Numerics.diagnostics.r_momentum_source_audit (log-only source-balanced R-momentum audit; default false, bit-neutral)
...
- Adds the exact P1 triangle-degenerate corner-mass closed form w_k=(r_k+Σr)/(4Σr) under kinematic_basis_rz_v1.
- Corner-mass default update: corner_mass_convention default bbsw_radial_v0 -> kinematic_basis_rz_v1 (legacy frozen configs pin to bbsw_radial_v0).
- add Numerics.hydro.time_integration and Numerics.hydro.total_energy_identity_check (the midpoint-integration update; defaults pc_v0/false; legacy frozen-config default completion; no HDF5 schema change)
- Time-integration default update: time_integration default pc_v0 -> midpoint_v1 (legacy frozen configs pin to pc_v0).
  A 2026-07-27 review adds Numerics.profile.legacy_regression.enabled (bool, default False) and Numerics.profile.legacy_regression.revision (string, default "2026-07-27", known values {"2026-07-27"}); legacy frozen-config default completion fills both defaults, with no HDF5 schema change. A follow-up on 2026-07-27 flips the default of Numerics.ale.swept_volume_sign_fixed to True (corrected oriented donor convention); legacy behavior requires Numerics.profile.legacy_regression@2026-07-27 or an explicit False, and restart files enforce their recorded resolved swept-volume contract (metadata/swept_volume_contract).
- Adds `Numerics.ale.dgcl_commit_gate` (bool, default False) and `Numerics.ale.dgcl_commit_rtol` (double, default 1.0e-11); legacy frozen-config default completion fills both defaults, and frozen-config emits them only when enabled or non-default. No HDF5 schema change.
- Renames the production-audit log labels and internal identifiers from `gcl` to `vol_closure`; log labels only; HDF5 unchanged.
- Adds `Numerics.plic.rho_material_aware_donor` (bool, default False); legacy frozen-config default completion fills False, with no HDF5 schema change.
...
- A124(b) (2026-08-15) adds two fail-loud guards to the corner-mass layer (structured assert: aw_compatible_force_work without the canonical subzonal partition; multiblock trap: finite mixed-sign pentagon/star-P1 vertex weights beyond a 1e-12 relative band) and documents the per-topology corner-mass contract incl. the parent-volume fallback's folded-quad mass-closure violation (docs/design/a124b_corner_mass_contract_20260815.md); no namelist key, no HDF5 schema change, bit-neutral on every reachable valid configuration.
...
- Adds `Numerics.hydro.geometric_retry_stagnation.*` (default-off fail-fast retry diagnostic); legacy frozen-config default completion fills the defaults, with no HDF5 schema change.
...
- I1-B Tier 2 adds no namelist keys and no HDF5 root schema change. It additively extends existing hotspot/areal-density diagnostics with optional snapshot/history datasets: `/diagnostics/hotspot_gas/v1/hotspot_Te_*`, 2T-only `hotspot_Ti_*`, `hotspot_energy_*`, `hotspot_work_proxy_*`, snapshot `/diagnostics/areal_density/v1/rhoR_hotspot_tracer`, and history `implosion/rho_R_hotspot_tracer` plus per-angle scalar aliases. Absence means the diagnostic was disabled or the gas tracer was unavailable; old readers may ignore presence.
- Adds `Numerics.hydro.cascade_on_hydro_retry_enabled` (bool, default False); legacy frozen-config default completion fills False, with no HDF5 schema change.
- Adds `Numerics.ale.force_rezone_every_n_steps` (int, default 0, validation >=0); legacy frozen-config default completion fills 0, with no HDF5 schema change.
- Adds `Numerics.ale.corner_post_tangle_strict_floor_enabled` (bool, default False); legacy frozen-config default completion fills False, with no HDF5 schema change.
...
- Phase II-a adds default-off `Numerics.hydro.aw_compatible_force_work`; the frozen config elides False and emits the field only when True. No HDF5 schema change.
...
- Endgame E0 adds scalar int32 `/metadata/pentagon_corner_mass_partition_version=2` (physics epoch `clean_pentagon_v2`). Restart rejects a missing or non-2 value as an artifact-substrate checkpoint; `TENRYU_I1B_RESTART_FORENSIC=1` permits forensic loading with a non-production-comparable warning. `TENRYU_I1B_RESTART_ALLOW_CONFIG_DRIFT` does not bypass this epoch check.
...
- 2026-08-30 `mesh_epoch_enabled` and `mesh_epoch_max_per_step` added to `Numerics.ale`; defaults are filled by legacy frozen-config default completion. No HDF5 schema change.
...
- 2026-05-12: adds default-off `Numerics.diagnostics.mesh_degeneracy_forensics.velocity_history_*` and the separate JSONL stream `mesh_degeneracy_velocity_history_<i>_<j>.jsonl`; legacy frozen-config default completion fills `False`, `-1`, `1`, `True`, and `5000`. No HDF5 schema change.
- production-audit infrastructure (commits cc62bad5, f56f8612, 547d894c, f08646b8) bumps frozen-config schema V22 -> V23. V23 adds default-off `Numerics.diagnostics.production_audit.*` fields: `enabled=False`, `tier="none"`, `audit_json_path="<output_dir>/audit_summary.json"`, `escape_valve_budget.mass_max=0.0`, `escape_valve_budget.energy_max=0.0`, `region_of_interest=[]`, `gcl.enabled=False`, `positivity.enabled=False`, and `positivity.fatal_on_neg=False`.
...
- I1B-GEO adds default-off `Numerics.diagnostics.mesh_quality_min.enabled` and optional `/diagnostics/mesh_quality_min/v1/`. The group is emitted only when that dedicated flag is true, is additive, backward-compatible, and diagnostic-only; old readers ignore it when absent or unknown, so no migration is required.
...
**Production-audit output schema**：
...
- opacity.tmat_skip_lte_repair: bool（既定 False。False = non-LTE tables get the PE:=PA repair on flagged nodes；True = load raw PE、CRE A/B decks 用。2026-09-14 修正: 従来は namelist 検証時の変換にしか渡っておらず、実行時の表再読み込み（FLD 1D/2D・S_N 1D/2D・IMC・persistent loop・硬 X 線診断）では常に修復が適用されていた — 全読み込み点へ配線。`compute_dt_rad_limit` の dt 制限は従来どおり常に修復を省略）
- opacity.tmat_kirchhoff_pe: bool（既定 False。True = TMAT 読み込み時に全ノード・全群で κ_PE := κ_PA（Kirchhoff の法則）。PROPACEOS 由来の表（変換時 kirchhoff_pe=0）は各群の Wien 裾で κ_PE が 1e-99 へのアンダーフローや κ_PA の 10³ 倍の過大値を持ち、Fleck 因子が数 meV で桁跳びして FLD 外側反復が収束しない — LTE 表では True にする。`is_lte` 属性は変えない（NLTE 係数経路のまま値だけ置換）。`tmat_skip_lte_repair` と同様に凍結 namelist には出力しない。NIF DS デッキ `examples/nifds/liquid_d2_1d.py` は True。2026-09-14）
...
**Numerics / dt**：
- dt：initial_s=1e-15, max_s=1e-9, min_s=1e-20, growth_factor=1.2, cfl_hydro=0.3, cfl_length_2d="sqrt_area", edge_accel_displacement_cfl_enabled=False, cfl_cond=0.25, f_min_fleck=0.01, floor_stall_max_consecutive_steps=0（爆縮典型値。ユーザ調整必須。floor stall detector と edge acceleration displacement CFL は既定で無効）
- `Numerics.dt.cfl_length_2d`: string, default `"sqrt_area"`, allowed `{"sqrt_area", "min_altitude"}`; legacy frozen-config default completion fills `"sqrt_area"`.
- `Numerics.dt.edge_accel_displacement_cfl_enabled`: bool, default False; legacy frozen-config default completion fills False.
- `Numerics.dt.floor_stall_max_consecutive_steps`: int, default 0, validation `>= 0`; 32 is the recommended production value that decks may opt into.
- numerics persistent_loop：enabled=False（**既定 OFF・bit 恒等**）, chunk_steps=128
- numerics z_reflection：mode=`"off"`（許可値 `{"off","audit"}`）
...
- hydro T_start_inactive_cells="passive_fill"（許可値 `{"passive_fill","rigid_wall","cold_equilibrium"}`；NUMERICS §2.1.1準拠）
...
- conduction：enabled=True, solver=sts, sts_floor_limiter=net, ion_conduction=False, ion_f_lim=1.0, f_lim=0.06, mfp_limiter_C=0.0, sts_damping=0.01, sts_max_stages=40, sts_subcycle_eta=0.9, sts_total_stages_max=200000, halo_strategy=every, hypre_rtol=1e-8, hypre_max_iter=50, hypre_amg_coarsen=10, hypre_amg_relax=18, hypre_amg_interp=6, hypre_amg_levels=25, face_kappa_policy = "kirchhoff_same_material", nonlocal_model="none", snb_n_groups=24, snb_E_max_over_Te=20.0, snb_mfp="geometric_r2", snb_efield="none", snb_picard_max_iters=8, snb_picard_rtol=0.01
...
- diagnostics.shock_approach: enabled=False, every=50, target_radius_cm=0.0, bins=192, h_cell_cm=0.0, sectors=0, modal_l_max=4, sector_confidence_nu=2.75, sector_guard_crossings=9.0
- `Numerics.diagnostics.shock_approach.sectors`: int, default `0`, allowed `0` or even integers `>=4`; legacy frozen-config default completion fills `0`.
- `Numerics.diagnostics.shock_approach.modal_l_max`: int, default `4`, allowed `[0,4]`; legacy frozen-config default completion fills `4`.
- `Numerics.diagnostics.shock_approach.sector_confidence_nu`: float, default `2.75`, validation `>0`; legacy frozen-config default completion fills `2.75`.
- `Numerics.diagnostics.shock_approach.sector_guard_crossings`: float, default `9.0`, validation `>=0`; legacy frozen-config default completion fills `9.0`.
...
| `rho_material_aware_donor` | `False` | bool; CF6 preview |
...
hard step functions and are not recommended for production. The initial range was
[8, 16]; the CF6 empirical-calibration range expanded to [4, 16] to allow faster init for empirical runs.

The material-volume remap runtime semantics are intentionally narrower than the namelist surface:
...
The CF6 preview adds `rho_material_aware_donor=False` by default.  When enabled, the
...
The PLIC material-volume remap is serial-only.  A configuration with `part.n_ranks > 1` and
...
A path-versioned history group is added
`/diagnostics/material_interface/v1/`. `kSchemaVersion` remains 1 because this
is additive. When PLIC is disabled, the group is omitted. At introduction, the
group is created only when PLIC is enabled and writes identification attributes:
...
The provenance wiring fills the group with per-sample scalars (`time_s`, `step`,
...
The material-volume remap also exposes the in-memory `plic_remap_fallback_engaged` observability
flag for provenance attribute wiring.  The HDF5 schema version remains
...
constraint. It is enabled only when `numerics.plic.enabled=True`. The claim policy
...
denominator.  The material-volume remap supplies the remap fallback signal and the maximum
...
scalar ALE remap baseline, completing the material-volume remap is still allowed, but the provenance/calibration stages must
deny a `production_comparable` claim until calibration or optimization closes
the gap.  On failure, the claim policy downgrades the run claim rather than allowing a
...
under `/diagnostics/ale_provenance/v1/`. The CF6 empirical calibration is
