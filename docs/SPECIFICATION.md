...
- `dimension: Literal["1D_SPH","1D_CYL","2D_RZ"]`
  - `"1D_CYL"`：1D cylindrical per-unit-length radial hydro（initial scope: pure hydro core — radiation/laser/conduction/ale1d are rejected at validation; NUMERICS 1D geometry note; verification gate H3-RADIAL-CYL）
...
- `multigroup_diffusion: dict`（`mode="multigroup_diffusion"` で使用。NUMERICS §6.7 参照）
  ...
  - `outer_accel: string`（既定 `"none"`；許容値 `{"none","anderson"}`；opt-in Anderson acceleration of the FLD outer fixed-point iteration; the converged exit is always the raw Newton output。2D_RZ に加えて 1D_SPH にも適用（2026-09-14 移植、`fld_anderson.cuh`; 有効時は 1D 外側ループのパイプライン化を使わない）。NIF DS デッキ `examples/nifds/liquid_d2_1d.py` は `"anderson"`）
...
- `persistent_loop: dict`（Persistent-loop v1 dispatch scaffolding。v1 では opt-in decision と scope refusal のみを行い、multi-kernel path を実行する）
...
- `materials: dict`（per-material state/schema introduction; top-level `Materials(...)` とは別名前空間）
  - `per_material_conservation_enabled: bool`（既定 `False`；per-material conservation arrays の master switch。False では `State.mass_per_material`, `Ee_per_material`, `Ei_per_material`, `Te_per_material`, `Ti_per_material` と valid flags は size 0 のまま）
...
- `hydro: dict`
  ...
  - `csw98_limiter_shock_floor_enabled: bool`（既定 `false`；Floor the 2D csw98 edge-AV limiter attenuation at `csw_shock_limiter_floor` on compressive sides (1D-parity shock floor). Default false is bit-identical to prior behavior.）
