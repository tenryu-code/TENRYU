...
- `ib: dict`（拡張 IB 物理。**既定はすべて OFF = 従来とビット同一**。いずれかを有効化した場合 v1 は 1D_SPH 限定（他次元は ConfigError）。NUMERICS §5.4.5 参照。2026-07-30 追加）
  ...
  - `species: list[list[float]]`（既定 `[]`；`[[Z_nuc, x_number], ...]` 最大 4 種、Z_nuc 狭義昇順、x>0、Σx∈[0.99,1.01]。`zeff_model="sequential_strip"` と `langdon_model≠"off"` で必須）
  ...
  - `langdon_model: Literal["off","legacy_vacuum_map"]`（既定 `"off"`。`"legacy_vacuum_map"` は Langdon 1980 の f_L(α) を vacuum-map 局所強度（単一 ±z ガウスビームの真空強度をレイ現在 R で評価）で適用。**単一ビーム + gaussian プロファイル必須**（違反は起動時エラー））
...
- ib：zeff_model=`"off"`, species=`[]`, coulomb_log_model=`"debye"`, langdon_model=`"off"`
...
- すべて既定 OFF = 生産経路は導入前とビット同一（GXII 二重バイナリ比較で 235/235 共通データセット byte 一致を確認）。有効化は v1 では 1D_SPH 限定。
...
- laser raytrace_skip：enabled=True, threshold=0.01, max_consecutive=10, norm=max_relative, crit_guard=0.01
