"""GXII solid CD sphere, 1D_SPH, square pulse 240J / 12ns, FLD radiation transport.

Long-pulse ladder run for ALE evaluation.
...
- Levermore-Pomraning flux limiter (default)
...
Reference run: GXII square 120J/6ns 1D_SPH FLD ALE-off matched TMAT Hugoniot
within 0.4% on shock front compression (peak ρ₂/ρ₁ ≈ 18.4× at t=3.93 ns,
T_e_center ≈ 63.8 eV at peak compression).
"""
...
Main(
    name="gxii_square_240J_12ns_fld_nr1500",
    dimension="1D_SPH",
...
Mesh(
    ...
    grid=dict(
        type="graded",
        segments=[{"r_start": 0.0, "r_end": R_SPHERE, "nr": 1500}],
...
def Te_profile(r_cm, z_cm=0.0):
    return 1.0  # opacity table T_min = 1.0 eV
...
Output(
    directory="outputs/gxii_square_240J_12ns_fld_nr1500",
    format="hdf5",
