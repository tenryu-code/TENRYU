...
NR = int(os.environ.get("TENRYU_MULTIBLOCK_HYDRO_SMOKE_NR", "8"))
NZ = int(os.environ.get("TENRYU_MULTIBLOCK_HYDRO_SMOKE_NZ", "32"))
SEED = int(os.environ.get("TENRYU_MULTIBLOCK_HYDRO_SMOKE_SEED", "12345"))
S_MAX = float(os.environ.get("TENRYU_MULTIBLOCK_HYDRO_SMOKE_S_MAX_CM", "0.07"))
T_END = float(os.environ.get("TENRYU_MULTIBLOCK_HYDRO_SMOKE_T_END_S", "1.0e-9"))
DT = float(os.environ.get("TENRYU_MULTIBLOCK_HYDRO_SMOKE_DT_S", "1.0e-9"))
ALE_ENABLED = os.environ.get("TENRYU_S4T1_ALE_ENABLED", "0") == "1"
ALE_CONFIG = dict(enabled=ALE_ENABLED)
if ALE_ENABLED:
    ALE_CONFIG["conservative_remap_enabled"] = False
OUTDIR = os.environ.get(
    "TENRYU_MULTIBLOCK_HYDRO_SMOKE_OUTDIR",
    "./build/output_verify_2d_rz_multiblock_hydro_smoke",
)
...
CASE_NAME = f"2d_rz_multiblock_hydro_smoke_nr{NR}_nz{NZ}_seed{SEED}"

print(
    "[deck:2d_rz_multiblock_hydro_smoke] "
    f"nr={NR} nz={NZ} seed={SEED} S_max_cm={S_MAX} "
...
Numerics(
    ...
    hydro=dict(
        enabled=True,
        boundary_2d=dict(
            # γ multiblock Cartesian core touches R=0; r_inner MUST be "axis"
            # for 2D_RZ when Mesh.r_min==0 per Numerics.hydro.boundary_2d
            # validation. Other boundaries remain reflect (no pressure drive).
            r_inner="axis",
            r_outer="reflect",
            z_bottom="reflect",
            z_top="reflect",
