# belt M1 treatment demo — pentagon-belt (stride-8) + M1 TMOP rezone +
# conservative remap via the multiblock orchestrator (ALE P4); control = the
# plain belt_384x1024 deck (Lagrangian onset 7.208e-7 s in killer-p2b).
import math
import os
...
OUTER_BELT_RADIUS_CM = (NR // 2 + KAPPA + 0.5) * DELTA_S_CM
CROSSING_TIME_ESTIMATE_S = (
    OUTER_BELT_RADIUS_CM - INNER_RING_RADIUS_CM
) / SHOCK_SPEED_CM_S
...
T_END_S = float(os.environ.get("TENRYU_KILLER_T_END_S", "1.2e-6"))

print(
    f"[deck:{CASE_NAME}] nr={NR} ntheta={NTHETA} "
    f"belt_layers={BELT_LAYERS} rho0_gcc={RHO0_GCC} "
    f"P0_dyn_cm2={P0_DYN_CM2} cs0_cm_s={CS0_CM_S} v0_cm_s={V0_CM_S} "
...
Mesh(
    ...
    topology_scheme="pentagon_belt_shell",
    pentagon_belt_layers=BELT_LAYERS,
)
...
Numerics(
    ...
    ale=dict(
        enabled=True,
        every_n_steps=1,
        rezone_solver="m1_tmop",
        m1_theta_reg=1.0,
        m1_sweeps=8,
        m1_lambda_tether=0.1,
