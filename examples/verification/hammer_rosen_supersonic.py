...
nr = int(os.environ.get("TENRYU_HR_NR", "400"))
outdir = os.environ.get("TENRYU_HR_OUTDIR", f"outputs/hammer_rosen_supersonic_nr{nr}")


def hammer_rosen_Tr_eV(t):
    if t < 1.0e-9:
        return 120.0
    if t < 2.0e-9:
        return 120.0 + 80.0 * ((t - 1.0e-9) / 1.0e-9)
    return 200.0


Main(
    name=f"hammer_rosen_supersonic_nr{nr}",
    dimension="1D_SPH",
    t_end=3.0e-9,
    seed=314159,
    max_steps=500000,
