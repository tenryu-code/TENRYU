...
N_C = int(
    os.environ.get(
        "TENRYU_AXIS_REZONE_GATE_N_C",
        os.environ.get("TENRYU_G4_5BLOCK_N_C", "8"),
    )
)
FAN_LAYERS = int(
    os.environ.get(
        "TENRYU_AXIS_REZONE_GATE_FAN_LAYERS",
        os.environ.get("TENRYU_G4_5BLOCK_FAN_LAYERS", str(max(4, round(N_C / 8.0)))),
    )
)
NR_SHELL = int(
    os.environ.get(
        "TENRYU_AXIS_REZONE_GATE_NR_SHELL",
        os.environ.get("TENRYU_G4_5BLOCK_NR_SHELL", str(N_C)),
    )
)
SEED = int(
    os.environ.get(
        "TENRYU_AXIS_REZONE_GATE_SEED",
        os.environ.get("TENRYU_G4_5BLOCK_SEED", "12345"),
    )
)
S_MAX = float(
    os.environ.get(
        "TENRYU_AXIS_REZONE_GATE_S_MAX_CM",
        os.environ.get("TENRYU_G4_5BLOCK_S_MAX_CM", "0.07"),
    )
)
OUTDIR = os.environ.get(
    "TENRYU_AXIS_REZONE_GATE_OUTDIR",
    os.environ.get(
        "TENRYU_G4_5BLOCK_OUTDIR",
        "./build/output_verify_2d_rz_multiblock_5block_axis_rezone_gate",
    ),
)
...
T_END = float(
    os.environ.get(
        "TENRYU_AXIS_REZONE_GATE_T_END_S",
        os.environ.get(
            "TENRYU_G4_5BLOCK_T_END_S",
            str(0.7 * S_MAX / CS_AMBIENT_CM_PER_S),
        ),
    )
)
DT_INITIAL = float(
    os.environ.get(
        "TENRYU_AXIS_REZONE_GATE_DT_INITIAL_S",
        os.environ.get("TENRYU_G4_5BLOCK_DT_INITIAL_S", "1.0e-9"),
    )
)
DT_MAX = float(
    os.environ.get(
        "TENRYU_AXIS_REZONE_GATE_DT_MAX_S",
        os.environ.get("TENRYU_G4_5BLOCK_DT_MAX_S", "1.0e-8"),
    )
)
CFL_HYDRO = float(
    os.environ.get(
        "TENRYU_AXIS_REZONE_GATE_CFL_HYDRO",
        os.environ.get("TENRYU_G4_5BLOCK_CFL_HYDRO", "0.5"),
    )
)
MAX_STEPS = int(
    os.environ.get(
        "TENRYU_AXIS_REZONE_GATE_MAX_STEPS",
        os.environ.get("TENRYU_G4_5BLOCK_MAX_STEPS", "20000"),
    )
)
TRACE_MESH_MOTION = (
    os.environ.get(
        "TENRYU_AXIS_REZONE_GATE_TRACE",
        os.environ.get("TENRYU_G4_5BLOCK_TRACE", "1"),
    )
    == "1"
)
TRACE_MAX_STEPS = int(
    os.environ.get(
        "TENRYU_AXIS_REZONE_GATE_TRACE_MAX_STEPS",
...
CASE_NAME = "multiblock_5block_axis_rezone_gate"

print(
    "[deck:2d_rz_multiblock_5block_axis_rezone_gate] "
    ...
    f"cfl_hydro={CFL_HYDRO} max_steps={MAX_STEPS} axis_rezone=True "
    f"trace_mesh_motion={TRACE_MESH_MOTION} outdir={OUTDIR} "
