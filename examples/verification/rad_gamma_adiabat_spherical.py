...
# W-K gate A (spherical): cold homologous coasting u = H r with a uniform
# seeded radiation field (seeded by the GATE post-load; radiation_field
# "zero" here), zero opacity, reflective radiation boundaries. The only
# actor on E_g is the gamma_r=4/3 volume update: E_g V^{4/3} must be
# invariant per cell.

Main(
    name="rad_gamma_adiabat_spherical",
    dimension="1D_SPH",
    t_end=2.9e-8,
    max_steps=100000,
    seed=12345,
...
Mesh(r_min=0.0, r_max=1.0, nr=64, grid="uniform", geometry_1d="spherical")
...
Geometry(
    ...
    rho=lambda r: 1.0,
    # a T^4 = 137.2 * 29.22^4 ~ 1.0e8 erg/cc: the radiation field is
    # initialized NATIVELY at equilibrium (the solver preserves it; a
    # gate-side seed at cold/out-of-range Te was killed in the first FLD
    # step — measured 2026-07-04).
    Te=lambda r: 1.0e-3,
    Ti=lambda r: 1.0e-3,
    velocity=lambda r: 2.0e6 * r,
    radiation_field="zero",
)

Numerics(
    dt=dict(initial_s=1.0e-10, cfl_hydro=0.3, growth_factor=1.2, max_s=1.0e-9, min_s=1.0e-20),
    # hydro outer must be FREE: the coasting u=H*r expansion would crush the
    # outermost cell against a pinned reflect wall (measured: cell nr-1
    # volume<0 at ~79 ns). The RADIATION boundary stays reflect (no E leak);
    # the per-cell E V^{4/3} law check is trajectory-insensitive.
...
Output(
    directory="./build/output_verify_rad_gamma_adiabat_sph",
