"""G2D-REMAP-B: localized species blob conservation through the CSR conservative
remap (spec section 10 item 5b) — burn network inert (cold), remap active."""
...
Geometry(
    volfrac=dict(
        DT=lambda r, z: 1.0 if abs(z) < 0.02 else 0.0,
        CH=lambda r, z: 0.0 if abs(z) < 0.02 else 1.0,
    ),
    rho=lambda r, z: 10.0,
    Te=lambda r, z: 5.0e1,
    Ti=lambda r, z: 5.0e1,
...
Output(
    directory="build/output_burn_remap_2d_blob",
    plot_every=20,
    history_every=1,
    checkpoint_every=0,
    plot_every_s=-1.0,
