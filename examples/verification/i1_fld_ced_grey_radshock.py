"""FLD constant-Eddington nED grey radiative-shock verification deck."""
...
M0 = _env_float("TENRYU_I1_FLD_CED_M0", 2.0)
NR = _env_int("TENRYU_I1_FLD_CED_NR", 64)
SEED = _env_int("TENRYU_I1_FLD_CED_SEED", 12345)
OUTDIR = os.environ.get("TENRYU_I1_FLD_CED_OUTDIR", "./build/output_verify_i1_fld_ced")
INIT_MODE = os.environ.get("TENRYU_I1_FLD_CED_INIT_MODE", "two_state").lower().replace("-", "_")
if INIT_MODE not in ("reference_table", "two_state"):
    raise ValueError("TENRYU_I1_FLD_CED_INIT_MODE must be reference_table or two_state")
...
default_ref = (
    ...
    / f"fld_ced_M{_safe_float_token(M0)}.json"
)
REFERENCE_PATH = Path(os.environ.get("TENRYU_I1_FLD_CED_REFERENCE", str(default_ref)))
...
R_MIN = _env_float("TENRYU_I1_FLD_CED_R_MIN_CM", 1.0e6)
R_MAX = _env_float("TENRYU_I1_FLD_CED_R_MAX_CM", R_MIN + DOMAIN_SPAN)
if not R_MAX > R_MIN:
    raise ValueError("TENRYU_I1_FLD_CED_R_MAX_CM must be greater than TENRYU_I1_FLD_CED_R_MIN_CM")
...
T_END = _env_float(
    "TENRYU_I1_FLD_CED_T_END_S",
    max(5.0 * TAU_REL, HYDRO_CROSSING_T_END),
)
DT_INITIAL = _env_float("TENRYU_I1_FLD_CED_DT_INITIAL_S", min(1.0e-12, T_END / 1000.0))
DT_MAX = _env_float("TENRYU_I1_FLD_CED_DT_MAX_S", max(DT_INITIAL, T_END / 200.0))
MAX_STEPS = _env_int("TENRYU_I1_FLD_CED_MAX_STEPS", 1000000)
PLOT_EVERY_STEPS = _env_int("TENRYU_I1_FLD_CED_PLOT_EVERY_STEPS", 0)
PLOT_EVERY_S = _env_float("TENRYU_I1_FLD_CED_PLOT_EVERY_S", T_END / 20.0)
HISTORY_EVERY_STEPS = _env_int("TENRYU_I1_FLD_CED_HISTORY_EVERY_STEPS", 1)
CONDUCTION_KAPPA = _env_float("TENRYU_I1_FLD_CED_CONDUCTION_KAPPA", 1.0e-20)

CASE_NAME = (
    f"i1_fld_ced_grey_radshock_M{_safe_float_token(M0)}"
    f"_nr{NR}_seed{SEED}_{INIT_MODE}"
)

print(
    "[deck:i1_fld_ced_grey_radshock] "
    ...
    "hydro=True grey_fld=True flux_limiter=none conduction=True qei=True "
    "laser=False dimension=1D_SPH"
)
...
Main(
    ...
    max_steps=MAX_STEPS,
    seed=SEED,
    verbosity=os.environ.get("TENRYU_I1_FLD_CED_VERBOSITY", "quiet"),
)
...
Numerics(
    ...
    conduction=dict(
        enabled=True,
        solver=os.environ.get("TENRYU_I1_FLD_CED_COND_SOLVER", "sts"),
        f_lim=0.06,
