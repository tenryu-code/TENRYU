...
M0 = _env_float("TENRYU_I1_LR07_M0", 2.0)
NR = _env_int("TENRYU_I1_LR07_NR", 64)
SEED = _env_int("TENRYU_I1_LR07_SEED", 12345)
OUTDIR = os.environ.get("TENRYU_I1_LR07_OUTDIR", "./build/output_verify_i1_lr07_eda")
INIT_MODE = os.environ.get("TENRYU_I1_LR07_INIT_MODE", "two_state").lower().replace("-", "_")
if INIT_MODE not in ("reference_table", "two_state"):
    raise ValueError("TENRYU_I1_LR07_INIT_MODE must be reference_table or two_state")
...
default_ref = (
    ...
    / "data"
    / "lr07_eda_reference"
    / f"lr07_eda_M{_safe_float_token(M0)}.json"
)
REFERENCE_PATH = Path(os.environ.get("TENRYU_I1_LR07_REFERENCE", str(default_ref)))
...
R_MIN = _env_float("TENRYU_I1_LR07_R_MIN_CM", 100.0)
R_MAX = _env_float("TENRYU_I1_LR07_R_MAX_CM", R_MIN + DOMAIN_SPAN)
if not R_MAX > R_MIN:
    raise ValueError("TENRYU_I1_LR07_R_MAX_CM must be greater than TENRYU_I1_LR07_R_MIN_CM")
...
T_END = _env_float(
    "TENRYU_I1_LR07_T_END_S",
    1.5 * DOMAIN_SPAN / max(abs(float(UPSTREAM["u"])), 1.0e-300),
)
DT_INITIAL = _env_float("TENRYU_I1_LR07_DT_INITIAL_S", min(1.0e-12, T_END / 1000.0))
DT_MAX = _env_float("TENRYU_I1_LR07_DT_MAX_S", max(DT_INITIAL, T_END / 200.0))
MAX_STEPS = _env_int("TENRYU_I1_LR07_MAX_STEPS", 1000000)
PLOT_EVERY_STEPS = _env_int("TENRYU_I1_LR07_PLOT_EVERY_STEPS", 0)
PLOT_EVERY_S = _env_float("TENRYU_I1_LR07_PLOT_EVERY_S", T_END / 20.0)
HISTORY_EVERY_STEPS = _env_int("TENRYU_I1_LR07_HISTORY_EVERY_STEPS", 1)
CONDUCTION_KAPPA = _env_float("TENRYU_I1_LR07_CONDUCTION_KAPPA", 1.0e-20)

CASE_NAME = (
    f"i1_lr07_eda_grey_radshock_M{_safe_float_token(M0)}"
    f"_nr{NR}_seed{SEED}_{INIT_MODE}"
)

print(
    "[deck:i1_lr07_eda_grey_radshock] "
    ...
    f"profile_scale={PROFILE_SCALE} t_end_s={T_END} "
...
Main(
    ...
    max_steps=MAX_STEPS,
    seed=SEED,
    verbosity=os.environ.get("TENRYU_I1_LR07_VERBOSITY", "quiet"),
)
...
Numerics(
    ...
    conduction=dict(
        enabled=True,
        solver=os.environ.get("TENRYU_I1_LR07_COND_SOLVER", "sts"),
        f_lim=0.06,
