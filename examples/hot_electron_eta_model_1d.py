"""GXII solid CD sphere, 1D_SPH, with the eta(t) physics model (eta_mode="model").
...
Simon 1983 PhysFluids 26.3107 (TPD threshold); Vu 2012 PoP 19.102703 (TPD
saturation time constant); Colaitis 2016 PoP 23.072703 (SRS conversion
closure).

Published-closure presets (verification §6.3):
TPD-only reproduction: eta_inf=eta_hard_cap=0.026; SRS reproduction:
threshold_multiplier=1, eta_inf=eta_hard_cap=0.125 (NOT production).
...
Laser(
    ...
    hot_electron=dict(
        ...
        angular_model="cone",
        eta_mode="model",
        eta_model=dict(ln_filter_tau_s=5.0e-12, eta_total_cap=0.08),
        ...
        sources=[
            dict(
                ...
                # srs defaults: C=8.0, eta_inf=0.08, eta_hard_cap=0.08,
                # fixed tau=6 ps.
                # Design doc §13.5: capture 0.18 / eval 0.20 separation.
                eval_nc_fraction=0.20,
            ),
            dict(
                mechanism="tpd",       # ring 45+-10 deg around the ray
                T_hot_eV=6.0e4,        # Vu 2012 / Rovere 2023 band
                # Parse-time defaults: threshold_multiplier=1.0,
                # eta_inf=0.01, eta_hard_cap=0.03,
                # relaxation vu2012 clip 3-10 ps.
                eval_nc_fraction=0.25,
