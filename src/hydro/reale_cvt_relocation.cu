...
double coefficient_of_variation(const std::vector<double>& monitor_mass,
...
Assignment assign_samples(
  ...
  assignment.cv = coefficient_of_variation(
      assignment.monitor_mass, assignment.sample_count);
  return assignment;
}
...
CvtRelaxResult reale_cvt_relax(
    ...
    const double damping) {
  validate_inputs(
      gen_r, gen_z, domain, n_iters, grid_nr, grid_nz, damping);

  Assignment assignment =
      assign_samples(gen_r, gen_z, monitor, domain, grid_nr, grid_nz);
  CvtRelaxResult result;
  result.cv_initial = assignment.cv;
  result.cv_per_iteration.reserve(static_cast<std::size_t>(n_iters));
  ...
  for (int iteration = 0; iteration < n_iters; ++iteration) {
    ...
    assignment =
        assign_samples(gen_r, gen_z, monitor, domain, grid_nr, grid_nz);
    result.cv_per_iteration.push_back(assignment.cv);
  }

  result.n_empty = assignment.n_empty;
  result.cv_final =
      n_iters > 0 ? result.cv_per_iteration.back() : result.cv_initial;
  return result;
}
