...
void ensure_parent_groups(const hid_t file, const std::string& dataset_path) {
  ...
  while (true) {
    ...
    if (!group.empty() &&
      ...
      TENRYU_ASSERT(gid >= 0,
                    "PositivityHistory H5Gcreate2 failed: " + group);
      warn_h5_close_failure(H5Gclose(gid),
                            "H5Gclose",
                            "PositivityHistory::ensure_parent_groups");
    }
    ++pos;
  }
}
...
hid_t open_or_create_group(const hid_t file, const std::string& path) {
  ...
  if (tenryu::io::h5_link_exists(file, path.c_str(), H5P_DEFAULT) > 0) {
    ...
    TENRYU_ASSERT(gid >= 0, "PositivityHistory H5Gopen2 failed: " + path);
    return gid;
  }
  const hid_t gid =
      H5Gcreate2(file, path.c_str(), H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  TENRYU_ASSERT(gid >= 0, "PositivityHistory H5Gcreate2 failed: " + path);
  return gid;
}
...
void append_scalar_native(const hid_t file,
  ...
  if (tenryu::io::h5_link_exists(file, path.c_str(), H5P_DEFAULT) > 0) {
    ...
    TENRYU_ASSERT(H5Sselect_hyperslab(file_space,
                                      ...
                                      count,
                                      nullptr) >= 0,
                  "PositivityHistory H5Sselect_hyperslab failed");
    const hid_t mem_space = H5Screate_simple(1, count, nullptr);
    TENRYU_ASSERT(mem_space >= 0,
                  "PositivityHistory H5Screate_simple append failed");
    TENRYU_ASSERT(H5Dwrite(dset,
                           ...
                           H5P_DEFAULT,
                           &value) >= 0,
                  std::string("PositivityHistory H5Dwrite(") + label +
                      " append) failed");
    warn_h5_close_failure(H5Sclose(mem_space),
                          "H5Sclose",
                          "PositivityHistory::append_scalar_native(mem)");
    warn_h5_close_failure(H5Sclose(file_space),
                          "H5Sclose",
                          "PositivityHistory::append_scalar_native(file)");
    warn_h5_close_failure(H5Dclose(dset),
                          "H5Dclose",
                          "PositivityHistory::append_scalar_native(dset)");
    return;
  }
  ...
  TENRYU_ASSERT(space >= 0,
                "PositivityHistory H5Screate_simple create failed");
  const hid_t dcpl = H5Pcreate(H5P_DATASET_CREATE);
  TENRYU_ASSERT(dcpl >= 0, "PositivityHistory H5Pcreate failed");
  TENRYU_ASSERT(H5Pset_chunk(dcpl, 1, chunk) >= 0,
                "PositivityHistory H5Pset_chunk failed");
  const hid_t dset = H5Dcreate2(file,
                                ...
                                H5P_DEFAULT);
  TENRYU_ASSERT(dset >= 0,
                std::string("PositivityHistory H5Dcreate2(") + label +
                    ") failed");
  TENRYU_ASSERT(H5Dwrite(dset,
                         h5_type,
                         H5S_ALL,
  ...
  warn_h5_close_failure(H5Dclose(dset),
                        "H5Dclose",
                        "PositivityHistory::append_scalar_native(create dset)");
  warn_h5_close_failure(H5Pclose(dcpl),
                        "H5Pclose",
                        "PositivityHistory::append_scalar_native(dcpl)");
  warn_h5_close_failure(H5Sclose(space),
                        "H5Sclose",
                        "PositivityHistory::append_scalar_native(space)");
}
...
void write_positivity_history_file(
  ...
  if (!result.enabled || history_path.empty()) {
    return;
  }
  ...
  TENRYU_ASSERT(file >= 0,
                "PositivityHistory failed to open/create history file");

  const std::string group_path = "/diagnostics/positivity/v1";
  const hid_t group = open_or_create_group(file, group_path);
  write_string_attribute_if_missing(
      group, "schema_version", "tenryu.positivity.v1");
  warn_h5_close_failure(H5Gclose(group),
                        "H5Gclose",
                        "PositivityHistory::write group");

  const std::string base = group_path + "/";
  append_scalar_double(file, base + "time_s", result.time);
  append_scalar_i64(file, base + "step", static_cast<std::int64_t>(result.step));
  append_scalar_i64(file,
                    base + "has_negative",
                    result.has_negative ? std::int64_t{1} : std::int64_t{0});
  for (std::size_t i = 0; i < tenryu::verification::kPositivityFieldCount; ++i) {
    append_scalar_double(
        file,
        base + tenryu::verification::positivity_min_dataset_name(
                   static_cast<tenryu::verification::PositivityField>(i)),
        result.minimum[i]);
  }

  warn_h5_close_failure(H5Fclose(file),
                        "H5Fclose",
                        "PositivityHistory::write file");
#else
  (void)history_path;
  (void)result;
#endif
