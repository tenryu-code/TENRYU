...
bool run_su_olson_verify() {
  ...
  // Independent-discretization reference for the AS-BUILT verify model
  // (grey unlimited-diffusion FLD + constant-cv matter): generated by
  // tools/su_olson_fld_reference.py (explicit FTCS on the same 800-cell
  // grid, symmetric exchange, vacuum outer leak cE/2). TENRYU's FLD run
  // matches these to 0.6% / 4% / 14% (xi = 0.01 / 1.0 / 3.16228; the tail
  // point sits on the steep front where both solutions are ~1e-2 of the
  // center). NOTE this is implementation verification, not the published
  // Su & Olson (1997) alpha*T^3 benchmark: the standard FLD matter update
  // has no alpha*T^3 heat capacity (the persistent path does); the
  // transport triple {0.42254748, 0.17776605, 0.01869095} = 0.2*V_transport
  // (tau=10) remains in tools/su_olson_sn_reference.py for a future S_N
  // variant, and 0.2*V_diffusion = {0.350718, 0.193142, 0.019668} is the
  // alpha*T^3 diffusion target if that matter model lands.
  const std::vector<double> xi_probe = {0.01, 1.0, 3.16228};
  const std::vector<double> u_ref = {0.86009429, 0.34196629, 0.00693311};
  ...
  for (std::size_t p = 0; p < xi_probe.size(); ++p) {
    ...
    const double u =
        Te[static_cast<std::size_t>(best_cell)] /
        std::max(T_ref, cfg.numerics.floors.Te);
    u_sim[p] = u;
