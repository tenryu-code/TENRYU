---
name: tenryu-docs-qa
description: TENRYU に関する質問（ビルド・実行・namelist キーと既定値・物理と数値法・出力・GUI・ソースの挙動）に、ローカルチェックアウトの文書とソースだけを根拠に答えるスキル。「TENRYU の〜はどう設定する」「このキーの意味と既定値は」「この物理はどう実装されている」「この出力は何か」と聞かれたときに使う。Use when asked any question about TENRYU that must be answered from the local checkout's documents and source with citations; read-only, no deck generation, no code edits.
metadata:
  short-description: TENRYU 質問応答（文書とソースを根拠に回答）
---
