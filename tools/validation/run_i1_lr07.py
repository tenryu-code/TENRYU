#!/usr/bin/env python3
"""Run and analyze the I1 LR07-EDA grey radiative-shock verification gate."""
...
STAGE_ID = "I1"
LADDER_ROW = "i1_lr07_eda_grey_radshock"
A_EV = 1.3720e2
E_FLOOR = 1.0e-300
...
def parse_grid_list(value: str) -> list[tuple[int, int]]:
    ...
    for item in value.split(";"):
        ...
        grids.append((nr, nz))
    if len(grids) < 3:
        raise argparse.ArgumentTypeError("I1 LR07 requires at least three grids")
    return grids
    ...
    if dimension == "1D_SPH":
        ...
        if bad:
            raise argparse.ArgumentTypeError(
                "1D_SPH LR07 grids must be one-dimensional entries like '64;128;256', "
                f"got {', '.join(bad)}"
            )
...
def case_name(mach: float, nr: int, nz: int, seed: int, init_mode: str, dimension: str) -> str:
    if dimension == "1D_SPH":
        return (
            f"i1_lr07_eda_grey_radshock_M{safe_float_token(mach)}"
...
def lr07_branch_labels(table: dict[str, Any], x: Any) -> Any:
    import numpy as np
...
def lr07_identity_rel_by_branch(
    ref: dict[str, Any],
    flux_field: str,
    branch: str,
    *,
    ...
    x = lr07_table_field(table, "x_cm")
    rho = lr07_table_field(table, "rho_g_per_cc")
    T = lr07_table_field(table, "T_eV")
    F_ref = lr07_table_field(table, flux_field, "F_rad_erg_cm2_s")
    branches = lr07_branch_labels(table, x)
...
def lr07_identity_stats(
    ...
    *,
    ...
    rels = {
        branch: lr07_identity_rel_by_branch(
            ref,
            flux_field,
            branch,
...
def reference_identity_branch_wise(ref: dict[str, Any]) -> dict[str, Any]:
    primary = lr07_identity_stats(
        ref,
        "F_rad_erg_per_cm2_s",
        branch_oriented_flux=False,
    )
    energy_field = (
        "F_rad_energy_invariant_erg_per_cm2_s" if lr07_schema_is_v2(ref) else "F_rad_erg_per_cm2_s"
    )
    energy = lr07_identity_stats(
        ref,
        energy_field,
    ...
    return {
        ...
        "schema_v2": lr07_schema_is_v2(ref),
        "thresholds": {
...
def interpolate_lr07_table_to_cells(table: dict[str, Any], x_cells: Any) -> dict[str, Any]:
    import numpy as np
...
def build_t0_admissibility_reference(
    ...
    if init_mode == "reference_table":
        return interpolate_lr07_table_to_cells(ref_json["table"], x_cells)
...
def profile_l2_three_coords(snapshot: Snapshot, ref: dict[str, Any]) -> dict[str, float]:
    ...
    ref_T = np.asarray(ref["table"]["T_eV"], dtype=float)
...
def profile_metrics(snapshot: Snapshot, ref: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "precursor_Trad_l2": rel_l2(Trad[pre], interp_reference(ref, "T_eV", aligned_x[pre])) if np.count_nonzero(pre) >= 3 else math.inf,
        "profile_l2_three_coords": profile_l2_three_coords(snapshot, ref),
...
def compute_closure_defect_metrics(
    ...
    E_eda_from_Te = a_eV * np.asarray(Te, dtype=float) ** 4
    ...
    if np.asarray(x).size >= 2:
        ...
        dEdr_eda = np.gradient(E_eda_from_Te, x)
    else:
        dEdr_rad = np.full_like(E_from_Trad, np.nan)
        dEdr_eda = np.full_like(E_eda_from_Te, np.nan)
    F_diff_from_Erad = -D * dEdr_rad
    F_diff_from_aTe4 = -D * dEdr_eda
    F_rad_arr = np.asarray(F_rad, dtype=float)
    flux_resid_nonEDA = (F_rad_arr - F_diff_from_Erad) / np.maximum(
        np.abs(F_rad_arr) + np.abs(F_diff_from_Erad), E_FLOOR
    )
    flux_resid_EDA = (F_rad_arr - F_diff_from_aTe4) / np.maximum(
        np.abs(F_rad_arr) + np.abs(F_diff_from_aTe4), E_FLOOR
    )

    return {
        "delta_Tr": delta_Tr,
        "delta_ei": delta_ei,
        "flux_resid_nonEDA": flux_resid_nonEDA,
        "flux_resid_EDA": flux_resid_EDA,
        "F_rad": F_rad_arr,
        "F_diff_from_Erad": F_diff_from_Erad,
        "F_diff_from_aTe4": F_diff_from_aTe4,
...
def closure_defect_summary(
    ...
    return {
        ...
        "max_abs_flux_resid_nonEDA": finite_max_abs(metrics["flux_resid_nonEDA"], interior),
        "max_abs_flux_resid_EDA": finite_max_abs(metrics["flux_resid_EDA"], interior),
    }
...
def aggregate_closure_defects(rows: list[dict[str, Any]]) -> dict[str, Any]:
    if not rows:
        return {
            ...
            "max_abs_flux_resid_nonEDA": math.inf,
            "max_abs_flux_resid_EDA": math.inf,
            "flux_sources": [],
        }
    max_keys = [
        ...
        "max_abs_flux_resid_nonEDA",
        "max_abs_flux_resid_EDA",
    ]
...
def snapshot_dump_arrays(
    ...
    return {
        ...
        "F_diff_aTe4": metrics["F_diff_from_aTe4"],
...
def active_env(
    ...
    *,
    ...
    env = {
        "TENRYU_I1_LR07_M0": str(args.mach),
        "TENRYU_I1_LR07_NR": str(nr),
        "TENRYU_I1_LR07_SEED": str(args.seed),
        "TENRYU_I1_LR07_OUTDIR": str(outdir),
        "TENRYU_I1_LR07_REFERENCE": str(args.reference),
        "TENRYU_I1_LR07_INIT_MODE": args.init_mode,
        "TENRYU_I1_LR07_MAX_STEPS": str(max_steps if max_steps is not None else args.max_steps),
        "TENRYU_I1_LR07_HISTORY_EVERY_STEPS": "1",
        "TENRYU_I1_LR07_VERBOSITY": args.verbosity,
    }
    if dimension == "2D_RZ":
        env["TENRYU_I1_LR07_NZ"] = str(nz)
        env["TENRYU_I1_LR07_MESH_MOTION"] = args.mesh_motion
    ...
    if effective_t_end_s is not None:
        env["TENRYU_I1_LR07_T_END_S"] = str(effective_t_end_s)
    if effective_dt_initial_s is not None:
        env["TENRYU_I1_LR07_DT_INITIAL_S"] = str(effective_dt_initial_s)
    if effective_dt_max_s is not None:
        env["TENRYU_I1_LR07_DT_MAX_S"] = str(effective_dt_max_s)
    return env
...
def t0_diagnostics(snapshot: Snapshot, ref: dict[str, Any], init_mode: str) -> dict[str, Any]:
    ...
    plateau_rel = {
        ...
        "Trad_minus_Te_over_Te": trad_te_mismatch,
    }
    ...
    diagnostics = {
        ...
        "max_abs_T_rad_minus_T_e_over_T_e_at_t0": trad_te_mismatch,
    ...
    diagnostics["gates"] = {
        ...
        "eda_temperature_equilibrium": trad_te_mismatch <= T0_TRAD_TE_REL_GATE,
    }
    if init_mode == "two_state":
        diagnostics["gates"].update(
            {
                ...
                "Trad_Te_plateau_match": plateau_rel["Trad_minus_Te_over_Te"] <= 1.0e-3,
            }
        )
...
def aggregate(rows: list[dict[str, Any]], ref: dict[str, Any]) -> dict[str, Any]:
    ...
    gates = {
        ...
        "gate1_low_prad_and_force": max_prad <= LOW_PRAD_GATE and max_force <= LOW_FORCE_GATE,
    ...
    return {
        ...
        "fine_max_abs_flux_resid_nonEDA": fine_closure.get("max_abs_flux_resid_nonEDA", math.inf),
        "fine_max_abs_flux_resid_EDA": fine_closure.get("max_abs_flux_resid_EDA", math.inf),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--deck", default="examples/verification/i1_lr07_eda_grey_radshock.py")
    parser.add_argument("--reference", default="tests/verification/data/lr07_eda_reference/lr07_eda_M2.json")
    ...
    parser.add_argument("--out-root", default="./build/i1_lr07_eda_runs")
    parser.add_argument("--summary-json", default="./build/i1_lr07_eda_runs/summary.json")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if dimension == "1D_SPH" and args.mesh_motion != "lagrangian":
        parser.error("1D_SPH LR07 requires --mesh-motion lagrangian")
    ...
    if t0_aggregate["passed"]:
        ...
        production_t_end = args.t_end_s if args.t_end_s is not None else reference_t_end_s(ref, 1.5)
        ...
        agg = aggregate(rows, ref)
    payload = {
        "schema_version": "tenryu.i1_lr07_eda_grey_radshock.summary.v6",
    ...
    partial_pass = (
        ...
        and bool(agg.get("gates", {}).get("gate1_low_prad_and_force"))
